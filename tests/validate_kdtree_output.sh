...
cd "${DIR}/.." || exit 1  # cd to project root

# Send output to log file and terminal (log file at project root)
LOG_FILE="validate_kdtree_output.txt"
...
./tests/setup_validate_kdtree_output.sh
...
if [ ! -f "${INPUT_SAGE_DIR}/model_0.hdf5" ]; then
    ...
    echo "Please run 'tests/sage-model-tests/validate_and_time_workflow.sh' (for benchmark output) or 'tests/sage-model-tests/run_test_hdf5_one_step.sh' (for standard output) first to generate input data."
    exit 1
fi
