...
namespace tao {
   ...
   public:
      ...
      typedef boost::mpl::map<boost::mpl::pair<std::string,        boost::mpl::int_<STRING>>,
                              ...
                              boost::mpl::pair<float,              boost::mpl::int_<FLOAT>>> type_map;
            ...
            else if( std::get<1>( field ) == SCALAR )
            {
               switch( std::get<2>( field ) )
               {
                  ...
                  case INTEGER:
                     ...
                     break;
                  case UNSIGNED:
                     std::get<0>( field ) = new std::vector<unsigned>( *boost::any_cast<std::vector<unsigned>*>( val ) );
                     break;
                  case UNSIGNED_LONG:
            ...
            else
            {
               switch( std::get<2>( field ) )
               {
                  ...
                  case INTEGER:
                     ...
                     break;
                  case UNSIGNED:
                     std::get<0>( field ) = new hpc::matrix<unsigned>( *boost::any_cast<hpc::matrix<unsigned>*>( val ) );
                     break;
                  case UNSIGNED_LONG:
      ...
      update_size()
      {
         ...
         for( const auto& item : _fields )
         {
            ...
            switch( std::get<2>( field ) )
            {
               ...
               case UNSIGNED:
                  if( std::get<1>( field ) == SCALAR )
                     _size = std::min<unsigned>( _size, boost::any_cast<std::vector<unsigned>*>( val )->size() );
                  else
                     _size = std::min<unsigned>( _size, boost::any_cast<hpc::matrix<unsigned>*>( val )->n_rows() );
                  break;
               case UNSIGNED_LONG:
                  if( std::get<1>( field ) == SCALAR )
