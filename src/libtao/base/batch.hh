...
public:
    ...
    typedef boost::mpl::map<
        ...
        boost::mpl::pair<unsigned long long, boost::mpl::int_<UNSIGNED_LONG_LONG>>,
...
public:
        ...
        , _mask(src._mask)
    {
        for (const auto& item : src._fields)
        {
            ...
            else if (std::get<1>(field) == SCALAR)
            {
                ...
                case DOUBLE:
                    std::get<0>(field) =
                        new std::vector<double>(*boost::any_cast<std::vector<double>*>(val));
                    break;
                case INTEGER:
                    std::get<0>(field) =
                ...
                case UNSIGNED:
                    std::get<0>(field) =
                        new std::vector<unsigned>(*boost::any_cast<std::vector<unsigned>*>(val));
                    break;
                case UNSIGNED_LONG:
                ...
                case LONG_LONG:
                    std::get<0>(field) =
                        new std::vector<long long>(*boost::any_cast<std::vector<long long>*>(val));
                    break;
                case UNSIGNED_LONG_LONG:
            ...
            else
            {
                ...
                case DOUBLE:
                    std::get<0>(field) =
                        new hpc::matrix<double>(*boost::any_cast<hpc::matrix<double>*>(val));
                    break;
                case INTEGER:
                    std::get<0>(field) =
                ...
                case UNSIGNED:
                    std::get<0>(field) =
                        new hpc::matrix<unsigned>(*boost::any_cast<hpc::matrix<unsigned>*>(val));
                    break;
                case UNSIGNED_LONG:
                ...
                case LONG_LONG:
                    std::get<0>(field) =
                        new hpc::matrix<long long>(*boost::any_cast<hpc::matrix<long long>*>(val));
                    break;
                case UNSIGNED_LONG_LONG:
    ...
    std::unordered_map<std::string, field_type> getfields() const { return _fields; };
        ...
        for (const auto& item : _fields)
        {
            ...
            case STRING:
                if (std::get<1>(field) == SCALAR)
                    ...
                    _size = std::min<unsigned>(
                        _size, boost::any_cast<hpc::matrix<std::string>*>(val)->n_rows());
                break;
            case DOUBLE:
                if (std::get<1>(field) == SCALAR)
                    _size = std::min<unsigned>(_size,
                                               boost::any_cast<std::vector<double>*>(val)->size());
                else
                    _size = std::min<unsigned>(
            ...
            case INTEGER:
                if (std::get<1>(field) == SCALAR)
                    _size =
                        std::min<unsigned>(_size, boost::any_cast<std::vector<int>*>(val)->size());
                else
                    _size = std::min<unsigned>(_size,
                                               boost::any_cast<hpc::matrix<int>*>(val)->n_rows());
                break;
            case UNSIGNED:
                if (std::get<1>(field) == SCALAR)
                    _size = std::min<unsigned>(
            ...
            case UNSIGNED_LONG:
                if (std::get<1>(field) == SCALAR)
                    _size = std::min<unsigned>(
                        _size, boost::any_cast<std::vector<unsigned long>*>(val)->size());
                else
                    _size = std::min<unsigned>(
                        _size, boost::any_cast<hpc::matrix<unsigned long>*>(val)->n_rows());
                break;
            case LONG_LONG:
                if (std::get<1>(field) == SCALAR)
                    _size = std::min<unsigned>(
            ...
            case UNSIGNED_LONG_LONG:
                if (std::get<1>(field) == SCALAR)
                    _size = std::min<unsigned>(
                        _size, boost::any_cast<std::vector<unsigned long long>*>(val)->size());
                else
                    _size = std::min<unsigned>(
                        _size, boost::any_cast<hpc::matrix<unsigned long long>*>(val)->n_rows());
                break;
            case FIELD_VALUE_TERMINAL:
                break;
            };
    ...
    bool has_field(const std::string& name) const { return _fields.find(name) != _fields.end(); }
    ...
    template <class U>
    const U& attribute(const std::string& name)
    {
    ...
    template <class U>
    const typename hpc::view<std::vector<U>> scalar(const std::string& name) const
    {
        ASSERT(std::get<1>(field(name)) == SCALAR,
               "Assertion in batch object. Requesting "
               "field \"",
    ...
    template <class U>
    hpc::matrix<U>& vector(const std::string& name)
    {
    ...
    friend std::ostream& operator<<(std::ostream& strm, const batch& obj) { return strm; }
