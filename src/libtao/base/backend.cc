...
backend::backend(tao::simulation const *sim) : _sim(sim) {}

void backend::set_simulation(tao::simulation const *sim) { _sim = sim; }

tao::simulation const *backend::simulation() const { return _sim; }

void backend::load_checkpoint(boost::property_tree::ptree const &pt) {}

// For some backends there are different fields expected.
// Override in derived classes if additional fields are needed.
void backend::add_conditional_fields(query<real_type> &qry) {}
...
///
void backend::init_batch(batch<real_type> &bat, query<real_type> &qry) const {
  ...
  for (auto const &field : qry.output_fields()) {
    ...
    if (this->_field_map.find(field) != this->_field_map.end()) {
      ...
      if (this->_field_types.find(mapped_field) != this->_field_types.end()) {
        ...
        std::cerr << "WARNING: Alias '" << field << "' maps to '"
                  << mapped_field
                  << "' but that field doesn't exist in _field_types"
                  << std::endl;
      }
