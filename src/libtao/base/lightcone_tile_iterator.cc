...
lightcone_tile_iterator::lightcone_tile_iterator()
    : _lc(0), _done(true), _idx(std::numeric_limits<unsigned>::max()) {}

lightcone_tile_iterator::lightcone_tile_iterator(tao::lightcone const &lc)
    : _lc(&lc), _done(false), _idx(0) {
  ...
  hpc::num::ecs_to_cartesian<real_type>(min_ra, min_dec, crd[0], crd[1], crd[2],
                                        lc.min_dist());
  ...
  bool ok = lc.overlap(
      crd, std::array<real_type, 3>{crd[0] + bs, crd[1] + bs, crd[2] + bs});
  if (!ok) {
    first[0]--;
    crd[0] -= bs;
    ok = lc.overlap(
...
void lightcone_tile_iterator::save_checkpoint(
