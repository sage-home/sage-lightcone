...
namespace tao {
        ...
        _idx( 0 )
   {
      ...
      hpc::num::ecs_to_cartesian<real_type>( min_ra, min_dec,
   ...
   tao::lightcone const*
   lightcone_tile_iterator::lightcone() const
   {
      return _lc;
   }

   bool
   lightcone_tile_iterator::done() const
   {
      return _done;
   }

   unsigned
   lightcone_tile_iterator::index() const
   {
      return _idx;
   }

   std::list<std::array<int,3>> const&
   lightcone_tile_iterator::remaining_tiles() const
   {
      return _rem_tiles;
   }

   std::map<std::array<int,3>,int> const&
   lightcone_tile_iterator::done_tiles() const
   {
      return _done_tiles;
   }

   void
   lightcone_tile_iterator::save_checkpoint( boost::property_tree::ptree& pt ) const
   {
   ...
   lightcone_tile_iterator::increment()
   {
	 ...
	 auto pos = _rem_tiles.front(); _rem_tiles.pop_front();
	 real_type bs = _lc->simulation()->box_size();
	 ...
	 LOGDLN( "Added to done set, which has value: ", cur_done_it->second );
         ...
         for( unsigned ii = 0; ii < 3; ++ii )
         {
		  ...
		  {
                     LOGDLN( "Not in overlap, incrementing current done iterator to: ", cur_done_it->second + 1 );
   ...
   bool
   lightcone_tile_iterator::equal( const lightcone_tile_iterator& op ) const
   {
      return _done == op._done;
   }

   lightcone_tile_iterator::reference_type
   lightcone_tile_iterator::dereference() const
   {
      return _tile;
   }
