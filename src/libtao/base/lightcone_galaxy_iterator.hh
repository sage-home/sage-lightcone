...
namespace tao {
   ...
   public:
      ...
      lightcone_galaxy_iterator( tao::lightcone const& lc,
                                 backend_type& be,
                                 query<real_type>& qry,
                                 tile_iterator tile_it,
                                 tao::batch<real_type>* bat = 0,
                                 filter const* filt = 0,
                                 unsigned first_tile = 0 )
         : _lc( &lc ),
           _be( &be ),
           _qry( &qry ),
           _tile_it(tile_it),
           ...
           _filt( filt ),
           _done( false ),
           _first_tile( first_tile ),
           _last_tile( first_tile )
      {
         ...
         for( unsigned ii = 0; ii < _first_tile && !_tile_it.done(); ++_tile_it, ++ii );
         ...
         if( !_tile_it.done() )
         {
            ...
            /*
            std::cout << "QUERY=["<<_tile_it->translation()[0]<<" "<<_tile_it->translation()[1]<<" "<<_tile_it->translation()[2]<<"] ["<<
                      _tile_it->min()[0]<<" "<<_tile_it->min()[1]<<" "<<_tile_it->min()[2]<<"] ["   <<
                      _tile_it->origin()[0]<<" "<<_tile_it->origin()[1]<<" "<<_tile_it->origin()[2]<<"] ROT["   <<
                      _tile_it->rotation()[0]<<" "<<_tile_it->rotation()[1]<<" "<<_tile_it->rotation()[2]<<"]"   <<
                      std::endl;
            */
            _gal_it = _be->galaxy_begin(
               *_qry,
               *_tile_it,
               _bat,
               _filt,
