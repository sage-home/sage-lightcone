...
void data_dict::saveSidecar(const std::string &fn,
  ...
  for (unsigned iii = 0; iii < fields.size(); iii++) {
    ...
    xmlfile << "      group=\"" << fields[iii]._group << "\">"
            << fields[iii]._name << "</Field>\n";
  }
  xmlfile << "  </sageinput>\n";
  if (as_settings)
    xmlfile << "</settings>\n";
...
herr_t data_dict::getFieldsH5(hid_t g_id, const char *name,
...
std::vector<tao::data_dict_field>
data_dict::getFieldsANY(xml_dict &h5xml, const char *rootnode) {
  ...
  for (const xpath_node *it = fields.begin(); it != fields.end(); ++it) {
    ...
    if (cur.attribute("stats_minimum")) {
      ...
      std::cout << "FOUND stats_minimum[" << field_str << "]=" << stats_minimum
                << std::endl;
    }
    ...
    if (type_str == "int" || type_str == "INT" || type_str == "short" ||
        type_str == "SMALLINT") {
      ...
      field._offset = offset;
      offset += sizeof(ANY);
    } else if (type_str == "float" || type_str == "FLOAT4" ||
               type_str == "FLOAT8") {
...
int32_t data_dict::findField(std::vector<tao::data_dict_field> &fields,
                             std::string name) {
...
int32_t data_dict::findLabel(std::vector<tao::data_dict_field> &fields,
                             std::string label) {
  // I want findField to be case insensitive
...
hpc::h5::derive
data_dict::getComposite(std::vector<tao::data_dict_field> &fields,
  ...
  for (uint32_t i = 0; i < fields.size(); i++) {
    der.add(fields[i]._mtype, fields[i]._offset, fields[i]._ftype,
            fields[i]._name);
  }
  der.commit(mdt, fdt);
  return der;
...
hpc::h5::derive
data_dict::getCompositeANY(std::vector<tao::data_dict_field> &fields,
                           hpc::h5::datatype &mdt, hpc::h5::datatype &fdt) {
  // Build composite data type
  int size = 0;
  for (uint32_t i = 0; i < fields.size(); i++) {
...
void data_dict::setFieldStats(std::vector<tao::data_dict_field> &fields,
                              std::string name, long long min, long long max) {
