...
class bad_option : public hpc::exception {};

///
/// Exception thrown when there is any trouble parsing XML.
///
class bad_xml : public bad_option {};
...
public:
  ...
  xml_dict(std::istream &strm, const std::string &xpath_root = std::string());
  ...
  /// @throws bad_xml If any error occurs during XML parsing.
  ///
  void read(std::istream &stream, const std::string &xpath_root = std::string(),
  ...
  ///
  template <class T> boost::optional<T> opt(const std::string &path) const {
  ...
  ///
  template <class T> std::list<T> get_list(const std::string &path) const {
  ...
  ///
  template <class T>
  std::list<T> get_list(const std::string &path,
  ...
  xpath_node_set get_nodes(const std::string &xpath) const;

  xml_node get_root() const;

protected:
  xml_node _find_root(xml_node &node, const std::string &xpath_root) const;

  void _merge(std::istream &stream, const std::string &path,
              const std::string &filename = std::string());

  void _merge_node(xml_node merge_into, xml_node merge_from);

  xml_node _get_node(const std::string &path, bool except = true) const;

  std::string _xform_path(const std::string &path) const;

  template <class T> T _coerce(const std::string &value) const {
