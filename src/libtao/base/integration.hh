...
real_type integrate(hpc::num::spline_integrator<real_type> const &integ,
                    Spline const &sp) {
...
template <class Spline> real_type integrate(const Spline &sp) {
  hpc::num::spline_integrator<real_type> integ;
  return -M_C * integrate(integ, sp);
}

template <class Spline0, class Spline1>
real_type integrate(const hpc::num::spline_spline_integrator<real_type> &integ,
                    const Spline0 &sp0, const Spline1 &sp1) {
  return integ(sp0, sp1, [](double x, double val0, double val1) {
...
template <class Spline0, class Spline1>
real_type integrate(const Spline0 &sp0, const Spline1 &sp1) {
