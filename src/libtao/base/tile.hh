...
///
///
template <class T> class tile : public box<T> {
public:
  typedef T real_type;
...
public:
  ...
  void set_lightcone(const lightcone *lc,
                     std::array<real_type, 3> offs = {{0, 0, 0}}) {
    if (lc) {
      _lc = lc;
  ...
  galaxy_begin(tao::query<real_type> &query, Backend &be) const {
    return be.galaxy_begin(query, *this);
  }

  template <class Backend>
  typename Backend::tile_galaxy_iterator
  galaxy_end(tao::query<real_type> &query, Backend &be) const {
