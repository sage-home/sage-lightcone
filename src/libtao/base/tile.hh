...
///
template <class T>
class tile : public box<T>
{
public:
    typedef T real_type;

public:
    tile(const tao::lightcone* lc = 0, std::array<real_type, 3> offs = {{0, 0, 0}})
        : box<T>(0)
        , _lc(0)
    {
    ...
    void set_lightcone(const lightcone* lc, std::array<real_type, 3> offs = {{0, 0, 0}})
    {
        if (lc)
        {
            _lc = lc;
    ...
    void set_offset(const std::array<real_type, 3>& offs)
    {
        this->_min = offs;
    ...
    const tao::lightcone* lightcone() const { return _lc; }

    template <class Backend>
    typename Backend::tile_galaxy_iterator galaxy_begin(tao::query<real_type>& query,
                                                        Backend& be) const
    {
    ...
    template <class Backend>
    typename Backend::tile_galaxy_iterator galaxy_end(tao::query<real_type>& query,
                                                      Backend& be) const
    {
        return be.galaxy_end(query, *this);
    }

    bool contains(std::array<real_type, 3> const& crd, unsigned snap) const
    {
        // TODO: Include rotation and shifting.
        std::array<real_type, 3> ecs;
        hpc::num::cartesian_to_ecs<real_type>(crd[0] + this->_min[0], // include tile offset
                                              crd[1] + this->_min[1], // include tile offset
                                              crd[2] + this->_min[2], // include tile offset
                                              ecs[0], ecs[1], ecs[2]);
        return ecs[0] >= _lc->min_ra() && ecs[0] < _lc->max_ra() && ecs[1] >= _lc->min_dec() &&
               ecs[1] < _lc->max_dec() && ecs[2] >= _lc->min_dist(snap) &&
               ecs[2] < _lc->max_dist(snap);
    }

    template <class Iter>
    bool box_collision(Iter bnds_begin, unsigned snap) const
    {
        ...
        ecs_min[2] = _lc->min_dist(snap);
        ecs_max[2] = _lc->max_dist(snap);
