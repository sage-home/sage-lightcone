...
void filter::set_field(const std::string &name, const std::string &min,
...
filter::iterator filter::begin(const tao::batch<real_type> &bat,
                               bool filter_field_enabled) const {
  return iterator(this, &bat, filter_field_enabled, false);
}

filter::iterator filter::end(const tao::batch<real_type> &bat,
                             bool filter_field_enabled) const {
...
bool filter_iterator::equal(const filter_iterator &op) const {
  return _idx == op._idx;
}
