...
void filter::set_field(const std::string& name, const std::string& min, const std::string& max)
{
...
void filter::set_type(batch<real_type>::field_value_type type)
{
    _type = type;
    switch (_type)
    {
...
filter::iterator filter::begin(const tao::batch<real_type>& bat, bool filter_field_enabled) const
{
    return iterator(this, &bat, filter_field_enabled, false);
}

filter::iterator filter::end(const tao::batch<real_type>& bat, bool filter_field_enabled) const
{
    return iterator(this, &bat, filter_field_enabled, true);
}

const std::string& filter::field_name() const { return _field_name; }
...
filter_iterator::filter_iterator(const filter* filt, const batch<real_type>* bat,
...
void filter_iterator::increment()
{
    ++_idx;
...
bool filter_iterator::equal(const filter_iterator& op) const { return _idx == op._idx; }

unsigned filter_iterator::dereference() const { return _idx; }

void filter_iterator::_settle(bool filter_field_enabled)
{
