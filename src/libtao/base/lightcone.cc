...
void lightcone::set_geometry(real_type ra_min, real_type ra_max, real_type dec_min,
                             real_type dec_max, real_type z_max, real_type z_min)
{
    ...
    ASSERT(z_min >= 0.0, "Minimum redshift must be greater than or equal to zero.");
...
void lightcone::set_origin(std::array<real_type, 3> const& orig) { _orig = orig; }
...
void lightcone::set_min_ra(real_type ra_min)
{
    ASSERT(ra_min >= 0.0 && ra_min <= 360.0,
           "Minimum RA cannot be less than zero or greater than 360 degrees.");
    ASSERT(to_radians(ra_min) < _ra[1], "Minimum RA must be less than maximum RA.");
    _ra[0] = to_radians(ra_min);
    _recalc();
}
...
void lightcone::set_max_ra(real_type ra_max)
{
    ASSERT(ra_max >= 0.0 && ra_max <= 360.0,
           "Maximum RA cannot be less than zero or greater than 360 degrees.");
    ASSERT(to_radians(ra_max) > _ra[0], "Minimum RA must be less than maximum RA.");
    _ra[1] = to_radians(ra_max);
    _recalc();
}
...
void lightcone::set_min_dec(real_type dec_min)
{
    ASSERT(dec_min >= -90.0 && dec_min <= 90.0,
           "Minimum DEC cannot be less than -90 or greater than 90 degrees.");
    ASSERT(to_radians(dec_min) < _dec[1], "Minimum DEC must be less than maximum DEC.");
    _dec[0] = to_radians(dec_min);
    _recalc();
}
...
void lightcone::set_max_dec(real_type dec_max)
{
    ASSERT(dec_max >= -90.0 && dec_max <= 90.0,
           "Maximum DEC cannot be less than -90 or greater than 90 degrees.");
    ASSERT(to_radians(dec_max) > _dec[0], "Minimum DEC must be less than maximum DEC.");
    _dec[1] = to_radians(dec_max);
    _recalc();
}
...
lightcone::tile_iterator lightcone::tile_begin() const { return tile_iterator(*this); }
...
{
    ASSERT(snap >= _snap_bins.front() && snap <= _snap_bins.back(), "Invalid snapshot.");
...
std::vector<unsigned> const& lightcone::snapshot_bins() const { return _snap_bins; }

std::vector<real_type> const& lightcone::distance_bins() const { return _dist_bins; }

std::vector<real_type> const& lightcone::redshift_bins() const { return _z_bins; }

real_type lightcone::distance_to_redshift(real_type dist) const { return _dist_to_z[dist]; }
...
void lightcone::_recalc()
{
    if (_sim)
    {
        ...
        _dist[0] = hpc::num::redshift_to_comoving_distance(_z[0], 1000, _sim->hubble(),
                                                           _sim->omega_l(), _sim->omega_m()) *
                   _sim->h();
        _dist[1] = hpc::num::redshift_to_comoving_distance(_z[1], 1000, _sim->hubble(),
                                                           _sim->omega_l(), _sim->omega_m()) *
                   _sim->h();
        ...
        _z_bins.front() = std::max(_sim->redshift(_sim->num_snapshots() - 1), _z[1]);
        _dist_bins.front() = hpc::num::redshift_to_comoving_distance(
            _z_bins.front(), 1000, _sim->hubble(), _sim->omega_l(), _sim->omega_m());
        ...
        // Build the distance to redshift interpolator.
        {
            ...
            for (unsigned ii = 0; ii < num_points; ++ii)
            {
                ...
                dists[ii] = hpc::num::redshift_to_comoving_distance(zs[ii], 1000, _sim->hubble(),
                                                                    _sim->omega_l(),
                                                                    _sim->omega_m()) *
