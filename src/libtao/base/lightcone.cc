...
namespace tao {
   ...
   void
   lightcone::copy(const lightcone& from)
   {
      _sim = from._sim;
      _ra = from._ra;
      _dec = from._dec;
   ...
   void
   lightcone::set_random( bool rand, int rng_seed,
                          engine_type* engine )
   {
      _rand = rand;
      _rng_seed = rng_seed;
   ...
   void
   lightcone::set_ra_offset( real_type angle )
   {
      _ra_offs = angle;
   }

   void
   lightcone::set_dec_offset( real_type angle )
   {
      _dec_offs = angle;
   }

   real_type
   lightcone::ra_offset() const
   {
      return _ra_offs;
   }

   real_type
   lightcone::dec_offset() const
   {
      return _dec_offs;
   }
   ...
   real_type
   lightcone::min_dist( unsigned snap ) const
   {
   ...
   real_type
   lightcone::max_dist( unsigned snap ) const
   {
      ASSERT( snap >= _snap_bins.front() && snap <= _snap_bins.back(),
              "Invalid snapshot." );
   ...
   lightcone::_recalc()
   {
      if( _sim )
      {
         ...
         //std::cout <<"Distance range: ["<<_dist[0]<<", "<<_dist[1]<< ")"<<std::endl;
         ...
         _dist_bins.back() = hpc::num::redshift_to_comoving_distance( _z_bins.back(), 1000, _sim->hubble(), _sim->omega_l(), _sim->omega_m() );
         _z_bins.front() = std::max( _sim->redshift( _sim->num_snapshots() - 1 ), _z[1] );
         _dist_bins.front() = hpc::num::redshift_to_comoving_distance( _z_bins.front(), 1000, _sim->hubble(), _sim->omega_l(), _sim->omega_m() );
         for( unsigned ii = 1; ii < _dist_bins.size() - 1; ++ii )
         {
            _z_bins[ii] = _sim->redshift( first + ii - 1 );
            _dist_bins[ii] = hpc::num::redshift_to_comoving_distance( _z_bins[ii], 1000, _sim->hubble(), _sim->omega_l(), _sim->omega_m() );
         }
