...
template <class T> struct cosmology {
  T hubble;
  T omega_m;
  T omega_l;
  T omega_k;
...
template <class T> auto expansion_to_redshift(T ef) -> T {
  return 1.0 / ef - 1.0;
}

template <class T> T redshift_to_age_func(double x, void *param) {
  cosmology<T> *cosmo = (cosmology<T> *)param;
  return 1.0 / sqrt(cosmo->omega_k + cosmo->omega_m / x +
                    cosmo->omega_r / (x * x) + cosmo->omega_l * x * x);
}

template <class T>
T redshift_to_age(T redshift, T hubble = 73.0, T omega_m = 0.25,
                  T omega_l = 0.75) {
  ...
  gsl_integration_qag(&func, 0.0, upp, 1e-5, 1e-8, 1000, GSL_INTEG_GAUSS21,
                      work, &res, &abserr);
...
real_type approx_observed_redshift(lightcone const &lc,
...
int GetIntersection(T fDst1, T fDst2, BoxSeqT const &pb1, BoxSeqT const &pb2,
                    BoxSeqT &Hit) {
...
int InBox(BoxSeqT &Hit, BoxSeqT const &Box1, BoxSeqT const &Box2,
          const int Axis) {

  if (Axis == 1 && Hit[2] >= Box1[2] && Hit[2] <= Box2[2] &&
      Hit[1] >= Box1[1] && Hit[1] <= Box2[1])
    return 1;
  if (Axis == 2 && Hit[2] >= Box1[2] && Hit[2] <= Box2[2] &&
      Hit[0] >= Box1[0] && Hit[0] <= Box2[0])
    return 1;
  if (Axis == 3 && Hit[0] >= Box1[0] && Hit[0] <= Box2[0] &&
      Hit[1] >= Box1[1] && Hit[1] <= Box2[1])
    return 1;
  return 0;
}
...
int checklinebox(BoxSeqT const &B1, BoxSeqT const &B2, LineSeqT const &L1,
                 LineSeqT const &L2, LineSeqT &Hit) {
  ...
  if ((GetIntersection<BoxSeqT, real_type>(L1[0] - B1[0], L2[0] - B1[0], L1, L2,
                                           Hit) &&
       InBox(Hit, B1, B2, 1)) ||
      (GetIntersection<BoxSeqT, real_type>(L1[1] - B1[1], L2[1] - B1[1], L1, L2,
                                           Hit) &&
       InBox(Hit, B1, B2, 2)) ||
      (GetIntersection<BoxSeqT, real_type>(L1[2] - B1[2], L2[2] - B1[2], L1, L2,
                                           Hit) &&
       InBox(Hit, B1, B2, 3)) ||
      (GetIntersection<BoxSeqT, real_type>(L1[0] - B2[0], L2[0] - B2[0], L1, L2,
                                           Hit) &&
       InBox(Hit, B1, B2, 1)) ||
      (GetIntersection<BoxSeqT, real_type>(L1[1] - B2[1], L2[1] - B2[1], L1, L2,
                                           Hit) &&
       InBox(Hit, B1, B2, 2)) ||
      (GetIntersection<BoxSeqT, real_type>(L1[2] - B2[2], L2[2] - B2[2], L1, L2,
                                           Hit) &&
       InBox(Hit, B1, B2, 3)))
    return true;
