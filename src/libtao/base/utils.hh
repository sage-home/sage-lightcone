...
template <class T>
T redshift_to_age_func(double x, void* param)
{
    cosmology<T>* cosmo = (cosmology<T>*)param;
    return 1.0 / sqrt(cosmo->omega_k + cosmo->omega_m / x + cosmo->omega_r / (x * x) +
                      cosmo->omega_l * x * x);
}

template <class T>
T redshift_to_age(T redshift, T hubble = 73.0, T omega_m = 0.25, T omega_l = 0.75)
{
