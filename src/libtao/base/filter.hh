...
public:
    ...
    ///
    bool operator()(batch<real_type> const& bat, bool filter_field_enabled, unsigned idx) const
    {
        if (bat.masked(idx))
            return false;
    ...
    ///
    void set_field(const std::string& name, const std::string& min, const std::string& max);
    ...
    template <class T>
    boost::optional<T> minimum() const
    {
        if (_min)
        {
            T val;
    ...
    template <class T>
    boost::optional<T> maximum() const
    {
        if (_max)
        {
            T val;
    ...
    ///
    iterator begin(const tao::batch<real_type>& bat, bool filter_field_enabled) const;
    ...
    ///
    iterator end(const tao::batch<real_type>& bat, bool filter_field_enabled) const;

    void settletomask(tao::batch<real_type>& bat, bool filter_field_enabled) const
    {
        unsigned int ii = 0;
        while (ii < bat.size())
        {
    ...
    inline bool ends_with(std::string const& value, std::string const& ending) const
    {
...
class filter_iterator
    : public boost::iterator_facade<filter_iterator, unsigned, std::forward_iterator_tag, unsigned>
{
...
public:
    ...
    filter_iterator(const filter* filt, const batch<real_type>* bat, bool filter_field_enabled,
                    bool done);
