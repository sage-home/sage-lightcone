...
namespace tao {
   ...
   public:
      ...
      ///
      bool
      operator()( batch<real_type> const& bat, bool filter_field_enabled,
		  unsigned idx ) const
      {
      	if (bat.masked(idx))
      		return false;
	 ...
	 switch( _type )
	 {
      ...
      /// and maximum values are also given as string representations of their values.
      ///
         ...
         if( _max )
	 {
      ...
      ///
      iterator
      begin( const tao::batch<real_type>& bat, bool filter_field_enabled ) const;
      ...
      ///
      iterator
      end( const tao::batch<real_type>& bat, bool filter_field_enabled ) const;

	   void
	   settletomask(tao::batch<real_type>& bat, bool filter_field_enabled) const
	   {
		   unsigned int ii=0;
		   while( ii < bat.size() )
		   {
   ...
   protected:
      ...
      void
      _settle(bool field_filter_enabled);
