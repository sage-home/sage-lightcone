...
public:
  ...
  /// @param[in] sim  Simulation for the lightcone.
  ///
  void set_simulation(tao::simulation const *sim);
  ...
  ///
  void set_geometry(real_type ra_min, real_type ra_max, real_type dec_min,
                    real_type dec_max, real_type z_max, real_type z_min = 0);

  void set_min_ra(real_type ra_min);

  void set_max_ra(real_type ra_max);

  void set_min_dec(real_type dec_min);

  void set_max_dec(real_type dec_max);

  void set_min_redshift(real_type z_min);

  void set_max_redshift(real_type z_max);

  real_type min_ra() const;

  real_type max_ra() const;

  real_type min_dec() const;

  real_type max_dec() const;

  real_type min_redshift() const;

  real_type max_redshift() const;

  real_type min_dist() const;

  real_type max_dist() const;

  real_type min_dist(unsigned snap) const;

  real_type max_dist(unsigned snap) const;
  ...
  ///
  void set_random(bool rand, int rng_seed = -1,
  ...
  ///
  void set_ra_offset(real_type angle);

  void set_dec_offset(real_type angle);

  /// Get viewing angle.
  /// @returns Viewing inclination angle.
  ///
  real_type ra_offset() const;

  real_type dec_offset() const;

  /// Set lightcone origin.
  /// @param orig  Lightcone origin.
  ///
  void set_origin(std::array<real_type, 3> const &orig);

  /// Get lightcone origin.
  /// @returns Lightcone origin.
  ///
  std::array<real_type, 3> const &origin() const;

  void set_single_snapshot(bool state);

  bool single_snapshot() const;

  void set_snapshot(unsigned snap);
  ...
  galaxy_begin(query<real_type> &qry, Backend &be,
               tao::batch<real_type> *bat = 0, filter const *filt = 0) {
    typename Backend::lightcone_galaxy_iterator result =
        be.galaxy_begin(qry, *this, bat, filt);
    return result;
  }

  template <class Backend>
  typename Backend::lightcone_galaxy_iterator galaxy_end(query<real_type> &qry,
                                                         Backend &be) {
    return be.galaxy_end(qry, *this);
  }

  std::vector<unsigned> const &snapshot_bins() const;

  std::vector<real_type> const &distance_bins() const;

  std::vector<real_type> const &redshift_bins() const;

  real_type distance_to_redshift(real_type dist) const;

  hpc::engine_type *rng_engine() const;

  void rng_reset() const {
    if (_rng_seed != -1) {
      rng_engine()->seed(_rng_seed);
    }
  }

  bool overlap(std::array<real_type, 3> const &min,
               std::array<real_type, 3> const &max) const;

  lightcone dereference() const { return *this; }
