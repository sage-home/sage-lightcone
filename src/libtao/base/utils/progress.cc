...
namespace tao {
    ...
    progress::_receiver() {
        ...
        while (n_finished < _comm.size()) {
            ...
            if (_comm.iprobe(stat, MPI_ANY_SOURCE, progress_tag)) {
                value_type val;
                _comm.recv((double *) &val, hpc::mpi::datatype::double_floating, stat.MPI_SOURCE, 2, stat.MPI_TAG);
                _lock.lock();
                if (val.first < 0.0) {
    ...
    progress::_reporter() {
        while (!_done) {
            ...
            // Take the average of the historical values. It's easy, and seems to provide
    ...
    void
    progress::_update_history() {
        // The way this routine operates is based on maintaining a list of received values
        // for each rank. When new values come in, we stash them in the source rank's list
        // and then call this routine. Each entry in the cache corresponds to a 2D point, the
        // coordinates of which are number of objects processed, and when the measurement was
        // taken.
        //
        // Once in here, we try to find as many points in time where we have an entry in the
        // cache from each rank available. When we find one such point we can calculate the
        // global instantaneous average rate of object processing, and push that to our rate list.
        // If we should ever find that any one of our lists is exhausted, we can terminate this
        // routine and wait for more entries to arrive.
        //
        // `while( 1 )` is used as we don't know how many complete entries in the cache we'll
        // find, so it's easier to drop out once we exhaust one rank's list.
        while (1) {
            value_type val;
            ...
            for (unsigned ii = 0; ii < _cache.size(); ++ii) {
                ...
                // This is where we terminate, as we've hit the end of one of our cache lists.
                if (!_cache[ii].size())
                    return;
