...
public:
    ...
    std::array<std::array<real_type, 2>, 3> const& bounds() const { return _bnds; }
    ...
    typename Backend::box_galaxy_iterator galaxy_begin(tao::query<real_type>& query, Backend& be,
    ...
    {
        return crd[0] >= _min[0] && crd[0] < _max[0] && crd[1] >= _min[1] && crd[1] < _max[1] &&
               crd[2] >= _min[2] && crd[2] < _max[2];
    }

    bool contains(double const& x, double const& y, double const& z) const
    {
        return x >= _min[0] && x < _max[0] && y >= _min[1] && y < _max[1] && z >= _min[2] &&
               z < _max[2];
    }
