...
namespace tao {
   ...
   public:
	 ...
	 _update_random();
      ...
      std::array<std::array<real_type,2>,3> const&
      bounds() const
      {
         return _bnds;
      }
      ...
      contains( std::array<real_type,3> const& crd,
                unsigned snap = 0 ) const
      {
         return crd[0] >= _min[0] && crd[0] < _max[0] &&
                crd[1] >= _min[1] && crd[1] < _max[1] &&
                crd[2] >= _min[2] && crd[2] < _max[2];
      }

       bool
       contains( double const& x,double const& y,double const& z) const
       {
      ...
      template< class Iter >
      bool
      box_collision( Iter const& bnds_begin,
                     unsigned snap = 0 ) const
      {
