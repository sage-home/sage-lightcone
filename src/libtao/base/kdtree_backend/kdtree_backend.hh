...
namespace tao {
   namespace backends {
      ...
      public:
         ...
         void
         connect( const cli_dict& global_cli_dict);

         void
         close();

         void
         open( hpc::fs::path const& fn );
         ...
         std::string
         make_snapshot_name( unsigned snap ) const;
         ...
         lightcone_galaxy_iterator
         galaxy_end( query<real_type> const& qry,
         ...
         void
         load_lightcone_data( unsigned cell,
      ...
      public:
         ...
         static getFields( hid_t g_id, const char *name, const H5L_info_t *info, void *op_data) {
          ...
          bool
         operator==( kdtree_galaxy_iterator const& op ) const
         {
            return _it == op._it;
         }
         ...
         batch<real_type>&
         deref( lightcone const* lc = nullptr )
         {
            ASSERT( _it != _kdt->end() );
            if( !_ready )
            {
      ...
      protected:
         ...
         void
         _copy_lightcone_data( lightcone const* lc = nullptr )
         {
            ...
            if( lc )
            {
               ...
               redshift = _bat->set_scalar<real_type>( "cosmological_redshift" );
            }
	    ...
	    //else
            ...
            do
            {
               ...
               if( first != _ii ) {
                  ++_it;
                 //LOGILN( "b.block: ", _it.cell(), " ",_ii," ", ((_ii+1)%_n_ranks)," more=",(_it != _be->kdtree().end()));
               }
               ...
               while( _it != _be->kdtree().end() )
               {
                  ...
                  if (_it.is_leaf())
                  {
                      ...
                      //std::cout<<"@"<<_snap<<" "<<res<<"="<<_it.bounds()[0][0]+_dom.min()[0]<<","<<_it.bounds()[0][1]+_dom.min()[0]<<
                      //         "," <<_it.bounds()[1][0]+_dom.min()[1]<<","<<_it.bounds()[1][1]+_dom.min()[1]<<
                      //         ","<<_it.bounds()[2][0]+_dom.min()[2]<<","<<_it.bounds()[2][1]+_dom.min()[2]<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmin<<","<<ymin<<","<<zmin<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmax<<","<<ymin<<","<<zmin<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmax<<","<<ymax<<","<<zmin<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmin<<","<<ymax<<","<<zmin<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmin<<","<<ymax<<","<<zmax<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmin<<","<<ymin<<","<<zmax<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmax<<","<<ymin<<","<<zmax<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmax<<","<<ymax<<","<<zmax<<std::endl;
                      std::cout<<"@ "<<res<<","<<","<<_snap<<","<<xmin<<","<<ymax<<","<<zmax<<std::endl;
                      _count++;
                  }
                   */
                  ...
                  if( _it.is_leaf() ) {
                      ...
                      //LOGILN( "break: ", _it.cell() ," ",_todo," more=",(_it != _be->kdtree().end()));
                      break;
                  }
                  ++_it;
                  //LOGILN( "c.block: ", _it.cell(), " ",_ii," ", ((_ii+1)%_n_ranks != _rank)," more=",(_it != _be->kdtree().end()));
               }
               //LOGILN("more",((_ii+1)%_n_ranks != _rank)," more=",(_it != _be->kdtree().end()));
            }
	    ...
	    //if (_todo>0)
          ...
          _fetch()
          {
             ...
             for (unsigned int i=0;i<count;i++)
             {
                ...
                if (!_dom.contains(crd, _snap)) {
                   _bat->mask(i);
                   //std::cout << "mask["<<i<<"]=" << posx[i] << "," << posy[i] << "," << posz[i] << std::endl;
                }
             }
          ...
          void
          _calc_fields() {
              if( _lc ) { // TODO Better way to decide the context (this is, avoid when using box rather than light cone)
                 ...
                 for (unsigned ii = 0; ii < _bat->size(); ++ii) {
                    _bat->unmask(ii);
		    //LOGILN( "galaxyindex: ", galaxyindex[ii],"=off=",_it.cell()," ",off+ii );
                    ...
                    if (_lc) {
                       ...
                       //std::cout<<"x,y,z="<<pos_x[ii]<<","<<pos_y[ii]<<","<<pos_z[ii]<<"="<<gid[ii]<<" "<<disk_sfr[ii]<<" "<<vel_x[ii]<<" "<<vel_y[ii]<<" "<<vel_z[ii]<<std::endl;
			  ...
			  //}
                       ...
                       if (recalc)
                          hpc::num::ecs_to_cartesian<real_type>(ra[ii], dec[ii], pos_x[ii], pos_y[ii], pos_z[ii],
                                                                dist[ii]);

                       // Check angles.
                       //ASSERT(ra[ii] >= _dom._lc->min_ra() - EPSILON && ra[ii] <= _dom._lc->max_ra() + EPSILON,
                       //       "Calculated RA exceeds limits: ", to_degrees(ra[ii]));
                       //ASSERT(dec[ii] >= _dom._lc->min_dec() - EPSILON && dec[ii] <= _dom._lc->max_dec() + EPSILON,
