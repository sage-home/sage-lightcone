...
public:
    ...
    kdtree_lightcone_galaxy_iterator(lightcone const& lc, backend_type& be, query<real_type>& qry,
                                     batch<real_type>* bat = 0, filter const* flt = 0)
        : _lc(&lc)
        , _be(&be)
        , _qry(&qry)
        , _bat(bat)
        ...
        , _n_processed_gals(0)
    {
        ...
        // do
        {
            ...
            //_snap_lc = mylc;
            // Keep it broader than it needs to be to allow comparison with RDB (Also
            // will need a separate instance of _snap_lc for each snapshot eventually)
            //_snap_lc->set_geometry( hpc::to_degrees( lc.min_ra() ), hpc::to_degrees(
            // lc.max_ra() ),
            //                       hpc::to_degrees( lc.min_dec() ), hpc::to_degrees(
            //                       lc.max_dec() ), lc.redshift_bins()[_snap_ii],
            //                       lc.redshift_bins()[_snap_ii + 1] );
            be.load_snapshot(lc.snapshot_bins()[_snap_ii]);
            // TODO: leaking code
            _gal_it = new lightcone_galaxy_iterator<backend_type>(
                *mylc, be, qry, _tile_it_common_to_all_snapshots, bat, flt);
    ...
    reference_type operator*() { return *_gal_it; }
        ...
        if (++(*_gal_it) == *_gal_end)
        {
            if (++_snap_ii < _lc->snapshot_bins().size())
            {
                ...
                mylc->set_geometry(hpc::to_degrees(_lc->min_ra()), hpc::to_degrees(_lc->max_ra()),
                                   ...
                                   _lc->redshift_bins()[_snap_ii],
                                   _lc->redshift_bins()[_snap_ii + 1]);
                // Keep it broader than it needs to be to allow comparison with RDB
                // (Also will need a separate instance of _snap_lc for each snapshot
                // eventually)
                //_snap_lc->set_geometry( hpc::to_degrees( _lc->min_ra() ),
                // hpc::to_degrees( _lc->max_ra() ),
                //                       hpc::to_degrees( _lc->min_dec() ),
                //                       hpc::to_degrees( _lc->max_dec() ),
                //                      _lc->redshift_bins()[_snap_ii],
                //                      _lc->redshift_bins()[_snap_ii + 1] );
                _be->load_snapshot(_lc->snapshot_bins()[_snap_ii]);
                // TODO: leaking code
                _gal_it = new lightcone_galaxy_iterator<backend_type>(
                    *mylc, *_be, *_qry, _tile_it_common_to_all_snapshots, _bat, _flt);
                _gal_end = new lightcone_galaxy_iterator<backend_type>();
            }
        }
    }

    bool operator==(kdtree_lightcone_galaxy_iterator const& op) const
    {
    ...
    unsigned long long n_processed_gals() const { return _gal_it->n_processed_gals(); }

    unsigned tile_index() const { return _gal_it->tile_index(); }

    bool should_checkpoint() { return _gal_it->should_checkpoint(); }

    void save_checkpoint(boost::property_tree::ptree& pt) { _gal_it->save_checkpoint(pt); }
