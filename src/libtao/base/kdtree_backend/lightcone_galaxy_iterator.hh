...
template <class BackendT> class kdtree_lightcone_galaxy_iterator {
public:
...
public:
  ...
  kdtree_lightcone_galaxy_iterator(lightcone const &lc, backend_type &be,
                                   query<real_type> &qry,
                                   batch<real_type> *bat = 0,
                                   filter const *flt = 0)
      : _lc(&lc), _be(&be), _qry(&qry), _bat(bat), _flt(flt),
        _n_processed_gals(0) {
    _snap_ii = 0;
    ...
    // do
    {
      ...
      mylc->copy(*_snap_lc);
      mylc->set_geometry(
          hpc::to_degrees(lc.min_ra()), hpc::to_degrees(lc.max_ra()),
  ...
  void operator++() {
    if (++(*_gal_it) == *_gal_end) {
      if (++_snap_ii < _lc->snapshot_bins().size()) {
        ...
        mylc->copy(*_snap_lc);
        mylc->set_geometry(
            hpc::to_degrees(_lc->min_ra()), hpc::to_degrees(_lc->max_ra()),
