...
static std::string read_dataset_attribute(hid_t loc_id,
...
kdtree_backend::kdtree_backend(hpc::mpi::comm const &comm)
    : _kdt(hpc::mpi::comm::self), _snap(std::numeric_limits<unsigned>::max()),
      _comm(&comm) {}
...
void kdtree_backend::connect(const cli_dict &global_cli_dict) {
  ...
  this->open(fn);
  std::cerr << "DEBUG: After open(), _field_types has " << _field_types.size()
            << " entries" << std::endl;
  ...
  for (const std::string &field : Hfields) {
    ...
    std::string desc =
        read_dataset_attribute(file_id, field_path, "Description");
    if (!desc.empty()) {
  ...
  std::cerr << "DEBUG: At end of connect(), _field_types has "
            << _field_types.size() << " entries" << std::endl;
  std::cerr << "DEBUG: _field_map has " << _field_map.size() << " entries"
            << std::endl;
}
...
void kdtree_backend::open(hpc::fs::path const &fn) {
  ...
  for (hsize_t i = 0; i < group_info.nlinks; i++) {
    char name_buf[256];
    H5Lget_name_by_idx(data_group.id(), ".", H5_INDEX_NAME, H5_ITER_NATIVE, i,
...
tao::simulation const *kdtree_backend::load_simulation() {
  ...
  std::vector<real_type> redshifts =
      _file.read<std::vector<real_type>>("snapshot_redshifts");
  ...
  auto *sim =
      new tao::simulation(_file.read<real_type>("cosmology/box_size"),
                          _file.read<real_type>("cosmology/hubble"),
                          _file.read<real_type>("cosmology/omega_m"),
                          _file.read<real_type>("cosmology/omega_l"), zs);
  set_simulation(sim);
  return sim;
}
...
kdtree_backend::galaxy_begin(query<real_type> &qry, box<real_type> const &box,
                             batch<real_type> *bat, filter const *flt) {
...
kdtree_backend::galaxy_begin(query<real_type> &qry, lightcone const &lc,
                             batch<real_type> *bat, filter const *flt) {
...
kdtree_backend::galaxy_begin(query<real_type> &qry, tile<real_type> const &tile,
                             batch<real_type> *bat, filter const *flt,
                             bool first) {
...
void kdtree_backend::load_lightcone_data(
    unsigned cell, std::vector<lightcone_data> &data) const {
  unsigned n_elems = _cell_cnts[cell];
  unsigned long long offs = _cell_offs[cell];
  data.resize(n_elems);
  _file.dataset("lightcone/data")
      .read(data.data(), _lc_mem_type, n_elems, offs);
}
...
std::vector<unsigned> const &kdtree_backend::cell_counts() const {
  return _cell_cnts;
}

std::vector<unsigned long long> const &kdtree_backend::cell_offs() const {
  return _cell_offs;
}
...
unsigned kdtree_backend::get_kdtree_branches() const {
