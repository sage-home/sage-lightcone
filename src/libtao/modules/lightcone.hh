...
///
template <class Backend> class lightcone : public module<Backend> {
public:
  typedef Backend backend_type;
  typedef module<backend_type> module_type;

  // Type of geometry to use.
  enum geometry_type { CONE, BOX, PENCIL };

  // Factory function used to create a new module.
  static module_type *factory(std::string const &name, pugi::xml_node base) {
...
public:
  lightcone(std::string const &name = std::string(),
      ...
      : module_type(name, base), _my_be(false), _be(0) {}
...
protected:
  void _read_options(const cli_dict &global_cli_dict) {
    ...
    if (_unique) {
      if (!_calc_origin()) {
        LOGILN("No unique cones can be computed for ",
               global_cli_dict._dataset);
        ...
        std::cerr << "  Dec: [" << min_dec << ", " << max_dec << ")"
                  << std::endl;
        std::cerr << "  RA: [" << min_ra << ", " << max_ra << ")"
                  << std::endl;
        std::cerr << "  Redshift: [" << min_z << ", " << max_z << ")"
                  << std::endl;
        std::cerr << "  Please check the ranges and/or the input hdf5 file."
                  << std::endl;
        std::cerr << "  Try smaller ranges for Dec, RA, and/or redshift."
                  << std::endl;
        exit(1);
      }
    }
    ...
    static const std::set<std::string> calculated_fields = {
        "ra", "dec", "distance", "redshift_cosmological", "redshift_observed",
        "sfr"};
    ...
    if (global_cli_dict._output_fields.size() == 0) {
      ...
      auto process_field = [&](const std::string &field) {
        ...
        auto it = std::find(all_field_names.begin(), all_field_names.end(),
                            field_lower);
  ...
  bool _calc_origin() {
    ...
    std::array<real_type, 3> ori =
        tao::calc_subcone_origin<real_type>(_lc, sub_idx);
