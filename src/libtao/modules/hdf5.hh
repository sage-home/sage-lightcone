...
static void write_dataset_string_attribute(hid_t dset_id,
...
static const std::map<std::string, FieldMetadata> calculated_fields_metadata = {
    {"redshift_cosmological",
     {"Cosmological redshift calculated from distance", "dimensionless"}},
    {"redshift_observed",
     {"Observed redshift including peculiar velocity", "dimensionless"}},
...
template <class Backend> class hdf5 : public module<Backend> {
public:
  typedef Backend backend_type;
  typedef module<backend_type> module_type;

  static module_type *factory(const std::string &name, pugi::xml_node base) {
      ...
      if (!result_files.empty()) {
        ...
        std::vector<std::string> field_names;
        {
          ...
          for (hsize_t i = 0; i < group_info.nlinks; i++) {
            char name_buf[256];
            H5Lget_name_by_idx(first_h5file.id(), ".", H5_INDEX_NAME,
                               H5_ITER_NATIVE, i, name_buf, sizeof(name_buf),
                               H5P_DEFAULT);
        ...
        for (const auto &field_name : field_names) {
          ...
          for (const auto &file : result_files) {
            ...
            if (first_file) {
              ...
              // Create dataset
              hpc::h5::dataset dst_dset(_file0, dataset_name, dtype, dspace,
                                        props);
              dst_dset.set_extent(size);

              // Copy HDF5 attributes from source to destination
              hid_t src_id =
                  H5Dopen2(h5file.id(), dataset_name.c_str(), H5P_DEFAULT);
              hid_t dst_id =
                  H5Dopen2(_file0.id(), dataset_name.c_str(), H5P_DEFAULT);
...
public:
  hdf5(const std::string &name = std::string(),
      ...
      : module_type(name, base), _chunk_size(10000), _ready(false),
        _records(0) {}
  ...
  initialise(const cli_dict &global_cli_dict,
    ...
    std::vector<std::string> available_fields;
    {
      ...
      for (hsize_t i = 0; i < group_info.nlinks; i++) {
        char name_buf[256];
        H5Lget_name_by_idx(data_group.id(), ".", H5_INDEX_NAME, H5_ITER_NATIVE,
                           i, name_buf, sizeof(name_buf), H5P_DEFAULT);
    ...
    LOGILN("Initialising HDF5 module. nfields from input HDF5 file: ", nfields,
           setindent(2));
  ...
  static std::string _construct_filename_static(const std::string &base_name,
                                                int rank) {
  ...
  std::string _encode(std::string _toencode_string) {
    ...
    while (std::string::npos !=
           (pos = _toencode_string.find_first_of(reserved_chars, pos))) {
  ...
  virtual void execute() {
    ...
    if (!_ready) {
      ...
      for (const auto &field : _fields) {
        hpc::to_lower(
            (std::string &)field); // input field name of hdf5 must be lowercase
        ...
        h5::dataset *dset =
            new hpc::h5::dataset(_file, field_name, dtype, dspace, props);
        ...
        if (!_input_kdtree_file.empty()) {
          ...
          if (input_file_id >= 0) {
            ...
            hid_t input_dset_id = H5Dopen2(
                input_file_id, input_dataset_path.c_str(), H5P_DEFAULT);
            ...
            if (input_dset_id >= 0 && output_dset_id >= 0) {
              ...
              if (H5Aexists(input_dset_id, "Description") > 0) {
                hid_t attr_id =
                    H5Aopen(input_dset_id, "Description", H5P_DEFAULT);
                hid_t type_id = H5Aget_type(attr_id);
                size_t attr_size = H5Tget_size(type_id);
                std::vector<char> attr_buf(attr_size + 1, 0);
                H5Aread(attr_id, type_id, attr_buf.data());
        ...
        case tao::batch<real_type>::DOUBLE: {
          const hpc::view<std::vector<double>> thedata =
              bat.scalar<double>(field);
          for (auto it = _filt->begin(bat, true); it != _filt->end(bat, true);
               ++it, ++ii) {
            dspace = dset->dataspace();
            dspace.select_one(ii);
            _write_field_DOUBLE(bat, *it, field, *dset, dspace, mem_space,
                                thedata);
          }
        } break;
        case tao::batch<real_type>::FLOAT: {
          const hpc::view<std::vector<float>> thedata =
              bat.scalar<float>(field);
          for (auto it = _filt->begin(bat, true); it != _filt->end(bat, true);
               ++it, ++ii) {
            dspace = dset->dataspace();
            dspace.select_one(ii);
            _write_field_FLOAT(bat, *it, field, *dset, dspace, mem_space,
                               thedata);
          }
        } break;
        case tao::batch<real_type>::INTEGER: {
          ...
          for (auto it = _filt->begin(bat, true); it != _filt->end(bat, true);
            ...
            dspace = dset->dataspace();
            dspace.select_one(ii);
            _write_field_INTEGER(bat, *it, field, *dset, dspace, mem_space,
                                 thedata);
          }
        } break;
        case tao::batch<real_type>::UNSIGNED_LONG_LONG: {
          ...
          for (auto it = _filt->begin(bat, true); it != _filt->end(bat, true);
            ...
            dspace = dset->dataspace();
            dspace.select_one(ii);
            _write_field_UNSIGNED_LONG_LONG(bat, *it, field, *dset, dspace,
                                            mem_space, thedata);
          }
        } break;
        case tao::batch<real_type>::LONG_LONG: {
          ...
          for (auto it = _filt->begin(bat, true); it != _filt->end(bat, true);
            ...
            dspace = dset->dataspace();
            dspace.select_one(ii);
            _write_field_LONG_LONG(bat, *it, field, *dset, dspace, mem_space,
                                   thedata);
          }
        } break;
        default:
...
protected:
  ...
  static void write_chunked_dataset(hpc::h5::dataset &dst_dset,
    ...
    for (hsize_t chunk = 0; chunk < num_chunks; chunk++) {
      ...
      // Write chunk
      dst_dset.write(buffer + (chunk_start * dtype.size()), dtype, mem_space,
                     file_space);
    }
  }

  h5::datatype _field_type(const tao::batch<real_type> &bat,
  ...
  void _write_field(const tao::batch<real_type> &bat, unsigned idx,
  ...
  void _write_field_UNSIGNED_LONG_LONG(
      const tao::batch<real_type> &bat, unsigned idx, const std::string &field,
      h5::dataset &dset, h5::dataspace &dspace, h5::dataspace &mem_space,
      const hpc::view<std::vector<unsigned long long>> &thedata) {
    {
  ...
  void
  _write_field_LONG_LONG(const tao::batch<real_type> &bat, unsigned idx,
                         const std::string &field, h5::dataset &dset,
                         h5::dataspace &dspace, h5::dataspace &mem_space,
                         const hpc::view<std::vector<long long>> &thedata) {
    {
  ...
  typename tao::batch<tao::real_type>::field_value_type
  _datatypeAsTAO(hpc::h5::datatype datatype) {
...
protected:
  ...
  std::string _fn;
  std::string
      _input_kdtree_file; // Path to input kdtree file for attribute reading
