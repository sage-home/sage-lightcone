...
// 			   boost::optional<const vector<hsize_t>::view&>
// chunk_size, 			   bool deflate )
// {
// 	 BOOST_MPL_ASSERT( (mpl::has_key<h5::datatype::type_map,char>) );
// 	 h5::datatype datatype(
// mpl::at<h5::datatype::type_map,char>::type::value );
...
// 	    file_set.write( data[ii].c_str(), datatype, mem_space, file_space,
// *this->_comm );
// 	 }
// }
...
//    // Insert each string.
//    const char* first = full_str.data(), *last = full_str.data() +
//    full_str.size(); const char* mark = first; unsigned idx = 0; while( first
//    != last )
//    {
//       if( *first == 0 )
//       {
...
// template<>
// void file::writeItem(const std::string& name, const int& itm, const
// MPI::Comm& comm) {
//     this->writeItem(name, &itm, DataType::NATIVE_INT, comm);
// }

// void file::writeItem(const std::string& name, const void* itm, DataType&
// dataType, const MPI::Comm& comm) {
...
// template<>
// void file::write(const std::string& name, const VectorView<int>& data, const
// Comm& comm) {
//     Vector<int> dataSize(1);
...
// template<>
// void file::write(const std::string& name, const VectorView<int,2>& data,
// const Comm& comm, int splitDim) {
...
// template<>
// void file::write(const std::string& name, const VectorView<int>& data, const
// VectorView<hsize_t>& chunkSize, const Comm& comm, bool deflate) {
//     Vector<int> dataSize(1);
//     dataSize[0] = data.getSize();
//     this->write(name, dataSize, data, DataType::NATIVE_INT, chunkSize, comm,
//     deflate);
// }

// template<>
// void file::write(const std::string& name, const VectorView<int,2>& data,
// const VectorView<hsize_t>& chunkSize, const Comm& comm, int splitDim, bool
// deflate) {
//     Vector<int> dataSize(2);
//     dataSize[0] = data.getSize(0);
//     dataSize[1] = data.getSize(1);
...
// template<>
// void file::write(const std::string& name, const VectorView<float>& data,
// const Comm& comm) {
//     Vector<int> dataSize(1);
//     dataSize[0] = data.getSize();
...
// template<>
// void file::write(const std::string& name, const VectorView<float,2>& data,
// const Comm& comm, int splitDim) {
//     Vector<int> dataSize(2);
//     dataSize[0] = data.getSize(0);
//     dataSize[1] = data.getSize(1);
...
// template<>
// void file::write(const std::string& name, const VectorView<float>& data,
// const VectorView<hsize_t>& chunkSize, const Comm& comm, bool deflate) {
//     Vector<int> dataSize(1);
//     dataSize[0] = data.getSize();
//     this->write(name, dataSize, data, DataType::NATIVE_FLOAT, chunkSize,
//     comm, deflate);
// }
...
//     dataSize[1] = data.getSize(1);
//     this->write(name, dataSize, data, DataType::NATIVE_FLOAT, chunkSize,
//     comm, splitDim, deflate);
// }

// template<>
// void file::write(const std::string& name, const VectorView<double>& data,
// const Comm& comm) {
//     Vector<int> dataSize(1);
//     dataSize[0] = data.getSize();
...
// template<>
// void file::write(const std::string& name, const VectorView<double,2>& data,
// const Comm& comm, int splitDim) {
//     Vector<int> dataSize(2);
//     dataSize[0] = data.getSize(0);
//     dataSize[1] = data.getSize(1);
...
//     Vector<int> dataSize(1);
//     dataSize[0] = data.getSize();
//     this->write(name, dataSize, data, DataType::NATIVE_DOUBLE, chunkSize,
//     comm, deflate);
// }
...
// void file::write(const std::string& name, const VectorView<int>& dataSize,
// const void* data, DataType& dataType, const Comm& comm, int splitDim) {
//     this->write(name, dataSize, data, dataType, VectorView<hsize_t>(), comm,
//     splitDim, false);
// }

// void file::write(const std::string& name, const VectorView<int>& dataSize,
// const void* data, DataType& dataType, const VectorView<hsize_t>& chunkSize,
// const Comm& comm, int splitDim, bool deflate) {
//     assert_throw(dataSize.prod() >= 0,
// 	   "Data size cannot be negative.");
//     assert_throw(chunkSize.isEmpty() || chunkSize.getSize() ==
//     dataSize.getSize(),
...
//     globalDataSize[splitDim] = comm.allReduce<int>(globalDataSize[splitDim],
//     MPI_SUM); int rankOffs = comm.chainRecv<int>();
...
// template<>
// void file::write(const std::string& name, const CSR<int>& data, const Comm&
// comm) {
//     this->writeCSR(name, data.getDispls(), data, DataType::NATIVE_INT, comm);
// }

// template<>
// void file::write(const std::string& name, const CSR<int>& data, const
// VectorView<hsize_t>& chunkSize, const Comm& comm, bool deflate) {
//     this->writeCSR(name, data.getDispls(), data, DataType::NATIVE_INT,
//     chunkSize, comm, deflate);
// }

// template<>
// void file::write(const std::string& name, const CSR<float>& data, const Comm&
// comm) {
//     this->writeCSR(name, data.getDispls(), data, DataType::NATIVE_FLOAT,
//     comm);
// }

// template<>
// void file::write(const std::string& name, const CSR<float>& data, const
// VectorView<hsize_t>& chunkSize, const Comm& comm, bool deflate) {
//     this->writeCSR(name, data.getDispls(), data, DataType::NATIVE_FLOAT,
//     chunkSize, comm, deflate);
// }
...
// template<>
// void file::write(const std::string& name, const CSR<double>& data, const
// Comm& comm) {
//     this->writeCSR(name, data.getDispls(), data, DataType::NATIVE_DOUBLE,
//     comm);
// }
...
// void file::writeCSR(const std::string& name, const VectorView<int>& displs,
// const void* data, DataType& dataType, const Comm& comm) {
//     this->writeCSR(name, displs, data, dataType, VectorView<hsize_t>(), comm,
//     false);
// }

// void file::writeCSR(const std::string& name, const VectorView<int>& displs,
// const void* data, DataType& dataType, const VectorView<hsize_t>& chunkSize,
// const Comm& comm, bool deflate) {
...
//     this->write<int>(name + "_displs", VectorView<int>(displs,
//     displs.getSize() - 1), chunkSize, comm, deflate);
...
//     this->write(subName, writeSize, data, dataType, chunkSize, comm,
//     deflate);
// }

// template<>
// void file::writeInterlaced(const std::string& name, const CSR<int>& data,
// hsize_t size, const VectorView<int>& offsets, const MPI::Comm& comm) {
//     this->writeInterlaced(name, data.getDispls(), data, DataType::NATIVE_INT,
//     size, offsets, comm);
// }

// void file::writeInterlaced(const std::string& name, const VectorView<int>&
// dataDispls, const void* data, const DataType& dataType, hsize_t size, const
// VectorView<int>& offsets, const MPI::Comm& comm) {
//     assert_throw(size >= 0,
...
// #endif
//     assert_throw((dataDispls.getSize() == 0 && offsets.getSize() == 0) ||
//     (dataDispls.getSize() - 1 == offsets.getSize()),
// 	   "There must be the same number of rows in the data CSR and the file
// displacements."); #ifndef NDEBUG
//     // Check that no element of data is to be written beyond the end of our
//     dataset. for(int ii = 0; ii < dataDispls.getSize() - 1; ++ii) {
// 	assert_throw(offsets[ii] + dataDispls[ii + 1] - dataDispls[ii] <= size,
// 	       "Attempting to write data beyond the limits of destination
// dataset.");
//     }
// #endif

//     // Check if the requested dataset exists. If so, be sure it is the same
//     size. If not,
...
// 	       "Cannot write an interlaced array to dataspace of dimension other
// than 1."); #ifndef NDEBUG 	Vector<hsize_t> dims;
// 	fileSpace.getSimpleExtentDims(dims);
// 	assert_throw(dims.prod() == size,
// 	       "Attempting to write an interlaced set of data to an existing
// dataset, but the " 	       "dataset is not matching the requested size.");
// #endif
//     }
//     else {
...
//    return ((_comm->rank() + 1)*global_size)/_comm->size() -
//    (_comm->rank()*global_size)/_comm->size();
// }
...
// template<>
// void file::read(const std::string& name, VectorView<int> data, const Comm&
// comm) { #ifndef NDEBUG
...
// template<>
// void file::read(const std::string& name, VectorView<float> data, const Comm&
// comm) { #ifndef NDEBUG
//     Proxy proxy(this->readDataSize(name), comm);
//     assert_throw(data.getSize() == proxy.getNumLocals(), 10);
// #endif
...
// template<>
// void file::read(const std::string& name, VectorView<double> data, const Comm&
// comm) { #ifndef NDEBUG
//     Proxy proxy(this->readDataSize(name), comm);
//     assert_throw(data.getSize() == proxy.getNumLocals(), 10);
// #endif
...
// template<>
// void file::reada(const std::string& name, Vector<int>& data, const Comm&
// comm) {
//     Proxy proxy(this->readDataSize(name), comm);
...
// template<>
// void file::reada(const std::string& name, Vector<float>& data, const Comm&
// comm) {
//     Proxy proxy(this->readDataSize(name), comm);
//     data.setSize(proxy.getNumLocals());
//     this->read(name, data, comm);
...
// template<>
// void file::reada(const std::string& name, Vector<double>& data, const Comm&
// comm) {
//     Proxy proxy(this->readDataSize(name), comm);
//     data.setSize(proxy.getNumLocals());
//     this->read(name, data, comm);
...
// template<>
// void file::read(const std::string& name, VectorView<int> data, const
// VectorView<hsize_t>& elems, const MPI::Comm& comm) { #ifndef NDEBUG
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
//     assert_throw(elems.getSize()%dims.getSize() == 0,
// 		 "Element selection array does not divide into the number of
// dimensions.");
...
// template<>
// void file::read(const std::string& name, VectorView<float> data, const
// VectorView<hsize_t>& elems, const MPI::Comm& comm) { #ifndef NDEBUG
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
//     assert_throw(elems.getSize()%dims.getSize() == 0,
...
// template<>
// void file::read(const std::string& name, VectorView<double> data, const
// VectorView<hsize_t>& elems, const MPI::Comm& comm) { #ifndef NDEBUG
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
//     assert_throw(elems.getSize()%dims.getSize() == 0,
...
// template<>
// void file::reada(const std::string& name, Vector<int>& data, const
// VectorView<hsize_t>& elems, const MPI::Comm& comm) {
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
...
// template<>
// void file::reada(const std::string& name, Vector<int,2>& data, const Comm&
// comm, int splitDim) {
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
...
// template<>
// void file::reada(const std::string& name, Vector<float,2>& data, const Comm&
// comm, int splitDim) {
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
//     assert_throw(dims.getSize() == 2,
...
// template<>
// void file::reada(const std::string& name, Vector<double,2>& data, const Comm&
// comm, int splitDim) {
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
//     assert_throw(dims.getSize() == 2,
...
// template<>
// void file::reada(const std::string& name, Vector<double,2>& data, const
// VectorView<hsize_t>& elems, const Comm& comm) {
//     Vector<hsize_t> dims;
//     this->readDataDims(name, dims);
//     assert_throw(dims.getSize() == 2,
// 	   "Invalid data rank as compared to an array of rank 2.");
...
// void file::read(const std::string& name, void* data, DataType& dataType,
// const Comm& comm, int splitDim) {
//     DataSet fileSet;
//     fileSet.open(*this, name);
//     DataSpace fileSpace;
...
// void file::read(const std::string& name, void* data, DataType& dataType,
// const VectorView<hsize_t>& elems, const Comm& comm) {
//     DataSet fileSet;
//     fileSet.open(*this, name);
//     DataSpace fileSpace;
//     fileSet.getSpace(fileSpace);
...
// 			 "Attempting to read an element not within the bounds of
// the dataset."); 	    curDim = (curDim + 1)%fileDims.getSize();
// 	}
//     }
// #endif
...
//     this->read(subName, VectorView<int>(data.accDispls(),
//     proxy.getNumLocals()), comm); data.setupArray(true);
...
// template<>
// void file::reada(const std::string& name, CSR<int>& data, const
// VectorView<hsize_t>& rows, const Comm& comm)
// {
