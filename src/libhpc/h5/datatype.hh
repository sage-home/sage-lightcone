...
public:
  ...
  static datatype ieee_f32le;
  static datatype ieee_f64le;

  typedef boost::mpl::map<
      boost::mpl::pair<int, boost::mpl::int_<-2>>,
      ...
      boost::mpl::pair<std::string, boost::mpl::int_<-11>>>
      type_map;

  explicit datatype(hid_t id = -1);

  datatype(datatype const &src);

  inline datatype(BOOST_RV_REF(datatype) src) : _id(src._id) { src._id = -1; }

  ~datatype();

  inline datatype &operator=(BOOST_COPY_ASSIGN_REF(datatype) src) {
    close();
    if (src._id != -1)
  ...
  inline datatype &operator=(BOOST_RV_REF(datatype) src) {
    close();
    _id = src._id;
    src._id = -1;
    return *this;
  ...
  void compound(size_t size);

  unsigned n_members() const;

  std::string member_name(unsigned idx) const;

  size_t member_offset(unsigned idx) const;

  h5::datatype member_type(unsigned idx) const;

  void close();

  size_t size() const;

  void insert(datatype const &type, std::string const &desc, size_t offs);

  hid_t id() const;

  bool operator==(h5::datatype const &other) const;
