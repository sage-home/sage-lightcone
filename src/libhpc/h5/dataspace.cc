...
dataspace::dataspace(hid_tag tag, hid_t id)
    : _id(id)
{
}

dataspace::dataspace(hsize_t size, bool unlimited)
    : _id(-1)
{
...
void dataspace::select_hyperslab(H5S_seloper_t op, hsize_t count, hsize_t start,
                                 boost::optional<hsize_t> stride, boost::optional<hsize_t> block)
{
    ...
    if (count)
    {
        INSIST(H5Sselect_hyperslab(_id, op, &start, stride ? &*stride : NULL, &count,
                                   block ? &*block : NULL),
               >= 0);
    }
    else
...
void dataspace::select_hyperslab(hsize_t count, hsize_t start, hsize_t stride, hsize_t block,
                                 H5S_seloper_t op)
{
    ASSERT(simple_extent_num_dims() == 1);
