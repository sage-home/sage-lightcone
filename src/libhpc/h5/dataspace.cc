...
namespace hpc {
    namespace h5 {
        ...
        dataspace::dataspace(dataspace const &src) : _id(-1) {
            *this = src;
        }
        ...
        void dataspace::select_hyperslab(H5S_seloper_t            op,
            ...
            if(count) {
                INSIST(H5Sselect_hyperslab(_id, op, &start, stride ? &*stride : NULL, &count, block ? &*block : NULL),
                       >= 0);
            } else
        ...
        void
            dataspace::select_hyperslab(hsize_t count, hsize_t start, hsize_t stride, hsize_t block, H5S_seloper_t op) {
            ASSERT(simple_extent_num_dims() == 1);
