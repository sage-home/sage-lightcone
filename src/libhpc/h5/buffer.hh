...
public:
    ...
    void create(h5::dataset& dset, h5::datatype const& mem_type, size_t size = buffer_default_size,
                size_t offs = 0)
    {
        close();
        _dset = &dset;
    ...
    template <class U>
    typename boost::disable_if<random_access_trait<U>>::type write(U const& data)
    {
    ...
    template <class U>
    inline void write(std::vector<U> const& buf)
    {
...
protected:
    template <class DataT>
    void _write_to_file(typename hpc::type_traits<DataT>::const_reference data)
    {
        if (data.size())
        {
            ASSERT(_dset);
