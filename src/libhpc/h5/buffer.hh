...
namespace hpc {
    namespace h5 {
        ...
        template <class T> class buffer {
          ...
          public:
            buffer() : _dset(0), _mem_type(0), _offs(0) {
            }
            ...
            template <class U> typename boost::disable_if<random_access_trait<U>>::type write(U const &data) {
            ...
            template <class U> inline void write(std::vector<U> const &buf) {
          ...
          protected:
            template <class DataT> void _write_to_file(typename hpc::type_traits<DataT>::const_reference data) {
                if(data.size()) {
                    ASSERT(_dset);
                    ASSERT(_mem_type);
