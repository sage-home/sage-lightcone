...
public:
    ...
    template <class DimsT, typename boost::disable_if<boost::is_fundamental<DimsT>, int>::type = 0>
    ...
    inline dataspace(BOOST_RV_REF(dataspace) src)
        : _id(src._id)
    {
        src._id = -1;
    }
    ...
    void select_hyperslab(
        H5S_seloper_t op, typename type_traits<BufT>::const_reference count,
        ...
        typename type_traits<BufT>::const_reference block = typename type_traits<BufT>::value())
    {
        ...
        {
            INSIST(H5Sselect_hyperslab(_id, op, start.data(), stride.empty() ? 0 : stride.data(),
    ...
    void select_hyperslab(H5S_seloper_t op, hsize_t count, hsize_t start,
                          boost::optional<hsize_t> stride = boost::optional<hsize_t>(),
                          boost::optional<hsize_t> block = boost::optional<hsize_t>());

    void select_hyperslab(hsize_t count, hsize_t start, hsize_t stride = 1, hsize_t block = 1,
