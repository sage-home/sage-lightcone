...
public:
  ...
  ///               or H5F_ACC_RDWR.
  ///
  file(std::string const &filename, unsigned flags,
       mpi::comm const &comm = mpi::comm::self,
       property_list const &props = property_list());

  ~file();

  void open(std::string const &filename, unsigned flags,
            mpi::comm const &comm = mpi::comm::self,
            property_list const &props = property_list());
  ...
  template <class BufT, typename boost::disable_if<random_access_trait<BufT>,
                                                   int>::type = 0>
  ...
  void read(std::string const &name, void *buf, h5::datatype const &type,
            hsize_t size, hsize_t offs,
            mpi::comm const &comm = mpi::comm::self);

  template <class T>
  typename boost::disable_if<random_access_trait<T>, T>::type
  read(std::string const &name, hsize_t elem = 0) {
  ...
  read(const std::string &name, typename type_traits<Buffer>::reference buf,
       hsize_t offs = 0) {
    h5::dataset file_set(*this, name);
  ...
  typename type_traits<BufT>::value read(std::string const &name,
                                         hsize_t offs = 0) {
