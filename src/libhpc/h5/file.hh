...
public:
    ...
    ///               or H5F_ACC_RDWR.
    ///
    file(std::string const& filename, unsigned flags, mpi::comm const& comm = mpi::comm::self,
         property_list const& props = property_list());

    ~file();

    void open(std::string const& filename, unsigned flags, mpi::comm const& comm = mpi::comm::self,
              property_list const& props = property_list());
    ...
    h5::group group(std::string const& name) const;

    template <class BufT, typename boost::disable_if<random_access_trait<BufT>, int>::type = 0>
    void write(std::string const& name, BufT const& value)
    {
    ...
    template <class T>
    void write_serial(std::string const& name, std::vector<T> const& buf)
    {
        write_serial<std::vector<T>>(name, buf);
    }

    template <class BufT, typename boost::enable_if<random_access_trait<BufT>, int>::type = 0>
    void write_serial(std::string const& name, BufT const& buf)
    {
    ...
    template <class BufT, typename boost::enable_if<random_access_trait<BufT>, int>::type = 0>
    void write(std::string const& name, BufT const& buf)
    {
        typedef typename BufT::value_type value_type;
    ...
    void read(std::string const& name, void* buf, h5::datatype const& type, hsize_t size,
              hsize_t offs, mpi::comm const& comm = mpi::comm::self);

    template <class T>
    typename boost::disable_if<random_access_trait<T>, T>::type read(std::string const& name,
                                                                     hsize_t elem = 0)
    {
    ...
    read(const std::string& name, typename type_traits<Buffer>::reference buf, hsize_t offs = 0)
    {
        h5::dataset file_set(*this, name);
    ...
    void read(const std::string& name, typename type_traits<Buffer>::reference buf,
    ...
    typename type_traits<BufT>::value read(std::string const& name, hsize_t offs = 0)
    {
    ...
    // template< class T >
    // void reada(const std::string& name, Vector<T,2>& data, const MPI::Comm&
    // comm=MPI::Comm::SELF, int splitDim=0);

    // template<class T>
    // void reada(const std::string& name, Vector<T,2>& data, const
    // VectorView<hsize_t>& elems, const MPI::Comm& comm=MPI::Comm::SELF);
    ...
    //    // TODO: Try and handle the CSR one row at a time. Not sure if this is a
    //    very good idea or not.
