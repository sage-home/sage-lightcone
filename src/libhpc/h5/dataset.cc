...
dataset::dataset(hid_t id, bool dummy)
    : _id(id)
{
}

dataset::dataset(h5::location const& loc, std::string const& name)
    : _id(-1)
{
    open(loc, name);
}

dataset::dataset(h5::location& loc, std::string const& name, h5::datatype const& type,
                 h5::dataspace const& space, property_list const& props)
    : _id(-1)
{
    create(loc, name, type, space, props);
}

dataset::dataset(h5::location& loc, std::string const& name, h5::datatype const& type, hsize_t size,
...
void dataset::open(const h5::location& loc, const std::string& name)
{
    this->close();
...
void dataset::create(h5::location& loc, std::string const& name, h5::datatype const& type,
                     h5::dataspace const& space, property_list const& props)
{
    close();
...
void dataset::create(h5::location& loc, std::string const& name, h5::datatype const& type,
                     hsize_t size, property_list const& props)
{
    close();
    _create_groups(loc, name);
...
bool dataset::is_open() const { return _id >= 0; }
...
h5::datatype dataset::datatype() const
{
    ...
    ASSERT(type >= 0);
    {
        ...
        {
            return datatype::native_int;
        }
        else if (H5Tequal(type, H5T_NATIVE_LLONG))
        {
            return datatype::native_llong;
        }
    }
    return h5::datatype(type);
}

H5T_class_t dataset::type_class() const
{
    hid_t type = H5Dget_type(this->_id);
    ASSERT(type >= 0);
...
h5::dataspace dataset::dataspace() const
{
...
hsize_t dataset::extent() const { return this->dataspace().size(); }

void dataset::set_extent(hsize_t size)
{
...
void dataset::read(void* buf, const h5::datatype& mem_type, const h5::dataspace& mem_space,
                   const h5::dataspace& file_space, const mpi::comm& comm) const
{
    hid_t plist_id;
#ifdef PARALLELHDF5
...
void dataset::read(void* buf, h5::datatype const& dtype, hsize_t size, hsize_t offset,
...
void dataset::write(void const* buf, h5::datatype const& mem_type, h5::dataspace const& mem_space,
                    h5::dataspace const& file_space, mpi::comm const& comm,
                    h5::property_list const& props) const
{
    ...
    INSIST(H5Dwrite(_id, mem_type.id(), mem_space.id(), file_space.id(), local_props.id(), buf),
           >= 0);
}

void dataset::write(void const* buf, h5::datatype const& type, hsize_t size, hsize_t offset,
                    mpi::comm const& comm, h5::property_list const& props) const
{
    h5::dataspace file_space(dataspace());
#ifndef NDEBUG
    hsize_t file_size = H5Sget_select_npoints(file_space.id());
    ASSERT(offset + size <= file_size, "Trying to write out of range: offset=", offset,
           ", size=", size, ", dataset size=", file_size);
#endif
    file_space.select_hyperslab(H5S_SELECT_SET, size, offset);
...
void dataset::_create_groups(h5::location& loc, std::string const& name) const
{
