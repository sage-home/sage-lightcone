...
dataset::dataset() : _id(-1) {}

dataset::dataset(hid_t id, bool dummy) : _id(id) {}

dataset::dataset(h5::location const &loc, std::string const &name) : _id(-1) {
  open(loc, name);
}

dataset::dataset(h5::location &loc, std::string const &name,
                 h5::datatype const &type, h5::dataspace const &space,
...
dataset::dataset(h5::location &loc, std::string const &name,
                 h5::datatype const &type, hsize_t size,
                 property_list const &props)
    : _id(-1) {
...
void dataset::create(h5::location &loc, std::string const &name,
                     h5::datatype const &type, h5::dataspace const &space,
                     property_list const &props) {
  close();
...
void dataset::read(void *buf, const h5::datatype &mem_type,
  ...
  INSIST(H5Dread(this->_id, mem_type.id(), mem_space.id(), file_space.id(),
                 plist_id, buf),
         >= 0);
...
void dataset::read(void *buf, h5::datatype const &dtype, hsize_t size,
                   hsize_t offset, mpi::comm const &comm) const {
...
void dataset::write(void const *buf, h5::datatype const &mem_type,
  ...
  ASSERT(mem_size == file_size, "Dataspace sizes don't match: ", mem_size, ", ",
         file_size);
#endif
...
#else
  ASSERT(
      comm == mpi::comm::null || comm.size() == 1,
      "Attempting to create parallel HDF5 file without parallel extensions.");
#endif

  INSIST(H5Dwrite(_id, mem_type.id(), mem_space.id(), file_space.id(),
                  local_props.id(), buf),
         >= 0);
}

void dataset::write(void const *buf, h5::datatype const &type, hsize_t size,
                    hsize_t offset, mpi::comm const &comm,
                    h5::property_list const &props) const {
  h5::dataspace file_space(dataspace());
#ifndef NDEBUG
  hsize_t file_size = H5Sget_select_npoints(file_space.id());
  ASSERT(offset + size <= file_size,
         "Trying to write out of range: offset=", offset, ", size=", size,
