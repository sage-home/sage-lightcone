...
public:
    ...
    dataset(hid_t id, bool dummy);

    dataset(h5::location const& loc, std::string const& name);

    dataset(h5::location& loc, std::string const& name, h5::datatype const& type,
            h5::dataspace const& space, property_list const& props = property_list());

    dataset(h5::location& loc, std::string const& name, h5::datatype const& type, hsize_t size,
            property_list const& props = property_list());

    inline dataset(BOOST_RV_REF(dataset) src)
        : _id(src._id)
    {
        src._id = -1;
    }
    ...
    inline dataset& operator=(BOOST_RV_REF(dataset) src)
    {
        close();
        _id = src._id;
        src._id = -1;
    ...
    void open(h5::location const& loc, std::string const& name);

    void create(h5::location& loc, std::string const& name, h5::datatype const& type,
                h5::dataspace const& space, property_list const& props = property_list());

    void create(h5::location& loc, std::string const& name, h5::datatype const& type, hsize_t size,
                property_list const& props = property_list());
    ...
    hsize_t extent() const;

    void read(void* buf, h5::datatype const& mem_type,
    ...
    void read(void* buf, h5::datatype const& type, hsize_t size, hsize_t offset,
              mpi::comm const& comm = mpi::comm::self) const;

    template <class T>
    typename boost::disable_if<random_access_trait<T>, T>::type read(hsize_t elem) const
    {
    ...
    template <class BufT, typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
    void read(typename type_traits<BufT>::reference buf, hsize_t offs = 0,
              mpi::comm const& comm = mpi::comm::self) const
    {
    ...
    template <class BufT, typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
    void read(BufT&& buf, hsize_t offs = 0, mpi::comm const& comm = mpi::comm::self) const
    {
        read<BufT>(std::forward<BufT>(buf), offs, comm);
    }

    template <class BufT, typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
    void reada(typename type_traits<BufT>::reference buf,
               mpi::comm const& comm = mpi::comm::self) const
    {
        buf.resize(extent());
    ...
    template <class Buffer, class Elements>
    void read(typename type_traits<Buffer>::reference buf,
              typename type_traits<Elements>::const_reference elems,
              const mpi::comm& comm = mpi::comm::self) const
    {
    ...
    void write(void const* buf, h5::datatype const& mem_type,
               h5::dataspace const& mem_space = h5::dataspace::all,
               h5::dataspace const& file_space = h5::dataspace::all,
               mpi::comm const& comm = mpi::comm::self,
               h5::property_list const& props = h5::property_list()) const;

    void write(void const* buf, h5::datatype const& type, hsize_t size, hsize_t offset,
               mpi::comm const& comm = mpi::comm::self,
               h5::property_list const& props = h5::property_list()) const;

    template <class DataT>
    typename boost::disable_if<random_access_trait<DataT>>::type
    write(typename type_traits<DataT>::const_reference data, hsize_t offs = 0)
    {
    ...
    template <class BufT, typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
    void write(BufT const& buf, hsize_t offs = 0, mpi::comm const& comm = mpi::comm::self) const
    {
...
protected:
    void _create_groups(h5::location& loc, std::string const& name) const;
...
///
template <class T>
void location::write(std::string const& name, T const& value)
{
    BOOST_MPL_ASSERT((boost::mpl::has_key<h5::datatype::type_map, T>));
    h5::datatype type(boost::mpl::at<h5::datatype::type_map, T>::type::value);
...
void location::write(std::string const& name, typename type_traits<Buffer>::const_reference buf,
                     mpi::comm const& comm)
{
    typedef typename Buffer::value_type value_type;
...
template <class BufT, typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
h5::dataset const& operator>>(h5::dataset const& ds, BufT&& buf)
{
