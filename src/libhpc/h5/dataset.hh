    ...
    namespace h5 {
        ...
        class dataset {
          ...
          public:
            ...
            inline dataset(BOOST_RV_REF(dataset) src) : _id(src._id) {
                src._id = -1;
            }
            ...
            template <class T> typename boost::disable_if<random_access_trait<T>, T>::type read(hsize_t elem) const {
            ...
            void read(typename type_traits<BufT>::reference buf,
                      hsize_t                               offs = 0,
                      mpi::comm const &                     comm = mpi::comm::self) const {
            ...
            template <class BufT, typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
            void reada(typename type_traits<BufT>::reference buf, mpi::comm const &comm = mpi::comm::self) const {
                buf.resize(extent());
        ...
        ///
        ///
        template <class T> void location::write(std::string const &name, T const &value) {
            BOOST_MPL_ASSERT((boost::mpl::has_key<h5::datatype::type_map, T>));
            h5::datatype type(boost::mpl::at<h5::datatype::type_map, T>::type::value);
