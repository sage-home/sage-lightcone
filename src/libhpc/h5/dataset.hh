...
public:
  ...
  dataset(h5::location &loc, std::string const &name, h5::datatype const &type,
          hsize_t size, property_list const &props = property_list());

  inline dataset(BOOST_RV_REF(dataset) src) : _id(src._id) { src._id = -1; }
  ...
  void create(h5::location &loc, std::string const &name,
  ...
  void read(typename type_traits<BufT>::reference buf, hsize_t offs = 0,
    ...
    h5::datatype type(
        boost::mpl::at<h5::datatype::type_map, value_type>::type::value);
  ...
  template <class BufT,
            typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
  void read(BufT &&buf, hsize_t offs = 0,
            mpi::comm const &comm = mpi::comm::self) const {
  ...
  void write(void const *buf, h5::datatype const &type, hsize_t size,
             hsize_t offset, mpi::comm const &comm = mpi::comm::self,
             h5::property_list const &props = h5::property_list()) const;
  ...
  template <class BufT,
            typename boost::disable_if<is_fundamental_r<BufT>, int>::type = 0>
  void write(BufT const &buf, hsize_t offs = 0,
             mpi::comm const &comm = mpi::comm::self) const {
...
void location::write(std::string const &name,
