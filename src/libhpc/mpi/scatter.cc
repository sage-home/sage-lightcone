...
namespace hpc {
    namespace mpi {
        ...
        scatter::scatter() : _comm(&mpi::comm::world) {
        }

        scatter::scatter(surjection const &srj, datatype const &type) {
            construct(srj, type);
        }

        scatter::scatter(scatter &&src) :
            _comm(src._comm), _inc_types(std::move(src._inc_types)), _out_types(std::move(src._out_types)),
            _zeros(std::move(src._zeros)), _ones(std::move(src._ones)) {
        }
        ...
        void scatter::construct(surjection const &srj, datatype const &type) {
            _comm       = &srj.comm();
