...
void vct::iscatter(void const* out, void* inc, mpi::datatype const& type, mpi::requests& reqs,
                   unsigned block_size, int tag) const
{
    ...
    for (unsigned ii = 0; ii < num_nbrs; ++ii)
        _comm->isend((uint8_t*)out + ii * raw_block_size, type, _nbrs[ii], reqs[cur_req++],
                     block_size, tag);
    for (unsigned ii = 0; ii < num_nbrs; ++ii)
        _comm->irecv((uint8_t*)inc + ii * raw_block_size, type, _nbrs[ii], reqs[cur_req++],
                     block_size, tag);
}
