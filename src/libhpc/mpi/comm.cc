...
namespace hpc {
    namespace mpi {
        ...
        void comm::clear() {
            if(this->_comm != MPI_COMM_WORLD && this->_comm != MPI_COMM_NULL && this->_comm != MPI_COMM_SELF) {
        ...
        const MPI_Comm &comm::mpi_comm() const {
            return this->_comm;
        }
        ...
        void comm::isend(const void *out, const datatype &type, int to, request &req, int count, int tag) const {
            ASSERT(count >= 0);
            MPI_INSIST(
                MPI_Isend((void *)out, count, type.mpi_datatype(), to, tag, this->_comm, &req.mod_mpi_request()));
        }

        void comm::issend(const void *out, const datatype &type, int to, request &req, int count, int tag) const {
            ASSERT(count >= 0);
            MPI_INSIST(
                MPI_Issend((void *)out, count, type.mpi_datatype(), to, tag, this->_comm, &req.mod_mpi_request()));
        }
        ...
        void comm::sendrecv(void *               out_data,
                            int                  out_cnt,
                            ...
                            int                  src,
                            int                  inc_tag) const {
            MPI_INSIST(MPI_Sendrecv(out_data,
                                    out_cnt,
                                    out_type.mpi_datatype(),
                                    dst,
                                    out_tag,
                                    inc_data,
                                    inc_cnt,
                                    inc_type.mpi_datatype(),
                                    src,
                                    inc_tag,
                                    _comm,
                                    0));
        ...
        void comm::exchange(void *               out_data,
                            int                  out_cnt,
                            mpi::datatype const &out_type,
                            void *               inc_data,
                            int                  inc_cnt,
        ...
        void comm::abort(int ec) const {
            MPI_Abort(_comm, ec);
        }

        void comm::alltoallw(void const *    out_data,
                             int const *     out_cnts,
            ...
            MPI_INSIST(MPI_Alltoallw((void *)out_data,
                                     (int *)out_cnts,
                                     (int *)out_displs,
        ...
        bool comm::operator==(MPI_Comm comm) const {
