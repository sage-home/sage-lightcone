...
namespace hpc {
    namespace mpi {
        ...
        async_event_handler::async_event_handler(int tag) : _tag(tag) {
        }

        int async_event_handler::tag() const {
            return _tag;
        }

        async::async(mpi::comm const &comm) : _comm(&comm), _max_evts(0) {
        }

        void async::set_comm(mpi::comm const &comm) {
            _comm = &comm;
            _ev_hndlrs.clear();
        }

        mpi::comm const &async::comm() const {
            return *_comm;
        }

        mpi::comm const &async::worker_comm() const {
            return _wkr_comm;
        }

        void async::set_max_events(unsigned max_evts) {
            _max_evts = max_evts;
        }

        void async::add_event_handler(async::event_handler *eh) {
            ASSERT(eh);
            ASSERT(eh->tag() != 0);
        ...
        boost::unordered_map<int, async::event_handler *> const &async::event_handlers() const {
            return _ev_hndlrs;
        }
        ...
        bool async::run() {
            ...
            if(_comm->size() > 1) {
                ...
                do {
                    ...
                    LOGBLOCKD("Have an incoming event from ", stat.MPI_SOURCE, " with tag ", stat.MPI_TAG);
