...
int async_event_handler::tag() const { return _tag; }
...
mpi::comm const &async::comm() const { return *_comm; }

mpi::comm const &async::worker_comm() const { return _wkr_comm; }

void async::set_max_events(unsigned max_evts) { _max_evts = max_evts; }
...
bool async::run() {
  ...
  if (_comm->size() > 1) {
    ...
    do {
      ...
      LOGBLOCKD("Have an incoming event from ", stat.MPI_SOURCE, " with tag ",
                stat.MPI_TAG);
