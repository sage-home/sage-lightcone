    ...
    namespace mpi {
        ...
        bool datatype::is_primitive() const {
            return _type == MPI_DATATYPE_NULL ||
                   ...
                   _type == MPI_C_BOOL ||
#endif
                   _type == MPI_INT || _type == MPI_UNSIGNED || _type == MPI_LONG || _type == MPI_UNSIGNED_LONG ||
                   _type == MPI_LONG_LONG || _type == MPI_UNSIGNED_LONG_LONG || _type == MPI_BYTE ||
        ...
        const MPI_Datatype &datatype::mpi_datatype() const {
            return this->_type;
        }
        ...
        //       std::bind1st(std::multiplies<mpi::lindex>(), block_size)); MPI_Type_create_indexed_block(idxs.size(),
        //       block_size, (int*)idxs.data(), base._type, &this->_type); boost::transform(idxs,
        //       ((vector<mpi::lindex>::view&)idxs).begin(), std::bind2nd(std::divides<mpi::lindex>(), block_size));
        //    }
        //    else
