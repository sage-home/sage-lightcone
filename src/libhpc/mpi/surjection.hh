...
public:
  ...
  surjection(unsigned n_gelems, IdxsT const &idxs,
             mpi::comm const &comm = mpi::comm::world) {
    construct(n_gelems, idxs, comm);
  }

  template <class IdxsT>
  void construct(unsigned n_gelems, IdxsT const &idxs,
                 mpi::comm const &comm = mpi::comm::world) {
    ...
    _make_required(n_gelems, idxs, n_ranks, req_idxs, _req_cnts, _req_displs,
                   _local);
    ...
    comm.alltoallv(req_idxs, _req_cnts, _req_displs, _out_idxs, _out_cnts,
                   _out_displs);
...
protected:
  std::vector<unsigned> _count_required(unsigned n_gelems,
                                        std::vector<unsigned> const &idxs,
                                        int n_ranks);

  void _make_required(unsigned n_gelems, std::vector<unsigned> const &idxs,
                      int n_ranks, std::vector<unsigned> &req_idxs,
