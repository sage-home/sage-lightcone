...
public:
    ...
    surjection(unsigned n_gelems, IdxsT const& idxs, mpi::comm const& comm = mpi::comm::world)
    {
        construct(n_gelems, idxs, comm);
    }

    template <class IdxsT>
    void construct(unsigned n_gelems, IdxsT const& idxs, mpi::comm const& comm = mpi::comm::world)
    {
    ...
    mpi::scatter scatter(datatype const& type) const;

    hpc::view<std::vector<unsigned>> out_indices(int rank) const;

    hpc::view<std::vector<unsigned>> inc_indices(int rank) const;

    mpi::comm const& comm() const;

protected:
    std::vector<unsigned> _count_required(unsigned n_gelems, std::vector<unsigned> const& idxs,
                                          int n_ranks);

    void _make_required(unsigned n_gelems, std::vector<unsigned> const& idxs, int n_ranks,
                        std::vector<unsigned>& req_idxs, std::vector<unsigned>& req_cnts,
                        std::vector<unsigned> const& req_displs, std::vector<unsigned>& local);
