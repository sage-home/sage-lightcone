...
public:
  ...
  template <class Seq>
  vct(Seq &&nbrs, mpi::comm const &comm = mpi::comm::world)
  ...
  template <class Seq>
  vct(Seq const &nbrs, mpi::comm const &comm = mpi::comm::world)
      : _comm((mpi::comm const *)&comm) {
    set_neighbors(nbrs);
  }
  ...
  template <class Seq> void set_neighbors(Seq &&nbrs) {
  ...
  template <class Seq> void set_neighbors(Seq const &nbrs) {
    ASSERT(!has_duplicates(nbrs), "Duplicates in neighbors.");
    ASSERT(!has_element(nbrs, _comm->rank()), "Neighbors contains my rank.");
    ASSERT(is_ordered(nbrs), "Neighbors are not ordered.");
    clear();
  ...
  unsigned n_neighbors() const;

  std::vector<unsigned> const &neighbors() const;

  unsigned rank_to_nbr(unsigned rank) const;

  template <class T>
  void ibcast(T const &out, view<std::vector<T>> inc, mpi::requests &reqs,
              int tag = 0) const {
    size_t num_nbrs = _nbrs.size();
  ...
  template <class T>
  void bcast(T const &out, view<std::vector<T>> inc, int tag = 0) const {
    mpi::requests reqs;
    ibcast(out, inc, reqs, tag);
  }

  void iscatter(void const *out, void *inc, mpi::datatype const &type,
                mpi::requests &reqs, unsigned block_size = 1,
                int tag = 0) const;

  template <class T>
  void iscatter(view<std::vector<T>> const &out, view<std::vector<T>> inc,
                mpi::requests &reqs, int tag = 0) const {
  ...
  template <class T>
  void scatter(view<std::vector<T>> const &out, view<std::vector<T>> inc,
               int tag = 0) const {
    mpi::requests reqs;
  ...
  void iscatter(void *out, view<std::vector<Index>> const &out_displs,
                void *inc, view<std::vector<Index>> inc_displs,
                mpi::datatype const &type, mpi::requests &reqs,
    ...
    // Check that the outgoing counts match the incoming counts.
    {
      ...
      for (unsigned ii = 0; ii < check_cnts.size(); ++ii)
        ASSERT(check_cnts[ii] == (inc_displs[ii + 1] - inc_displs[ii]),
               "VCT scatter sizes don't match.");
    }
    // Check that the number of blocks involved is constant.
    {
      ...
      for (unsigned ii = 0; ii < check_blocks.size(); ++ii)
        ASSERT(check_blocks[ii] == block_size,
               "VCT scatter block sizes don't match.");
    }
#endif
    ...
    unsigned cur_req = 0;
    for (unsigned ii = 0; ii < num_nbrs; ++ii) {
      _comm->isend((uint8_t *)out + out_displs[ii] * raw_block_size, type,
                   _nbrs[ii], reqs[cur_req++],
                   out_displs[ii + 1] - out_displs[ii], tag);
    }
    for (unsigned ii = 0; ii < num_nbrs; ++ii) {
      _comm->irecv((uint8_t *)inc + inc_displs[ii] * raw_block_size, type,
                   _nbrs[ii], reqs[cur_req++],
                   inc_displs[ii + 1] - inc_displs[ii], tag);
    }
  }

  template <class T, class Index>
  void iscatter(view<std::vector<T>> const &out,
                view<std::vector<Index>> const &out_displs,
                view<std::vector<T>> inc, view<std::vector<Index>> inc_displs,
                mpi::requests &reqs, int tag = 0) const {
    mpi::datatype type(MPI_MAP_TYPE(T));
    iscatter<Index>(out.data(), out_displs, inc.data(), inc_displs, type, reqs,
                    1, tag);
  }

  template <class T, class Index>
  void scatter(view<std::vector<T>> const &out,
               view<std::vector<Index>> const &out_displs,
               view<std::vector<T>> inc, view<std::vector<Index>> inc_displs,
               int tag = 0) const {
    mpi::requests reqs;
  ...
  template <class T, class Index>
  void scattera(view<std::vector<T>> const &out,
                view<std::vector<Index>> const &out_displs, std::vector<T> &inc,
                std::vector<Index> &inc_displs, int tag = 0) const {
    if (_nbrs.size()) {
  ...
  template <class Iter>
  void iscatter_types(void const *out, Iter out_datatypes, void *inc,
                      Iter inc_datatypes, mpi::requests &reqs) const {
    unsigned num_nbrs = _nbrs.size();
    reqs.resize(2 * num_nbrs);
    unsigned cur_req = 0;
    for (unsigned ii = 0; ii < num_nbrs; ++ii) {
  ...
  template <class Iter>
  void scatter_types(void const *out, Iter out_datatypes, void *inc,
  ...
  void scatter_displs(view<std::vector<DisplType>> const &out,
                      view<std::vector<DisplType>> inc, int tag = 0) const {
    typedef view<std::vector<DisplType>> view_type;

    ASSERT(out.size() == inc.size());
    if (out.size() > 1) {
      hpc::displs_to_counts<view_type>((view_type &)out);
      scatter<DisplType>(view_type(out, out.size() - 1),
                         view_type(inc, inc.size() - 1));
