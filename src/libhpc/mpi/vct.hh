...
public:
    ...
    template <class Seq>
    void set_neighbors(Seq const& nbrs)
    {
        ASSERT(!has_duplicates(nbrs), "Duplicates in neighbors.");
        ASSERT(!has_element(nbrs, _comm->rank()), "Neighbors contains my rank.");
        ASSERT(is_ordered(nbrs), "Neighbors are not ordered.");
    ...
    template <class T>
    void ibcast(T const& out, view<std::vector<T>> inc, mpi::requests& reqs, int tag = 0) const
    {
    ...
    void iscatter(void const* out, void* inc, mpi::datatype const& type, mpi::requests& reqs,
                  unsigned block_size = 1, int tag = 0) const;

    template <class T>
    void iscatter(view<std::vector<T>> const& out, view<std::vector<T>> inc, mpi::requests& reqs,
                  int tag = 0) const
    {
    ...
    template <class T>
    void scatter(view<std::vector<T>> const& out, view<std::vector<T>> inc, int tag = 0) const
    {
        mpi::requests reqs;
    ...
    void iscatter(void* out, view<std::vector<Index>> const& out_displs, void* inc,
                  view<std::vector<Index>> inc_displs, mpi::datatype const& type,
                  mpi::requests& reqs, unsigned block_size = 1, int tag = 0) const
    {
#ifndef NDEBUG
        ...
        for (unsigned ii = 0; ii < num_nbrs; ++ii)
        {
            _comm->isend((uint8_t*)out + out_displs[ii] * raw_block_size, type, _nbrs[ii],
                         reqs[cur_req++], out_displs[ii + 1] - out_displs[ii], tag);
        }
        for (unsigned ii = 0; ii < num_nbrs; ++ii)
        {
            _comm->irecv((uint8_t*)inc + inc_displs[ii] * raw_block_size, type, _nbrs[ii],
                         reqs[cur_req++], inc_displs[ii + 1] - inc_displs[ii], tag);
        }
    }

    template <class T, class Index>
    void iscatter(view<std::vector<T>> const& out, view<std::vector<Index>> const& out_displs,
    ...
    template <class T, class Index>
    void scatter(view<std::vector<T>> const& out, view<std::vector<Index>> const& out_displs,
                 view<std::vector<T>> inc, view<std::vector<Index>> inc_displs, int tag = 0) const
    {
        mpi::requests reqs;
    ...
    template <class T, class Index>
    void scattera(view<std::vector<T>> const& out, view<std::vector<Index>> const& out_displs,
                  std::vector<T>& inc, std::vector<Index>& inc_displs, int tag = 0) const
    {
        if (_nbrs.size())
        {
    ...
    template <class Iter>
    void iscatter_types(void const* out, Iter out_datatypes, void* inc, Iter inc_datatypes,
    ...
    template <class Iter>
    void scatter_types(void const* out, Iter out_datatypes, void* inc, Iter inc_datatypes) const
    {
        mpi::requests reqs;
    ...
    void scatter_displs(view<std::vector<DisplType>> const& out, view<std::vector<DisplType>> inc,
                        int tag = 0) const
    {
