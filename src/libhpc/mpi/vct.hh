    ...
    namespace mpi {
        ...
        class vct {
          public:
            ...
            template <class Seq>
            vct(Seq &&nbrs, mpi::comm const &comm = mpi::comm::world) : _comm((mpi::comm const *)&comm) {
            ...
            template <class Seq>
            vct(Seq const &nbrs, mpi::comm const &comm = mpi::comm::world) : _comm((mpi::comm const *)&comm) {
                set_neighbors(nbrs);
            }
            ...
            template <class T> void bcast(T const &out, view<std::vector<T>> inc, int tag = 0) const {
                mpi::requests reqs;
                ibcast(out, inc, reqs, tag);
            }

            void iscatter(void const *         out,
                          void *               inc,
            ...
            template <class Iter>
            void iscatter_types(void const *out, Iter out_datatypes, void *inc, Iter inc_datatypes, mpi::requests &reqs)
                const {
                unsigned num_nbrs = _nbrs.size();
                reqs.resize(2 * num_nbrs);
                unsigned cur_req = 0;
