...
bool start_chunk(int chunk, int nchunks) {
  ...
  if (_nchunks == 1)
    return true;
  for (int rank = 0; rank < comm::world.size(); rank++) {
    int me = rank * _nchunks + _chunk;
    std::string fn("checkpoint_");
    fn += std::to_string(me);
    fn += ".txt";
...
bool finish_chunk(int chunk, int nchunks) {
  int me = comm::world.rank() * _nchunks + _chunk;
  std::string fn("checkpoint_");
  fn += std::to_string(me);
  fn += ".txt";
  std::ofstream file(fn);
  file << _calls << std::endl;
  file.close();
  return true;
}
int chunk() { return _chunk; }
int nchunks() { return _nchunks; }
void set_rng(unsigned long long calls) { _calls = calls; }
unsigned long long get_rng() { return _calls; }
...
void initialise(int &argc, char **&argv) {
  ...
  if (!_init) {
    // Need to initialise data types here because we calculate sizes during
    // construction, which needs MPI to be initialized. Note that NULL is
    // already done.
