...
public:
  ...
  template <class Iter>
  mpi::comm construct(Iter const &begin, Iter const &end, unsigned offs = 0) {
    clear();
    if (_comm->size() == 1)
  ...
  template <class Iter> void split(Iter const &begin, Iter const &end) {
    clear();
    if (_comm->size() == 1)
  ...
  template <class Iter> void connect(Iter const &begin, Iter const &end) {
    clear();
    if (_comm->size() == 1)
  ...
  void transfer(void *data, mpi::datatype const &type) const;

  template <class Vec>
  void transfer(typename type_traits<Vec>::reference data) const {
    typedef typename type_traits<Vec>::value::value_type value_type;
    transfer(data.data(), mpi::datatype(MPI_MAP_TYPE(value_type)));
  }

  template <class T> void transfer(T &&data) const {
  ...
  bool collecting_left() const;

  unsigned left_size() const;

  unsigned right_size() const;

  std::vector<unsigned> const displs() const;

  std::vector<unsigned> const indices() const;

  std::vector<int> const partners() const;

protected:
  template <class Iter>
  mpi::comm _construct_serial(Iter const &begin, Iter const &end,
                              unsigned offs = 0) {
  ...
  template <class Iter>
  mpi::comm _construct_parallel(Iter const &begin, Iter const &end) {
  ...
  template <class Iter>
  void _split_parallel(Iter const &begin, Iter const &end) {
    ...
    ASSERT(comm.all_reduce(std::accumulate(begin, end, 0)) ==
               balanced_right_size(end - begin, *_comm),
           "Partition is not balanced, right sizes: ",
           comm.all_reduce(std::accumulate(begin, end, 0)), " -- ",
           balanced_right_size(end - begin, *_comm));
    ...
    for (unsigned ii = _balances.size();
         ii > _balances.size() - (_comm->size() / 2 + _comm->size() % 2);
         --ii) {
  ...
  template <class Iter>
  void _connect_parallel(Iter const &begin, Iter const &end) {
    mpi::comm const &comm = *_comm;
    int my_rank = comm.rank();
    ...
    for (int phase = 0; phase < 3; ++phase) {
      ...
      if (phase == 0)
        ...
        LOGDLN("Counts: ",
               hpc::view<std::vector<unsigned>>(_displs, _displs.size() - 1));
      ...
      while (left < _ranks.size() && bals[left] > 0) {
        ...
        ASSERT(
            bals[right] < 0,
            "Internal partition error: cannot have more on left than right.");
        ...
        if (_ranks[left] == my_rank || _ranks[right] == my_rank) {
          if (phase == 0) {
            ...
            if (phase == 2) {
              int partner =
                  (_ranks[left] == my_rank) ? _ranks[right] : _ranks[left];
