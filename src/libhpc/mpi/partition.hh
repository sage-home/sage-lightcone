...
public:
    ...
    template <class Iter>
    void split(Iter const& begin, Iter const& end)
    {
        clear();
        if (_comm->size() == 1)
    ...
    template <class Iter>
    void connect(Iter const& begin, Iter const& end)
    {
        clear();
        if (_comm->size() == 1)
...
protected:
    template <class Iter>
    mpi::comm _construct_serial(Iter const& begin, Iter const& end, unsigned offs = 0)
    {
    ...
    void _split_parallel(Iter const& begin, Iter const& end)
    {
        ...
        for (unsigned ii = _balances.size();
             ii > _balances.size() - (_comm->size() / 2 + _comm->size() % 2); --ii)
        {
