    ...
    namespace mpi {
        ...
        class balanced_partition {
          public:
            ...
            template <class Iter> mpi::comm construct(Iter const &begin, Iter const &end, unsigned offs = 0) {
                clear();
                if(_comm->size() == 1)
            ...
            template <class Vec> void transfer(typename type_traits<Vec>::reference data) const {
          ...
          protected:
            template <class Iter> mpi::comm _construct_serial(Iter const &begin, Iter const &end, unsigned offs = 0) {
            ...
            template <class Iter> mpi::comm _construct_parallel(Iter const &begin, Iter const &end) {
            ...
            template <class Iter> void _split_parallel(Iter const &begin, Iter const &end) {
                ...
                for(unsigned ii = _balances.size(); ii > _balances.size() - (_comm->size() / 2 + _comm->size() % 2);
                    --ii) {
            ...
            template <class Iter> void _connect_parallel(Iter const &begin, Iter const &end) {
                mpi::comm const &comm    = *_comm;
                int              my_rank = comm.rank();
