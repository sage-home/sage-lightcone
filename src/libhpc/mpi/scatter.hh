    ...
    namespace mpi {
        ...
        class scatter {
          public:
            ...
            template <class IdxsT>
            void construct(unsigned         n_gelems,
                           IdxsT const &    idxs,
                           datatype const & type,
                           mpi::comm const &comm = mpi::comm::world) {
                // COMMENTED-OUT BY GBP TO PREVENT A BUILD ERROR: variable has incomplete type 'hpc::mpi::surjection'
                // surjection srj( n_gelems, idxs, comm );
                // construct( srj, type );
            }

            template <class SrcT, class DstT> inline void transfer(SrcT const &src, DstT &&dst) {
