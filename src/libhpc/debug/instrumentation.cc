...
extern "C"
{
    ...
    struct instr_node
    {
        instr_node(hpc::debug::instrument* instr, instr_node* next = NULL)
            : instr(instr)
            , next(next)
            , ignore(true)
        {
    ...
    void add_instrument(hpc::debug::instrument& instr)
    {
        instr_head = new instr_node(&instr, instr_head);
    }

    void instr_func_details(void* func_addr, const char** file_name, char** func_name)
    {
        hpc::debug::func_details(func_addr, file_name, func_name);
    }

    instr_enabled_node& instr_get_enabled_node()
    {
    ...
    bool instr_is_enabled()
    {
        instr_enabled_node& node = instr_get_enabled_node();
        return node.enabled;
    }

    void instr_set_enabled(bool enabled)
    {
        instr_enabled_node& node = instr_get_enabled_node();
...
instrument::instrument() { add_instrument(*this); }
} // namespace debug
} // namespace hpc
