...
#include "libhpc/mpi/init.hh" // Replaces <mpi.h>
...
logger::logger(unsigned min_level, const std::string& tag)
...
void logger::add_tag(const std::string& tag) { _tags.insert(tag); }

void logger::push_level(unsigned level) { levels().push_front(level); }

void logger::pop_level() { levels().pop_front(); }

void logger::push_tag(const std::string& tag)
{
...
void logger::pop_tag(const std::string& tag)
{
...
bool logger::visible()
{
    ...
    if (!tag_vis)
    {
        for (std::set<std::string>::const_iterator it = _tags.begin(); it != _tags.end(); ++it)
        {
...
std::list<unsigned>& logger::levels()
{
    boost::thread::id tid = boost::this_thread::get_id();
    _levels_mutex.lock();
...
std::map<std::string, int>& logger::current_tags()
{
    boost::thread::id tid = boost::this_thread::get_id();
    _tags_mutex.lock();
...
void logger::_traits<logger&(logger&)>::impl::operator()(logger& log, logger& (*fp)(logger&))
{
    fp(log);
}

void logger::_traits<setindent_t>::impl::operator()(logger& log, setindent_t si) { log(si, false); }
