...
namespace hpc {
    namespace log {
        ...
        logger::logger(unsigned min_level, const std::string &tag) : _min_level(min_level) {
            if(!tag.empty())
                _tags.insert(tag);
        }

        logger::~logger() {
            for(std::map<boost::thread::id, std::stringstream *>::iterator it = _buf.begin(); it != _buf.end(); ++it) {
                if(it->second)
                    delete it->second;
            }
        }
        ...
        void logger::add_tag(const std::string &tag) {
            _tags.insert(tag);
        }

        void logger::push_level(unsigned level) {
        ...
        void logger::_traits<setindent_t>::impl::operator()(logger &log, setindent_t si) {
            log(si, false);
        }
