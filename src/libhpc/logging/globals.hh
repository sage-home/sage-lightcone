...
#define _LOG_(count, ...) _OSTREAM##count(::hpc::log::_stack, __VA_ARGS__)

#define _LOG(count, ...) _LOG_(count, __VA_ARGS__)

#define LOG(...) _LOG(PP_NARG(0, ##__VA_ARGS__), ##__VA_ARGS__)

#define LOGLN(...) LOG(__VA_ARGS__, ::hpc::log::endl)

#define LOGLV(level, ...) LOG(::hpc::log::pushlevel(level), __VA_ARGS__, ::hpc::log::poplevel)

#define LOGLVLN(level, ...) LOG(::hpc::log::pushlevel(level), __VA_ARGS__, ::hpc::log::endl, ::hpc::log::poplevel)
...
#ifndef NLOGDEBUG
#define LOGD(...) LOGLV(::hpc::log::debug, __VA_ARGS__)
#else
#define LOGD(...)
#endif

#define LOGI(...) LOGLV(::hpc::log::info, __VA_ARGS__)

#define LOGW(...) LOGLV(::hpc::log::warning, __VA_ARGS__)

#define LOGE(...) LOGLV(::hpc::log::error, __VA_ARGS__)

#ifndef NLOGTRIVIAL
#define LOGTLN(...) LOGLVLN(::hpc::log::trivial, __VA_ARGS__)
#else
#define LOGTLN(...)
#endif
...
#define LOGILN(...) LOGLVLN(::hpc::log::info, __VA_ARGS__)

#define LOGWLN(...) LOGLVLN(::hpc::log::warning, __VA_ARGS__)
...
#define LOG_FILE(filename) ::hpc::log::push(new ::hpc::log::file(filename))

#define LOG_CONSOLE() ::hpc::log::push(new ::hpc::log::stdout())

#define LOG_POP() ::hpc::log::pop()

#define LOG_PUSH_TAG(tag) ::hpc::log::push_tag(tag)

#define LOG_POP_TAG(tag) ::hpc::log::pop_tag(tag)
