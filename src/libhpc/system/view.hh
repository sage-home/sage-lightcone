...
namespace hpc {
    ...
    template <class Vector> class const_view {
      ...
      public:
        ...
        const_view(const_pointer ptr, size_t size, size_t offs = 0) : _ptr((pointer)ptr + offs), _size(size) {
        }
    ...
    class view<Vector, typename boost::enable_if<boost::is_const<Vector>>::type> : public const_view<Vector> {
      public:
      ...
      public:
        ...
        template <class Other, typename boost::enable_if<boost::is_base_of<vector_type, Other>, int>::type = 0>
        view(Other const &vec, size_t size, size_t offs = 0) : super_type(vec, size, offs) {
        }

        view(const_pointer ptr, size_t size, size_t offs = 0) : super_type(ptr, size, offs) {
        }
    };

    template <class Vector>
    class view<Vector, typename boost::disable_if<boost::is_const<Vector>>::type> : public const_view<Vector> {
      public:
        typedef const_view<Vector> super_type;
        typedef Vector             vector_type;
        typedef size_t             size_type;
    ...
    template <class T>
    class view_iterator : public boost::iterator_facade<view_iterator<T>, T, std::random_access_iterator_tag> {
      ...
      public:
        ...
        template <class U> view_iterator(const view_iterator<U> &src) : _ptr(src._ptr) {
        }
    ...
    template <class Vector> std::ostream &operator<<(std::ostream &strm, const_view<Vector> const &obj) {
        strm << "[";
        if(obj.size()) {
            strm << obj[0];
    ...
    template <class Vector> struct random_access_trait<view<Vector>> : boost::true_type {};
