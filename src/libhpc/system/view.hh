...
template <class Vector> class const_view {
public:
...
public:
  const_view() : _ptr(0), _size(0) {}

  template <class Other,
            typename boost::enable_if<boost::is_base_of<vector_type, Other>,
                                      int>::type = 0>
  const_view(Other const &vec) : _ptr((pointer)vec.data()), _size(vec.size()) {}

  template <class Other,
            typename boost::enable_if<boost::is_base_of<vector_type, Other>,
                                      int>::type = 0>
  const_view(Other const &vec, size_t size, size_t offs = 0)
      : _ptr((pointer)vec.data() + offs), _size(size) {
    ASSERT(size > 0);
    ASSERT(offs >= 0);
    ASSERT(offs + size <= vec.size());
  }

  const_view(const_pointer ptr, size_t size, size_t offs = 0)
      : _ptr((pointer)ptr + offs), _size(size) {}

  void assign(const_pointer ptr, size_t size = 0) {
    _ptr = (pointer)ptr;
    _size = size;
  }
  ...
  template <class Other,
            typename boost::enable_if<boost::is_base_of<vector_type, Other>,
                                      int>::type = 0>
  void assign(Other const &op, size_t size = 0, size_t offs = 0) {
  ...
  //         typename
  //         boost::enable_if<boost::is_base_of<vector_type,Other>,int>::type =
  //         0 )
  // {
  ...
  void shrink(size_t size, size_t offs = 0) {
  ...
  const_pointer const data() const { return _ptr; }
  ...
  const_element_reference operator[](size_t idx) const {
  ...
  template <class Other,
            typename boost::enable_if<boost::is_base_of<vector_type, Other>,
                                      int>::type = 0>
  bool compare(Other const &op) const {
    if (_size != op._size)
      return false;
  ...
  template <class Other> bool operator==(Other const &op) {
...
template <class Vector, class Enable = void> class view;

template <class Vector>
class view<Vector, typename boost::enable_if<boost::is_const<Vector>>::type>
...
public:
  ...
  template <class Other,
            typename boost::enable_if<boost::is_base_of<vector_type, Other>,
                                      int>::type = 0>
  view(Other const &vec) : super_type(vec) {}

  template <class Other,
            typename boost::enable_if<boost::is_base_of<vector_type, Other>,
                                      int>::type = 0>
  view(Other const &vec, size_t size, size_t offs = 0)
      : super_type(vec, size, offs) {}

  view(const_pointer ptr, size_t size, size_t offs = 0)
      : super_type(ptr, size, offs) {}
};

template <class Vector>
class view<Vector, typename boost::disable_if<boost::is_const<Vector>>::type>
    : public const_view<Vector> {
public:
  typedef const_view<Vector> super_type;
  typedef Vector vector_type;
...
public:
  ...
  template <class Other> view &operator=(Other const &op) {
    this->template assign<Other>(op);
    return *this;
  }

  operator pointer() { return this->_ptr; }

  operator const pointer() const { return this->_ptr; }
};

template <class T>
class view_iterator
    : public boost::iterator_facade<view_iterator<T>, T,
                                    std::random_access_iterator_tag> {
  friend class boost::iterator_core_access;
  // template< class > friend class view_iterator;

public:
  explicit view_iterator(T *ptr = 0) : _ptr(ptr) {}

  template <class U>
  view_iterator(const view_iterator<U> &src) : _ptr(src._ptr) {}
...
protected:
  ...
  template <class U> bool equal(const view_iterator<U> &op) const {
    return _ptr == op._ptr;
  }

  T &dereference() const { return *_ptr; }

  template <class U> size_t distance_to(const view_iterator<U> &op) const {
    return op._ptr - _ptr;
  }

  void advance(size_t dist) { _ptr += dist; }
...
template <class Vector>
std::ostream &operator<<(std::ostream &strm, const_view<Vector> const &obj) {
  strm << "[";
  if (obj.size()) {
    strm << obj[0];
...
template <class Vector> struct type_traits<view<Vector>> {
...
template <class Vector>
struct random_access_trait<view<Vector>> : boost::true_type {};
