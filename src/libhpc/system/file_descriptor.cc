...
namespace hpc {
    namespace os {
        ...
        file_descriptor::file_descriptor(fs::path const &pathname, file_descriptor::flags_type flags) : _fd(-1) {
            open(pathname, flags);
        }

        file_descriptor::file_descriptor(file_descriptor &&src) noexcept : _fd(src._fd) {
            src._fd = -1;
        }
