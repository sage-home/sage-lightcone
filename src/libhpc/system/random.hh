...
namespace hpc {
    ...
    typedef mpl::map<
        mpl::pair<double, boost::variate_generator<engine_type &, boost::random::uniform_real_distribution<double>>>,
        mpl::pair<float, boost::variate_generator<engine_type &, boost::random::uniform_real_distribution<float>>>,
        mpl::pair<int, boost::variate_generator<engine_type &, boost::random::uniform_int_distribution<int>>>>
    ...
    typename generator_types<T>::uniform_real
        uniform_real_generator(const T &low, const T &high, unsigned int seed = std::time(0)) {
    ...
    template <class T> T generate_uniform(const T &low = 0, const T &high = 1, engine_type &engine = hpc::engine) {
        return typename mpl::at<uniform_distribution_types, T>::type(low, high)(engine);
    }

    template <class T> T generate_normal(const T &mean = 0, const T &std_dev = 1, engine_type &engine = hpc::engine) {
    ...
    template <class T> class normal_generator {
      public:
        normal_generator(const T &mean = 0, const T &std_dev = 1) : _dist(mean, std_dev) {
        }
