...
typedef mpl::map<
    mpl::pair<double, boost::variate_generator<engine_type&,
                                               boost::random::uniform_real_distribution<double>>>,
    mpl::pair<float, boost::variate_generator<engine_type&,
                                              boost::random::uniform_real_distribution<float>>>,
    mpl::pair<int,
              boost::variate_generator<engine_type&, boost::random::uniform_int_distribution<int>>>>
    uniform_generator_types;

template <class T>
struct generator_types
{
    typedef boost::variate_generator<boost::mt11213b, boost::random::uniform_real_distribution<T>>
        uniform_real;
};

template <class T>
typename generator_types<T>::uniform_real uniform_real_generator(const T& low, const T& high,
                                                                 unsigned int seed = std::time(0))
{
    boost::mt11213b eng(seed);
    boost::random::uniform_real_distribution<double> dist(low, high);
    boost::variate_generator<boost::mt11213b, boost::random::uniform_real_distribution<double>> rnd(
        eng, dist);
    return rnd;
}

template <class T>
T generate_uniform(const T& low = 0, const T& high = 1, engine_type& engine = hpc::engine)
{
    return typename mpl::at<uniform_distribution_types, T>::type(low, high)(engine);
}

template <class T>
T generate_normal(const T& mean = 0, const T& std_dev = 1, engine_type& engine = hpc::engine)
{
...
{
public:
    uniform_generator(const T& low = 0, const T& high = 1)
        : _dist(low, high)
    {
    }
