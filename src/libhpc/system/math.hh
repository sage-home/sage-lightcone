...
template <class T>
CUDA_DEV_HOST_INL T fabs(T const& x);

template <>
CUDA_DEV_HOST_INL float fabs(float const& x)
{
    return ::fabsf(x);
}

template <>
CUDA_DEV_HOST_INL double fabs(double const& x)
{
    return ::fabs(x);
}
...
template <class T>
T distribute(T x, T rank, T num_ranks)
{
    return x / num_ranks + ((rank < (x % num_ranks)) ? 1 : 0);
}

template <class T>
hpc::array<T, 2> modulo(T size, T rank, T group)
{
#ifdef CXX_0X
