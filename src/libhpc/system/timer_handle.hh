...
namespace hpc {
    ...
    template <class TimeT, class ClockT> class timer_handle {
      ...
      public:
        timer_handle(timer_type *timer = 0, stop_type stop = NORMAL) : _timer(timer), _stop(stop) {
        }

        inline timer_handle(BOOST_RV_REF(timer_handle) src) : _timer(src._timer), _stop(src._stop) {
            src._timer = 0;
        }
