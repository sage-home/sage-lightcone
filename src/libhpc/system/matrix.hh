...
public:
  ...
  matrix(size_type n_rows, size_type n_cols)
      : _vec(n_rows * n_cols), _size{n_rows, n_cols} {}
  ...
  size_type n_rows() const { return _size[0]; }

  size_type n_cols() const { return _size[1]; }

  view<std::vector<value_type>> operator[](size_type row) {
    ASSERT(row < _size[0], "Matrix row out of bounds.");
    return view<std::vector<value_type>>(_vec, _size[1], row * _size[1]);
  }

  view<std::vector<value_type> const> operator[](size_type row) const {
    ASSERT(row < _size[0], "Matrix row out of bounds.");
  ...
  const_rows_iterator rows_begin(size_t row = 0) const {
    return const_rows_iterator(_vec.data() + row * _size[1], _size[1]);
  }

  const_rows_iterator rows_end() const {
    return const_rows_iterator(_vec.data() + _vec.size(), _size[1]);
  }

  const_rows_iterator rows_end(size_t row) const {
    return const_rows_iterator(_vec.data() + row * _size[1], _size[1]);
  }
...
class matrix_rows_iterator
    : public boost::iterator_facade<matrix_rows_iterator<T>, hpc::view<T>,
...
public:
  explicit matrix_rows_iterator(const_pointer_type ptr = 0, size_t row_size = 0)
      : _ptr((pointer_type)ptr), _row_size(row_size) {}

  template <class U>
  matrix_rows_iterator(matrix_rows_iterator<U> const &src)
      : _ptr(src._ptr), _row_size(src._row_size) {}

protected:
  void increment() { _ptr += _row_size; }

  void decrement() { _ptr -= _row_size; }

  template <class U> bool equal(matrix_rows_iterator<U> const &op) const {
    return _ptr == op._ptr;
  }

  reference_type dereference() const { return hpc::view<T>(_ptr, _row_size); }

  template <class U> size_t distance_to(view_iterator<U> const &op) const {
    return ((size_t)(op._ptr - _ptr)) / _row_size;
  }

  void advance(size_t dist) { _ptr += dist * _row_size; }
