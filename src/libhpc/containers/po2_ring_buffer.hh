...
public:
    ...
    po2_ring_buffer(size_type size) { resize(size); }
    ...
    size_type insert(Iterator start, const Iterator& finish)
    {
    ...
    /// was consumed.
    ///
    size_t consume(size_type size = 1)
    {
    ...
    typename hpc::view<std::vector<value_type>> first_vacant_chunk() const
    {
    ...
    typename hpc::view<std::vector<value_type>> second_vacant_chunk() const
    {
        size_t start = norm(_start + _size);
        size_t size;
        if (start < _start)
    ...
    size_t norm(size_t idx) const { return idx & _mask; }

    bool empty() const { return _size == 0; }

    size_t size() const { return _size; }

    size_t max_size() const { return _buf.size(); }

    size_t vacant() const { return _buf.size() - _size; }

    value_type& operator[](size_type idx) { return _buf[norm(_start + idx)]; }

    const value_type& operator[](size_type idx) const { return _buf[norm(_start + idx)]; }

    friend std::ostream& operator<<(std::ostream& strm, const po2_ring_buffer& obj)
    {
        strm << "[";
        if (obj._size)
        {
...
template <class T>
class po2_ring_buffer_iterator
    : public boost::iterator_facade<po2_ring_buffer_iterator<T>, T, std::random_access_iterator_tag,
                                    const T>
{
    friend class boost::iterator_core_access;

public:
    typedef boost::iterator_facade<po2_ring_buffer_iterator<T>, T, std::random_access_iterator_tag,
                                   const T>
        super_type;
    typedef T value_type;
...
public:
    po2_ring_buffer_iterator(const po2_ring_buffer<value_type>& buffer, size_type idx)
        : _buf(buffer)
        , _idx(idx)
    {
    }
...
protected:
    ...
    void advance(size_t elems) { _idx += elems; }

    bool equal(const po2_ring_buffer_iterator& op) const
    {
        return _buf.norm(_idx) == _buf.norm(op._idx);
    }

    const value_type& dereference() const { return _buf[_idx]; }

    difference_type distance_to(const po2_ring_buffer_iterator& op) const { return op._idx - _idx; }
