...
template <class T, class Value> class range_map_insert_iterator;

///
///
///
template <class T, class Value>
class range_map : public std::map<range<T>, Value> {
public:
  ...
  insert_iterator insert(const range_type &range, const Value &value) {
...
class range_map_insert_iterator
    : public boost::iterator_facade<
          range_map_insert_iterator<T, Value>, std::pair<range<T>, Value &>,
          std::forward_iterator_tag, std::pair<range<T>, Value &>> {
public:
  range_map_insert_iterator(range_map<T, Value> &map, const range<T> &range,
                            const Value &value)
      : _map(map), _range(range), _value(value) {
    _begin();
  }

  bool done() { return !_cur_val; }
...
protected:
  void _begin() {
    ...
    if (_low == _map.end() || _low->first.start() >= _range.finish()) {
      // Call parent insert.
      typename range_map<T, Value>::const_iterator res =
          ((typename range_map<T, Value>::super_type &)_map)
              .insert(_range, _value);
  ...
  void _overlap() {
    ...
    T start, finish;
    if (_low->first.start() < _range.start() &&
        _low->first.finish() > _range.start()) {
      start = _range.start();
      ...
      _map.erase(_low);
      _low = ((typename range_map<T, Value>::super_type &)_map)
                 .insert(range<T>(orig_start, start), orig_val)
                 .first;

      typename range_map<T, Value>::iterator res =
          ((typename range_map<T, Value>::super_type &)_map)
              .insert(_cur_rng, _low->second);
      ASSERT(res.second, "Element should not have existed");
    }

    // Block in perfect overlap?
    else if (_low->first.start() == _range.start() &&
             _low->first.finish() == _range.start()) {
      _cur_rng = _low->first;
      _cur_val = &_low->second;
    }
    ...
    // Block in central overlap?
             ...
             _low->first.finish() <= _range.finish()) {
      _cur_rng = _low->first;
      _cur_val = &_low->second;
    }
    ...
    // Block in rear overlap?
      ...
      _map.erase(_low);
      _low = ((typename range_map<T, Value>::super_type &)_map)
                 .insert(range<T>(finish, orig_finish), orig_val)
                 .first;
    ...
    // Block in total overlap?
      ...
      _map.erase(_low);
      _low = ((typename range_map<T, Value>::super_type &)_map)
                 .insert(range<T>(orig_start, _range.start()), orig_val)
                 .first;
      ...
      _cur_rng = _range;
      _cur_val = &_low->second;
      res = ((typename range_map<T, Value>::super_type &)_map)
                .insert(_cur_rng, _low->second);
      ASSERT(res.second, "Element should not have existed");
    }
  }
  ...
  bool equal(const range_map_insert_iterator &op) const {
    return _cur_val == op._cur_val;
  }

  std::pair<range<T>, Value &> dereference() const {
