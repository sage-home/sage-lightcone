...
public:
  csr(index num_rows = 0) : _num_rows(0) { this->num_rows(num_rows); }

  csr(const csr &csr) { this->duplicate(csr); }
  ...
  bool empty() const { return !this->_num_rows; }

  template <class Iter, class Map, class Predicate>
  void setup_3phase(index num_rows, Iter begin, Iter end, Map op,
    ...
    std::reverse_copy(this->_displs.begin(), this->_displs.end() - 1,
                      this->_displs.rbegin());
    this->_displs[0] = 0;
  }

  index num_rows() const { return this->_num_rows; }

  void num_rows(index num_rows) {
    this->deallocate();
  ...
  template <class Iter> void copy_counts(Iter cnts, index size) {
    this->num_rows(size);

    // Need to add one to the size to ensure we have enough room for
    // displacements. However, we can't do this if there are no counts.
    if (size)
      ++size;
  ...
  template <class Iter> void copy_displs(Iter displs, index size) {
    if (size > 1) {
  ...
  void take_displs(vector<index> &displs) {
    this->deallocate();
    if (displs.size() > 1) {
  ...
  void take_array(vector<T> &array) {
  ...
  void setup_array(bool with_counts = false) {
    if (with_counts)
  ...
  template <class MultiMap, class RowMap>
  void set_from_multimap(const MultiMap &entry_map, const RowMap &row_map) {
    typedef typename MultiMap::const_iterator multimap_citer;

    // Set the number of rows to the same size as the number of entries in the
    // row map.
    ...
    // Insert each element. Note that there is no need to increment the iterator
    // as the upper_bound operation will do this.
    for (multimap_citer it = entry_map.begin(); it != entry_map.end();) {
      multimap_citer up = entry_map.upper_bound(it->first);
      boost::copy(boost::make_iterator_range(it, up) |
  ...
  void take(vector<index> &displs, vector<T> &array) {
    if (displs.size()) {
  ...
  typename vector<index>::view counts() {
    return vector<index>::view(this->_displs, this->_num_rows);
  }

  void counts(vector<index> &cnts) const {
    cnts.resize(this->_num_rows);
    displs_to_counts(this->_displs.begin(), this->_displs.end(), cnts.begin());
  }

  const vector<index> &displs() const { return this->_displs; }

  vector<index> &mod_displs() { return this->_displs; }

  const vector<T> &array() const { return this->_array; }

  vector<T> &mod_array() { return this->_array; }

  index row_size(index row) const {
    return this->_displs[row + 1] - this->_displs[row];
  }

  void row_sizes(vector<index> &sizes) const {
    sizes.resize(this->_num_rows);
    for (index ii = 0; ii < this->_num_rows; ++ii)
      sizes[ii] = this->_displs[ii + 1] - this->_displs[ii];
  }

  const typename vector<T>::view operator[](index row) const {
    return typename vector<T>::view(this->_array, this->row_size(row),
                                    this->_displs[row]);
  }

  typename vector<T>::view operator[](index row) {
    return typename vector<T>::view(this->_array, this->row_size(row),
                                    this->_displs[row]);
  }
  ...
  bool operator==(const csr &op) const {
  ...
  bool operator!=(const csr &op) const { return !(*this == op); }

  friend std::ostream &operator<<(std::ostream &strm, const csr<T> &obj) {
...
template <class Displ, class T, class Iter, class Map, class Predicate>
void csr_3phase(index num_rows, vector<Displ> &displs, vector<T> &array,
                Iter begin, Iter end, Map mapping,
                Predicate pred = always<typename Iter::value_type>()) {
  // Begin from scratch.
  displs.deallocate();
  array.deallocate();
...
template <class Displ, class T, class Index, class Iter, class Map,
          class Predicate>
void csr_3phase(index num_rows, vector<Displ> &displs, vector<T> &array,
                vector<Index> &indices, Iter begin, Iter end, Map mapping,
                Predicate pred = always<typename Iter::value_type>()) {
  // Begin from scratch.
  displs.deallocate();
  array.deallocate();
