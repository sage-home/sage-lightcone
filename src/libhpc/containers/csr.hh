...
namespace hpc {
    ...
    template <class T> class csr {
      public:
        csr(index num_rows = 0) : _num_rows(0) {
        ...
        void setup_3phase(index     num_rows,
                          Iter      begin,
                          Iter      end,
                          Map       op,
        ...
        void take_array(vector<T> &array) {
            ASSERT((!this->_num_rows && !array.size()) || array.size() == this->_displs[this->_num_rows],
                   "Invalid array size.");
        ...
        void take(vector<index> &displs, vector<T> &array) {
            if(displs.size()) {
                ASSERT(displs[displs.size() - 1] == array.size(), "Cannot setup a CSR from mismatching array sizes.");
                this->_num_rows = displs.size() - 1;
                this->_displs.take(displs);
                this->_array.take(array);
            } else
        ...
        const vector<index> &displs() const {
            return this->_displs;
        }

        vector<index> &mod_displs() {
            return this->_displs;
        }
    ...
    void csr_3phase(index          num_rows,
                    vector<Displ> &displs,
                    vector<T> &    array,
                    Iter           begin,
                    Iter           end,
