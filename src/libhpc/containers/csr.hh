...
public:
    ...
    template <class Iter>
    void copy_counts(Iter cnts, index size)
    {
        this->num_rows(size);
    ...
    template <class Iter>
    void copy_displs(Iter displs, index size)
    {
        if (size > 1)
        {
    ...
    index row_size(index row) const { return this->_displs[row + 1] - this->_displs[row]; }
...
void csr_3phase(index num_rows, vector<Displ>& displs, vector<T>& array, Iter begin, Iter end,
                Map mapping, Predicate pred = always<typename Iter::value_type>())
{
    // Begin from scratch.
    displs.deallocate();
    array.deallocate();
...
template <class Displ, class T, class Index, class Iter, class Map, class Predicate>
void csr_3phase(index num_rows, vector<Displ>& displs, vector<T>& array, vector<Index>& indices,
