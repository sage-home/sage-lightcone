...
#ifndef hpc_containers_range_multimap_hh
#define hpc_containers_range_multimap_hh
...
class range_multimap : public map<range<T>, set<Value>>
{
public:
    ...
    void insert(const range_type& range, const Value& value)
    {
        ...
        //       if( cur == low )
        //       {
        //          // Only take the existing mapped value if the existing range
        //          begins
        //          // before the new range.
        //          if( cur->first.start() <= range.start() )
        //             new_ranges.push_back( std::make_pair( split_ranges[0],
        //             cur->second ) );
        //          else
        //             new_ranges.push_back( std::make_pair( split_ranges[0],
        //             mapped_type() ) );
        //          if( split_ranges.size() > 0 && cur->first.start() !=
        //          range.start() )
        //             new_ranges.push_back( std::make_pair( split_ranges[1],
        //             cur->second ) );
        //       }
        ...
        //       if( cur->first.finish() > range.finish() )
        //       {
        //          new_ranges.push_back( std::make_pair( split_ranges.back(),
        //          cur->second ) );
        //       }
        //       else
        //          new_ranges.push_back( std::make_pair( split_ranges.back(),
        ...
        //    {
        //       if( elem.first.start() < range.finish() && elem.first.finish() >
        //       range.start() )
...
///
template <class T, class Value>
void invert_mapping(const range_multimap<T, Value>& mapping, map<Value, set<range<T>>>& inv)
{
