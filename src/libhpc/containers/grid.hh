...
public:
  ...
  template <class InputIterator> grid(unsigned dim, InputIterator first) {
    this->setup(dim, first);
  }

  ~grid() {}

  void resize(unsigned dim, T sides) { setup_block(dim, sides); }

  void setup_block(unsigned dim, T sides) {
    reallocate(_sides, dim);
    std::fill(_sides.begin(), _sides.end(), sides);
    this->_setup_basis();
  }

  template <class InputIterator> void setup(unsigned dim, InputIterator first) {
    reallocate(_sides, dim);
  ...
  unsigned dimension() const { return this->_sides.size(); }

  T size() const { return this->_size; }

  template <class InputIterator> T project(InputIterator first) const {
    T idx = 0;
  ...
  void lift(T index, OutputIterator result) const {
