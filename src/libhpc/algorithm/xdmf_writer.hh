...
void write_xdmf_xml_begin(std::ofstream& xml_file);

void write_xdmf_close_grid_xml(std::ofstream& xml_file);

void write_xdmf_xml_end(std::ofstream& xml_file);

void write_xdmf_xml_domain_begin(std::ofstream& xml_file);

void write_xdmf_xml_domain_end(std::ofstream& xml_file);

void write_xdmf_xml_grid_begin(std::ofstream& xml_file, const std::string& name);

void write_xdmf_xml_topology(std::ofstream& xml_file, const std::string& h5_filename,
                             const std::string& grid, int dim, unsigned num_global_cells,
                             unsigned num_cell_points);

void write_xdmf_xml_geometry(std::ofstream& xml_file, const std::string& h5_filename,
                             const std::string& grid, int dim, unsigned num_global_cells,
                             unsigned num_cell_points);

template <class IterT>
void write_xdmf_xml_field(std::ofstream& xml_file, const std::string& h5_filename,
                          const std::string& grid, const std::string& name, IterT const& begin,
                          IterT const& end, mpi::comm const& comm = mpi::comm::world);

template <class IterT>
void write_xdmf_h5_field(h5::file& h5_file, const std::string& name, IterT const& begin,
                         IterT const& end, mpi::comm const& comm = mpi::comm::world);

void write_xdmf_open_grid_xml(std::ofstream& xml_file, const std::string& h5_filename,
                              const std::string& name, const kdtree<>& kdt);

template <class IterT>
void write_xdmf_xml_open_particles(std::ofstream& xml_file, const std::string& h5_filename,
                                   const std::string& name, IterT const& begin, IterT const& end,
                                   mpi::comm const& comm)
{
...
template <class IterT>
void write_xdmf_h5_particles(h5::file& h5_file, const std::string& grid, IterT const& begin,
                             IterT const& end, mpi::comm const& comm)
{
    ...
    unsigned num_cell_points = 1;
    {
        ...
        for (unsigned ii = global_offs; ii < global_offs + num_local_cells; ++ii)
        {
            ...
            topology_set.write(buffer.data(), h5::datatype::native_uint, mem_space, file_space,
                               comm);
        }
    }
    {
        ...
        for (unsigned ii = global_offs; ii < global_offs + num_local_cells; ++ii, ++it)
        {
            ...
            file_space.select_hyperslab<std::vector<hsize_t>>(H5S_SELECT_SET, count, start);
            geometry_set.write(buffer.data(), h5::datatype::native_double, mem_space, file_space,
                               comm);
        }
    }
    // {
    ...
    //    h5::dataspace file_space( space_dims );
    //    h5::dataset topology_set;
    //    topology_set.create( h5_file, "ownership", h5::datatype::native_int,
    //    file_space ); space_dims[0] = 1; h5::dataspace mem_space( space_dims );
    //    mem_space.select_all();
    ...
    //    ) {
    //       start[0] = ii;
    //       file_space.select_hyperslab<std::vector<hsize_t> >( H5S_SELECT_SET,
    //       count, start ); topology_set.write( &buffer,
    //       h5::datatype::native_int, mem_space, file_space, comm );
    //    }
    // }
}
...
public:
    ...
    xdmf_writer(const std::string& base_filename, mpi::comm const& comm = mpi::comm::world);

    ~xdmf_writer();

    void open(const std::string& base_filename, mpi::comm const& comm = mpi::comm::world);

    void open_grid(const std::string& name, const kdtree<>& kdt);

    template <class IterT>
    void open_grid(std::string const& name, IterT const& begin, IterT const& end,
    ...
    template <class IterT>
    void write_field(const std::string& name, IterT const& begin, IterT const& end)
    {
...
void write_xdmf(const std::string& base_filename, const std::string& name, const kdtree<>& kdt);

void write_xdmf_open_grid_h5(h5::file& h5_file, const std::string& grid, const kdtree<>& kdt);
...
void write_xdmf_open_xml(std::ofstream& xml_file, const std::string& h5_filename);

void write_xdmf_xml_close(std::ofstream& xml_file);

void write_xdmf_xml_attribute(std::ofstream& xml_file, const std::string& name,
                              const std::string& h5_filename, const std::string& h5_path,
                              const std::string& type, unsigned dim, unsigned num_global_cells,
...
{
    typedef typename IterT::value_type::value_type value_type;
    typedef mpl::map<mpl::pair<int, mpl::int_<0>>, mpl::pair<unsigned, mpl::int_<1>>,
                     mpl::pair<long, mpl::int_<2>>, mpl::pair<unsigned long, mpl::int_<3>>,
                     mpl::pair<float, mpl::int_<4>>, mpl::pair<double, mpl::int_<5>>>
        type_map;
