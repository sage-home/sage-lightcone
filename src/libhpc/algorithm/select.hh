...
struct select_function
{
    ...
    {
        long sum_left =
            std::count_if(start, finish, [x](const value_type& val) { return val <= x; });
...
typename Iterator::value_type select(Iterator const& start, Iterator const& finish, long position,
                                     mpi::comm const& comm = mpi::comm::world)
{
    ...
    value_type x1 =
        comm.all_reduce(*minmax.first, MPI_MIN) - std::numeric_limits<value_type>::epsilon();
    value_type x2 =
        comm.all_reduce(*minmax.second, MPI_MAX) + std::numeric_limits<value_type>::epsilon();
