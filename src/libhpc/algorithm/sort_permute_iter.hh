...
namespace hpc {
    ...
    template <class... Args> struct tuple_wrapper {
        ...
        tuple_wrapper(tuple_wrapper<typename std::add_lvalue_reference<Args>::type...> const &src) : data{src.data} {
        }

        template <class... SubArgs> tuple_wrapper &operator=(tuple_wrapper<SubArgs...> const &src) {
            data = src.data;
            return *this;
        }
    ...
    template <class... Args> void swap(tuple_wrapper<Args...> x, tuple_wrapper<Args...> y) {
        std::swap(x.data, y.data);
    }

    template <class SortIter, class PermuteIter> struct sort_permute_iter_helper_type {
    ...
    class sort_permute_iter
      ...
      protected:
        ...
        typename sort_permute_iter_helper_type<SortIter, PermuteIter>::reference_type dereference() const {
            return
                typename sort_permute_iter_helper_type<SortIter, PermuteIter>::reference_type(*this->_si, *this->_pi);
        }
