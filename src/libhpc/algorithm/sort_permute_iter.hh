...
template <class... Args> struct tuple_wrapper {
  std::tuple<Args...> data;

  tuple_wrapper(Args... src) : data{src...} {}

  tuple_wrapper(std::tuple<Args...> const &src) : data{src} {}

  tuple_wrapper(
      tuple_wrapper<typename std::add_lvalue_reference<Args>::type...> const
          &src)
      : data{src.data} {}

  template <class... SubArgs>
  tuple_wrapper &operator=(tuple_wrapper<SubArgs...> const &src) {
    data = src.data;
    return *this;
  }
...
template <class... Args>
void swap(tuple_wrapper<Args...> x, tuple_wrapper<Args...> y) {
...
class sort_permute_iter
    : public boost::iterator_facade<
          ...
          typename sort_permute_iter_helper_type<SortIter,
                                                 PermuteIter>::value_type,
          std::random_access_iterator_tag,
          typename sort_permute_iter_helper_type<SortIter,
                                                 PermuteIter>::reference_type,
          typename std::iterator_traits<SortIter>::difference_type> {
  friend class boost::iterator_core_access;

public:
  typedef
      typename std::iterator_traits<SortIter>::difference_type difference_type;

  sort_permute_iter() {}

  sort_permute_iter(SortIter si, PermuteIter pi) : _si(si), _pi(pi) {}
...
protected:
  ...
  dereference() const {
    return typename sort_permute_iter_helper_type<
        SortIter, PermuteIter>::reference_type(*this->_si, *this->_pi);
  }

  void advance(difference_type ii) {
    this->_si += ii;
    this->_pi += ii;
  }
...
template <class SortIter, class PermuteIter,
          class Comparison =
              std::less<typename std::iterator_traits<SortIter>::value_type>>
...
template <class SortIter, class PermuteIter>
sort_permute_iter<SortIter, PermuteIter>
make_sort_permute_iter(SortIter si, PermuteIter pi) {
