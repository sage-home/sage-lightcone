...
namespace hpc {
    ...
    template <class CoordT = double> class kdtree {
      ...
      public:
        ...
        template <class PartT> kdtree(PartT &part, mpi::comm const &comm = mpi::comm::world) : _comm(&comm) {
            construct(part);
        }
        ...
        unsigned n_leafs() const {
            return _splits.size() + 1;
        }

        unsigned n_branches() const {
            return _splits.size();
        }
        ...
        std::vector<split_type> const &splits() const {
            return _splits;
        }
        ...
        unsigned depth(unsigned cell) const {
            return log2i(cell + 1);
        }
      ...
      protected:
        ...
        template <class U> friend h5::location const &operator>>(h5::location const &loc, kdtree<U> &kdt);
    };
    ...
    template <class CoordT> class kdtree_iterator {
      ...
      public:
        ...
        bool operator==(kdtree_iterator const &op) const {
    ...
    template <class CoordT> h5::location &operator<<(h5::location &loc, kdtree<CoordT> const &kdt) {
        // Write bounds.
        {
    ...
    template <class CoordT> h5::location const &operator>>(h5::location const &loc, kdtree<CoordT> &kdt) {
        kdt.clear();
