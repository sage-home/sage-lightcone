...
public:
    ...
    kdtree(PartT& part, mpi::comm const& comm = mpi::comm::world)
        : _comm(&comm)
    {
        construct(part);
    }
    ...
    template <class PartT>
    void _construct_serial_recursive(PartT& part, unsigned cell,
    ...
    void leaf_geometry(unsigned leaf, IterT result) const
    {
...
protected:
    ...
    template <class U>
    friend h5::location const& operator>>(h5::location const& loc, kdtree<U>& kdt);
};
...
public:
    ...
    kdtree_iterator(kdtree_type const& kdt, unsigned cell = std::numeric_limits<unsigned>::max())
        : _kdt(&kdt)
        , _geom(kdt.n_dims())
        , _buf(kdt.max_depth())
    {
...
h5::location& operator<<(h5::location& loc, kdtree<CoordT> const& kdt)
{
    ...
    // Write splits.
    {
        ...
        der.add2(h5::datatype::native_double, HOFFSET(typename kdtree<CoordT>::split_type, pos),
                 "position");
        der.add2(h5::datatype::native_uint, HOFFSET(typename kdtree<CoordT>::split_type, dim),
                 "dimension");
        h5::datatype mdt, fdt;
        der.commit(mdt, fdt);
