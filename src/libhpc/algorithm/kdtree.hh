...
template <class CoordT> class kdtree_iterator;

template <class CoordT = double> class kdtree {
public:
...
public:
  kdtree(mpi::comm const &comm = mpi::comm::world) : _comm(&comm) {}

  template <class PartT>
  kdtree(PartT &part, mpi::comm const &comm = mpi::comm::world) : _comm(&comm) {
    construct(part);
  }
  ...
  unsigned parent(unsigned cell) const {
    ASSERT(cell > 0);
    return (cell - 1) >> 1;
  }

  unsigned n_dims() const { return _bnds.size(); }

  unsigned n_leafs() const { return _splits.size() + 1; }

  unsigned n_branches() const { return _splits.size(); }

  unsigned n_cells() const { return n_leafs() + n_branches(); }

  bool is_leaf(unsigned cell) const {
    ASSERT(cell < n_cells());
  ...
  std::vector<split_type> const &splits() const { return _splits; }

  template <class IterT> void leaf_geometry(unsigned leaf, IterT result) const {
  ...
  unsigned leaf_to_cell(unsigned leaf) const {
  ...
  unsigned depth(unsigned cell) const { return log2i(cell + 1); }
...
protected:
  ...
  template <class U>
  friend h5::location const &operator>>(h5::location const &loc,
                                        kdtree<U> &kdt);
};
...
public:
  ...
  unsigned cell() const { return _cell; }

  std::vector<std::array<coord_type, 2>> const &bounds() const { return _geom; }

  bool is_leaf() const { return _kdt->is_leaf(_cell); }

  bool operator==(kdtree_iterator const &op) const { return _cell == op._cell; }
...
h5::location &operator<<(h5::location &loc, kdtree<CoordT> const &kdt) {
  // Write bounds.
  {
  ...
  // Write splits.
  {
    ...
    der.add2(h5::datatype::native_double,
             HOFFSET(typename kdtree<CoordT>::split_type, pos), "position");
    der.add2(h5::datatype::native_uint,
             HOFFSET(typename kdtree<CoordT>::split_type, dim), "dimension");
    h5::datatype mdt, fdt;
    der.commit(mdt, fdt);
...
template <class CoordT>
h5::location const &operator>>(h5::location const &loc, kdtree<CoordT> &kdt) {
  kdt.clear();
