...
void iota(Iterator first, const Iterator& last, typename Iterator::value_type init)
{
    while (first != last)
        *first++ = init++;
}
...
void iota(Sequence& seq, Value init)
{
...
///
template <class Iterator>
std::pair<Iterator, Iterator> minmax_element(Iterator start, const Iterator& finish)
{
...
struct element : public std::unary_function<const Array&, typename Array::value_type>
{
...
struct element_tuple
    : public std::unary_function<const Tuple&, typename boost::tuples::element<Index, Tuple>>
{
    ...
    value_type operator()(const argument_type& tuple) const { return boost::get<Index>(tuple); }
};
...
class binary_expand : public std::unary_function<typename Operation::first_argument_type,
...
template <class T>
struct less_first : public std::less<T>
{
    ...
    bool operator()(const T& op_a, const T& op_b) const { return op_a.first < op_b.first; }
};
