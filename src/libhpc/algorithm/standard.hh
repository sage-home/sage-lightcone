...
namespace hpc {
    ...
    ///
    ///
    template <class Iterator> void iota(Iterator first, const Iterator &last, typename Iterator::value_type init) {
        while(first != last)
            *first++ = init++;
    }
    ...
    ///
    ///
    template <class Iterator> std::pair<Iterator, Iterator> minmax_element(Iterator start, const Iterator &finish) {
    ...
    ///
    template <class Map>
    struct map_derefer : public std::unary_function<typename Map::key_type, typename Map::mapped_type> {
        typedef typename Map::key_type    key_type;
        typedef typename Map::mapped_type mapped_type;
        typedef typename Map::value_type  value_type;
        ...
        typename Map::mapped_type operator()(const typename Map::key_type &key) const {
    ...
    ///
    ///
    template <class Array> struct element : public std::unary_function<const Array &, typename Array::value_type> {
        typedef index                      key_type;
        typedef typename Array::value_type mapped_type;
        typedef typename Array::value_type value_type;
        typedef typename Array::value_type result_type;

        element(index idx = 0) : _idx(idx) {
        }

        value_type operator()(const Array &array) const {
    ...
    struct element_tuple : public std::unary_function<const Tuple &, typename boost::tuples::element<Index, Tuple>> {
    ...
    class binary_compose : public std::binary_function<typename Operation2::argument_type,
    ...
    template <class Operation> inline binary_expand<Operation> expand2(const Operation &op) {
