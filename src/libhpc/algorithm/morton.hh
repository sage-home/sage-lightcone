...
template <int D>
CUDA_DEV_HOST uint32_t dilate(uint16_t x);

template <int D>
CUDA_DEV_HOST uint16_t undilate(uint32_t);

template <>
CUDA_DEV_HOST_INL uint32_t dilate<2>(uint16_t t)
{
    uint32_t r = t;
    r = (r | (r << 8)) & 0x00FF00FF;
    r = (r | (r << 4)) & 0x0F0F0F0F;
...
template <>
CUDA_DEV_HOST_INL uint16_t undilate<2>(uint32_t t)
{
    t = (t * 3) & 0x66666666;
    t = (t * 5) & 0x78787878;
    t = (t * 17) & 0x7F807F80;
...
template <>
CUDA_DEV_HOST_INL uint32_t dilate<3>(uint16_t t)
{
    uint32_t r = t;
...
template <>
CUDA_DEV_HOST_INL uint16_t undilate<3>(uint32_t t)
{
...
template <int D>
CUDA_DEV_HOST_INL uint32_t morton(uint16_t x, uint16_t y)
{
...
template <int D>
boost::array<uint16_t, D> unmorton(uint32_t idx);
