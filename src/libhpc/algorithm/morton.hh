...
namespace hpc {
    ...
    template <int D, int N, class First, class... Args> struct morton_impl<D, N, First, Args...> {
    ...
    template <int D> CUDA_DEV_HOST_INL uint32_t morton(uint16_t x, uint16_t y, uint16_t z) {
    ...
    template <> CUDA_DEV_HOST_INL boost::array<uint16_t, 2> unmorton<2>(uint32_t idx) {
#ifdef CXX_0X
        return boost::array<uint16_t, 2>{undilate<2>(idx & BOOST_BINARY(01010101 01010101 01010101 01010101)),
    ...
    template <> CUDA_DEV_HOST_INL boost::array<uint16_t, 3> unmorton<3>(uint32_t idx) {
#ifdef CXX_0X
        return boost::array<uint16_t, 3>{undilate<3>(idx & BOOST_BINARY(01001001 00100100 10010010 01001001)),
