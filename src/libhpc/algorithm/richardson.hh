...
CUDA_DEV_HOST T richardson(FuncT func, T const& x1, T const& x2, T x,
                           T const& tol = default_richardson_tolerance,
                           unsigned max_its = default_richardson_max_its, T omega = 1.0)
{
    ...
    for (ii = 0; ii < max_its; ++ii)
    {
        ...
        ASSERT((x1 - x) * (x - x2) >= 0.0, "Function moved outside bracketed range.");
