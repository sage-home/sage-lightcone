...
template <class IterT> class median_iterator {
public:
  typedef IterT iterator_type;
  typedef typename iterator_type::value_type median_type;

public:
  median_iterator(iterator_type it, median_type med = 0) : _it(it), _med(med) {}

  bool operator!=(median_iterator const &other) const {
    return _it != other._it;
  }

  int operator*() const { return (*_it <= _med) ? 0 : 1; }
...
make_median_iterator(IterT it, typename IterT::value_type med = 0) {
...
template <class DimIterT, class PermuteT> class binary_partitioner {
public:
  typedef DimIterT dim_iter_type;
  typedef PermuteT permute_type;
  typedef typename std::iterator_traits<dim_iter_type>::value_type::iterator
      coord_iter_type;
...
public:
  ...
  binary_partitioner(dim_iter_type dims_begin, dim_iter_type dims_end,
                     permute_type &perm, unsigned ppc = 1000,
                     mpi::comm const &comm = mpi::comm::world) {
    construct(dims_begin, dims_end, perm, ppc, comm);
  }

  binary_partitioner(binary_partitioner &&src)
      : _dims_begin(src._dims_begin), _dims_end(src._dims_end),
        _perm(src._perm), _max_ppc(src._max_ppc), _depth(src._depth),
        _lsize(src._lsize), _gsize(src._gsize),
        _n_leaf_cells(src._n_leaf_cells), _n_cells(src._n_cells),
        _n_inc_leaf_cells(src._n_inc_leaf_cells), _bnds(std::move(src._bnds)),
        _min(std::move(src._min)), _max(std::move(src._max)),
        _offs(std::move(src._offs)), _cnts(std::move(src._cnts)),
        _split(src._split), _split_and_side(src._split_and_side),
        _sub_size(src._sub_size), _sub_comm(src._sub_comm) {}

  binary_partitioner &operator=(binary_partitioner &&src) {
  ...
  void construct(dim_iter_type dims_begin, dim_iter_type dims_end,
                 permute_type &perm, unsigned ppc = 1000,
                 mpi::comm const &comm = mpi::comm::world) {
  ...
  unsigned n_elems() const { return _lsize; }

  unsigned n_gelems() const { return _gsize; }

  unsigned n_cells() const { return _n_cells; }

  unsigned n_leaf_cells() const { return _n_leaf_cells; }

  unsigned n_branch_cells() const { return _n_cells - _n_leaf_cells; }

  unsigned n_incomplete_leaf_cells() const { return _n_inc_leaf_cells; }

  unsigned depth() const { return _depth; }

  bool need_refinement(unsigned cell) const {
    ASSERT(cell < _n_cells);
    return _cnts[cell] > _max_ppc;
  }

  void partition(unsigned cell,
    ...
    if (comm.size() > 1) {
      ...
      part.split(make_median_iterator(crd_begin, med),
                 make_median_iterator(crd_end));
      ASSERT(part.left_size() == left_size,
             "Selection value too innacurate: ", part.left_size(),
             " != ", left_size);
      ...
      part.connect(make_median_iterator(crd_begin, med),
                   make_median_iterator(crd_end));

      // Stash results.
      _sub_comm = part.sub_comm();
      _split = std::tuple<coord_type, unsigned>(med, dim);
      _split_and_side = std::tuple<coord_type, unsigned, int>(
          med, dim, part.collecting_left() ? 0 : 1);
      _sub_size = sub_size;
      ...
      LOGDLN("Particles in cell:  ", crd_end - crd_begin);
      LOGDLN("Splitting into:     ", left_size, " -- ",
             (crd_end - crd_begin) - left_size);
      LOGDLN("Chosen dimension:   ", dim);
      LOGDLN("Chosen median:      ", med);
      ...
      unsigned on_left = (crd_end - crd_begin) -
                         std::accumulate(make_median_iterator(crd_begin, med),
                                         make_median_iterator(crd_end), 0);
      LOGDLN("Initially found ", on_left, " on the left.");
      if (on_left == left_size)
        part.construct(make_median_iterator(crd_begin, med),
                       make_median_iterator(crd_end), _offs[cell]);
      else {
  ...
  mpi::comm const &sub_comm() const { return _sub_comm; }

  std::tuple<coord_type, unsigned> const &split() const { return _split; }

  std::tuple<coord_type, unsigned, int> const &split_and_side() const {
    return _split_and_side;
  }

  std::vector<unsigned> const &offsets() const { return _offs; }

  std::vector<unsigned> const &counts() const { return _cnts; }

  std::vector<std::array<coord_type, 2>> const &bounds() const { return _bnds; }

  unsigned left_child(unsigned cell) const { return (cell + 1) * 2 - 1; }

  unsigned right_child(unsigned cell) const { return (cell + 1) * 2; }

  std::array<unsigned, 2> lowest_range(unsigned cell) const {
    if (cell > 0) {
      ...
      return std::array<unsigned, 2>{0, static_cast<unsigned>(1 << _depth)};
  }

  std::array<unsigned, 2> leaf_cell_range(unsigned cell) const {
  ...
  unsigned n_leaf_cells(unsigned cell) const {
    auto rng = leaf_cell_range(cell);
    return rng[1] - rng[0];
  }

  unsigned n_elems(unsigned cell) const {
    auto rng = leaf_cell_range(cell);
...
protected:
  unsigned _choose_dim(std::vector<std::array<coord_type, 2>> bnds) {
...
template <class DimIterT, class PermuteT>
binary_partitioner<DimIterT, PermuteT>
make_binary_partitioner(DimIterT dims_begin, DimIterT dims_end, PermuteT &perm,
                        unsigned ppc = 1000,
                        mpi::comm const &comm = mpi::comm::world) {
  return binary_partitioner<DimIterT, PermuteT>(dims_begin, dims_end, perm, ppc,
                                                comm);
}
