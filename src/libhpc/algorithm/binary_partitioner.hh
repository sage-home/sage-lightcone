...
public:
    median_iterator(iterator_type it, median_type med = 0)
        : _it(it)
        , _med(med)
    {
    }

    bool operator!=(median_iterator const& other) const { return _it != other._it; }
...
template <class IterT>
median_iterator<IterT> make_median_iterator(IterT it, typename IterT::value_type med = 0)
{
...
public:
    ...
    binary_partitioner(dim_iter_type dims_begin, dim_iter_type dims_end, permute_type& perm,
                       unsigned ppc = 1000, mpi::comm const& comm = mpi::comm::world)
    {
    ...
    void construct(dim_iter_type dims_begin, dim_iter_type dims_end, permute_type& perm,
                   unsigned ppc = 1000, mpi::comm const& comm = mpi::comm::world)
    {
    ...
    void partition(unsigned cell, std::vector<std::array<coord_type, 2>> const& bnds,
                   mpi::comm const& comm)
    {
        ...
        if (comm.size() > 1)
        {
            ...
            ASSERT(part.left_size() == left_size,
                   "Selection value too innacurate: ", part.left_size(), " != ", left_size);
            ...
            _split_and_side =
                std::tuple<coord_type, unsigned, int>(med, dim, part.collecting_left() ? 0 : 1);
            _sub_size = sub_size;
        ...
        else
        {
            ...
            unsigned on_left =
                (crd_end - crd_begin) - std::accumulate(make_median_iterator(crd_begin, med),
    ...
    std::tuple<coord_type, unsigned, int> const& split_and_side() const { return _split_and_side; }
...
protected:
    unsigned _choose_dim(std::vector<std::array<coord_type, 2>> bnds)
    {
        ...
        auto res = std::max_element(
            bnds.begin(), bnds.end(),
            [](std::array<coord_type, 2> const& x, std::array<coord_type, 2> const& y) {
...
make_binary_partitioner(DimIterT dims_begin, DimIterT dims_end, PermuteT& perm, unsigned ppc = 1000,
