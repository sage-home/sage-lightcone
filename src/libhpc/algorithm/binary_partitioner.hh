...
namespace hpc {
    ...
    template <class IterT> class median_iterator {
      ...
      public:
        ...
        typename iterator_type::difference_type operator-(median_iterator const &other) const {
            return _it - other._it;
        }
    ...
    template <class IterT> median_iterator<IterT> make_median_iterator(IterT it, typename IterT::value_type med = 0) {
    ...
    template <class DimIterT, class PermuteT> class binary_partitioner {
      ...
      public:
        ...
        binary_partitioner(binary_partitioner &&src) :
            _dims_begin(src._dims_begin), _dims_end(src._dims_end), _perm(src._perm), _max_ppc(src._max_ppc),
            _depth(src._depth), _lsize(src._lsize), _gsize(src._gsize), _n_leaf_cells(src._n_leaf_cells),
            _n_cells(src._n_cells), _n_inc_leaf_cells(src._n_inc_leaf_cells), _bnds(std::move(src._bnds)),
            _min(std::move(src._min)), _max(std::move(src._max)), _offs(std::move(src._offs)),
            _cnts(std::move(src._cnts)), _split(src._split), _split_and_side(src._split_and_side),
        ...
        void construct(dim_iter_type    dims_begin,
                       dim_iter_type    dims_end,
                       permute_type &   perm,
                       unsigned         ppc  = 1000,
                       mpi::comm const &comm = mpi::comm::world) {
        ...
        unsigned n_elems() const {
            return _lsize;
        }

        unsigned n_gelems() const {
            return _gsize;
        }
        ...
        unsigned n_leaf_cells() const {
            return _n_leaf_cells;
        }

        unsigned n_branch_cells() const {
            return _n_cells - _n_leaf_cells;
        }

        unsigned n_incomplete_leaf_cells() const {
        ...
        void partition(unsigned cell, std::vector<std::array<coord_type, 2>> const &bnds, mpi::comm const &comm) {
            ...
            if(comm.size() > 1) {
                ...
                ASSERT(part.left_size() == left_size,
                       "Selection value too innacurate: ",
                       part.left_size(),
                       " != ",
                       left_size);
                ...
                ASSERT(_offs[cell] != std::numeric_limits<unsigned>::max(), "Out-of-order cell passed to partition.");
                ASSERT(_cnts[cell] != std::numeric_limits<unsigned>::max(), "Out-of-order cell passed to partition.");
                ...
                unsigned                on_left =
                    (crd_end - crd_begin) -
                    std::accumulate(make_median_iterator(crd_begin, med), make_median_iterator(crd_end), 0);
        ...
        mpi::comm const &sub_comm() const {
            return _sub_comm;
        }

        std::tuple<coord_type, unsigned> const &split() const {
            return _split;
        }
        ...
        std::vector<unsigned> const &offsets() const {
            return _offs;
        }
        ...
        std::vector<std::array<coord_type, 2>> const &bounds() const {
            return _bnds;
        }

        unsigned left_child(unsigned cell) const {
            return (cell + 1) * 2 - 1;
        }

        unsigned right_child(unsigned cell) const {
            return (cell + 1) * 2;
        }
      ...
      protected:
        unsigned _choose_dim(std::vector<std::array<coord_type, 2>> bnds) {
            ...
            auto res = std::max_element(
                bnds.begin(), bnds.end(), [](std::array<coord_type, 2> const &x, std::array<coord_type, 2> const &y) {
    ...
    binary_partitioner<DimIterT, PermuteT> make_binary_partitioner(DimIterT         dims_begin,
                                                                   DimIterT         dims_end,
                                                                   PermuteT &       perm,
                                                                   unsigned         ppc  = 1000,
                                                                   mpi::comm const &comm = mpi::comm::world) {
