...
xdmf_writer::xdmf_writer(const std::string& base_filename, mpi::comm const& comm)
{
    this->open(base_filename, comm);
}

xdmf_writer::~xdmf_writer() { close(); }

void xdmf_writer::open(const std::string& base_filename, mpi::comm const& comm)
{
    std::string xml_filename = base_filename + ".xmf";
    this->_h5_filename = base_filename + ".h5";
    this->_xml_file.open(xml_filename, std::ios::out);
    write_xdmf_open_xml(this->_xml_file, this->_h5_filename);
    this->_h5_file.open(this->_h5_filename, H5F_ACC_TRUNC, comm);
}

void xdmf_writer::open_grid(const std::string& name, const kdtree<>& kdt)
{
    _grid = name;
    write_xdmf_open_grid_xml(this->_xml_file, this->_h5_filename, name, kdt);
    write_xdmf_open_grid_h5(this->_h5_file, _grid, kdt);
}

void xdmf_writer::close_grid() { write_xdmf_close_grid_xml(this->_xml_file); }

void xdmf_writer::close()
{
    write_xdmf_xml_domain_end(this->_xml_file);
    write_xdmf_xml_end(this->_xml_file);
    this->_xml_file.close();
    this->_h5_file.close();
}

void write_xdmf_open_xml(std::ofstream& xml_file, std::string const& h5_fn)
{
                              ...
                              const std::string& name, const kdtree<>& kdt)
{
    ...
    write_xdmf_xml_topology(xml_file, h5_filename, name, dim, num_global_cells, num_cell_points);
    write_xdmf_xml_geometry(xml_file, h5_filename, name, dim, num_global_cells, num_cell_points);
    // write_xdmf_xml_attribute( xml_file, "ownership", h5_filename, "/ownership",
    // "Int", 1, num_global_cells, true
    // );
}

void write_xdmf_xml_open_particles(std::ofstream& xml_file, const std::string& h5_filename,
                                   const std::string& name, unsigned n_parts, unsigned n_dims,
                                   mpi::comm const& comm)
{
    write_xdmf_xml_begin(xml_file);
    write_xdmf_xml_domain_begin(xml_file);
                             ...
                             unsigned num_cell_points)
{
    ...
    xml_file << "Dimensions=\"" << num_global_cells << " " << num_cell_points << "\">\n"
             << setindent(2);
    xml_file << indent << h5_filename << ":/" << grid << "/topology\n";
...
void write_xdmf_xml_geometry(std::ofstream& xml_file, const std::string& h5_filename,
                             ...
                             unsigned num_cell_points)
{
    static const char* geometry_map[4] = {"", "", "XY", "XYZ"};
    ...
    xml_file << "Dimensions=\"" << num_global_cells * num_cell_points << " " << dim << "\">\n"
             << setindent(2);
    xml_file << indent << h5_filename << ":/" << grid << "/geometry\n";
    xml_file << setindent(-2) << indent << "</DataItem>\n";
