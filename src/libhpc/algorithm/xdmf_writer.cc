...
void xdmf_writer::open(const std::string &base_filename,
                       mpi::comm const &comm) {
...
void write_xdmf_open_grid_xml(std::ofstream &xml_file,
  ...
  write_xdmf_xml_topology(xml_file, h5_filename, name, dim, num_global_cells,
                          num_cell_points);
  write_xdmf_xml_geometry(xml_file, h5_filename, name, dim, num_global_cells,
                          num_cell_points);
...
void write_xdmf_xml_open_particles(std::ofstream &xml_file,
                                   const std::string &h5_filename,
                                   const std::string &name, unsigned n_parts,
                                   unsigned n_dims, mpi::comm const &comm) {
  write_xdmf_xml_begin(xml_file);
  write_xdmf_xml_domain_begin(xml_file);
  write_xdmf_xml_grid_begin(xml_file, name);
