...
OutputIter counts_to_displs(InputIter first, InputIter const &last,
                            OutputIter result) {
...
template <class InputSeq, class OutputSeq>
void counts_to_displs_resize(InputSeq const &input, OutputSeq &output) {
  if (input.size()) {
...
template <class Seq>
void _counts_to_displs_i(typename type_traits<Seq>::reference seq) {
  if (seq.size() > 0)
...
template <class T>
inline std::vector<T> counts_to_displs(std::vector<T> const &cnts) {
  return counts_to_displs<std::vector<T>>(cnts);
}

template <class InputIter, class OutputIter>
OutputIter displs_to_counts(InputIter first, InputIter last,
                            OutputIter result) {
  typedef typename std::iterator_traits<InputIter>::value_type value_type;
...
template <class InputSeq, class OutputSeq>
void displs_to_counts_resize(const InputSeq &input, OutputSeq &output) {
  if (input.size() > 1) {
    output.resize(input.size() - 1);
...
void displs_to_counts(typename type_traits<Sequence>::reference seq) {
  displs_to_counts(seq.begin(), seq.size());
}

template <class DisplsIter>
DisplsIter correct_displs(DisplsIter first, size_t size) {
...
template <class Seq>
void correct_displs(typename hpc::type_traits<Seq>::reference seq) {
