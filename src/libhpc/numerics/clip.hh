...
namespace hpc {
    ...
    template <class Seq1, class Seq2, class Seq3> void cross_product_3(const Seq1 &x, const Seq2 &y, Seq3 &z) {
        z[0] = x[1] * y[2] - x[2] * y[1];
        z[1] = x[2] * y[0] - x[0] * y[2];
        z[2] = x[0] * y[1] - x[1] * y[0];
    }
    ...
    typename PointT::value_type distance_to_line_2(EndPointT const &ep0, EndPointT const &ep1, PointT const pnt) {
        ...
        if(r >= 0.0 && r <= 1.0) {
            ...
            return std::min(sqrt((ep0[0] - pnt[0]) * (ep0[0] - pnt[0]) + (ep0[1] - pnt[1]) * (ep0[1] - pnt[1])),
                            sqrt((ep1[0] - pnt[0]) * (ep1[0] - pnt[0]) + (ep1[1] - pnt[1]) * (ep1[1] - pnt[1])));
        }
    }

    template <class PointIterator, class HalfSpaceIterator>
    bool inside(PointIterator                                            point_first,
    ...
    template <class PointIterator, class HalfSpaceIterator>
    typename std::iterator_traits<PointIterator>::value_type half_space_eval(PointIterator        point_first,
                                                                             const PointIterator &point_last,
                                                                             HalfSpaceIterator    half_space_first) {
        typedef typename std::iterator_traits<PointIterator>::value_type real_type;
        real_type                                                        sum = 0.0;
    ...
    void line_half_space_intersection(Point1Iterator        point1_first,
        ...
        real1_type denom = inner_product(
            boost::make_transform_iterator(boost::make_zip_iterator(boost::make_tuple(point2_first, point1_first)),
                                           func),
            boost::make_transform_iterator(boost::make_zip_iterator(boost::make_tuple(point2_last, point1_last)), func),
            half_space_first);
        denom            = 1.0 / denom;
    ...
    bool box_half_space_overlap(MinIterator                                            min_first,
    ...
    void box_box_clip(MinIterator        min_first,
                      const MinIterator &min_last,
                      MaxIterator        max_first,
    ...
    void clip_edge(const HalfSpaceIterator &half_space,
                   PointIterator            first,
        ...
        while(first != last) {
            ...
            // Case 2, edge goes from outside window to inside.
                ...
                point_type tmp;
                line_half_space_intersection(pnts[edge[0]]->begin(),
                                             pnts[edge[0]]->end(),
                                             pnts[edge[1]]->begin(),
                                             pnts[edge[1]]->end(),
                                             half_space,
    ...
    void clip_polygon(double               box_size,
        ...
        hsp[0] = 1;
        clip_edge(
            hsp.begin(), poly_start, poly_finish, std::insert_iterator<std::list<point_type>>(tmp[0], tmp[0].begin()));
        hsp[0]   = -1;
        hsp[dim] = -box_size;
        if(dim == 1) {
    ...
    bool ecs_box_collision(EcsSeqT const &ecs_min,
        ...
        for(int ii = 0; ii < 8; ++ii) {
            hpc::num::cartesian_to_ecs<double>(
                crds(ii, 0), crds(ii, 1), crds(ii, 2), ecs(ii, 0), ecs(ii, 1), ecs(ii, 2));
        }
