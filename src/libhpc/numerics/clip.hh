...
magnitude(Iterator first, const Iterator &last) {
...
void cross_product_3(const Seq1 &x, const Seq2 &y, Seq3 &z) {
  z[0] = x[1] * y[2] - x[2] * y[1];
  z[1] = x[2] * y[0] - x[0] * y[2];
  z[2] = x[0] * y[1] - x[1] * y[0];
}

template <class EndPointT, class PointT>
typename PointT::value_type distance_to_line_2(EndPointT const &ep0,
                                               EndPointT const &ep1,
                                               PointT const pnt) {
  typedef typename PointT::value_type real_type;
  real_type numer = (pnt[0] - ep0[0]) * (ep1[0] - ep0[0]) +
                    (pnt[1] - ep0[1]) * (ep1[1] - ep0[1]);
  real_type denom = (ep1[0] - ep0[0]) * (ep1[0] - ep0[0]) +
                    (ep1[1] - ep0[1]) * (ep1[1] - ep0[1]);
  ...
  if (r >= 0.0 && r <= 1.0) {
    ...
    real_type dist =
        fabs(-perp[1] * (ep0[1] - pnt[1]) - (ep0[0] - pnt[0]) * perp[0]) *
        inv_mag;
    return dist;
    // real_type s = ((ep0[1] - pnt[1])*(ep1[0] - ep0[0]) - (ep0[0] -
    // pnt[0])*(ep1[1] - ep0[1]))/denom; return fabs( s )/sqrt( denom );
  } else {
    return std::min(sqrt((ep0[0] - pnt[0]) * (ep0[0] - pnt[0]) +
                         (ep0[1] - pnt[1]) * (ep0[1] - pnt[1])),
                    sqrt((ep1[0] - pnt[0]) * (ep1[0] - pnt[0]) +
                         (ep1[1] - pnt[1]) * (ep1[1] - pnt[1])));
  }
}

template <class PointIterator, class HalfSpaceIterator>
bool inside(
    PointIterator point_first, const PointIterator &point_last,
...
half_space_eval(PointIterator point_first, const PointIterator &point_last,
                HalfSpaceIterator half_space_first) {
  typedef typename std::iterator_traits<PointIterator>::value_type real_type;
  real_type sum = 0.0;
  while (point_first != point_last)
...
template <class Point1Iterator, class Point2Iterator, class HalfSpaceIterator,
          class ResultIterator>
void line_half_space_intersection(Point1Iterator point1_first,
  ...
  real1_type denom = inner_product(
      boost::make_transform_iterator(boost::make_zip_iterator(boost::make_tuple(
                                         point2_first, point1_first)),
                                     func),
  ...
  denom = 1.0 / denom;
  real1_type enumr =
      half_space_eval(point1_first, point1_last, half_space_first);
...
template <class MinIterator, class MaxIterator, class HalfSpaceIterator>
bool box_half_space_overlap(
    MinIterator min_first, const MinIterator &min_last, MaxIterator max_first,
    HalfSpaceIterator half_space_first,
  ...
  do {
    ...
    if (inside(point.begin(), point.end(), half_space_first, epsilon))
      return true;
  } while (boost::next_partial_permutation(perms.begin(), perms.begin() + 3,
                                           perms.end()));
  return false;
}

template <class MinIterator, class MaxIterator, class HalfSpaceIterator>
bool box_half_space_intersection(MinIterator min_first,
...
template <class BndIterA, class BndIterB>
bool box_box_collision(BndIterA a_begin, BndIterA const &a_end,
                       BndIterB b_begin) {
...
template <class MinIterator, class MaxIterator, class MinBndIterator,
          class MaxBndIterator, class ResultMinIterator,
          class ResultMaxIterator>
void box_box_clip(MinIterator min_first, const MinIterator &min_last,
                  MaxIterator max_first, MinBndIterator bnd_min_first,
                  MaxBndIterator bnd_max_first, ResultMinIterator min_result,
...
template <class MinIterator, class MaxIterator>
typename std::iterator_traits<MinIterator>::value_type
box_volume(MinIterator min_first, const MinIterator &min_last,
           MaxIterator max_first) {
...
polygon_area(PointIterator point_first, const PointIterator &point_last) {
...
template <class HalfSpaceIterator, class PointIterator, class ResultIterator>
void clip_edge(const HalfSpaceIterator &half_space, PointIterator first,
               const PointIterator &last, ResultIterator result) {
  typedef typename std::iterator_traits<PointIterator>::value_type point_type;
  typedef
      typename std::iterator_traits<HalfSpaceIterator>::value_type real_type;
  ...
  while (first != last) {
    pnts[edge[1]] = first++;
    intersect[edge[1]] =
        inside(pnts[edge[1]]->begin(), pnts[edge[1]]->end(), half_space);
    ...
    // Case 2, edge goes from outside window to inside.
      ...
      point_type tmp;
      line_half_space_intersection(pnts[edge[0]]->begin(), pnts[edge[0]]->end(),
                                   pnts[edge[1]]->begin(), pnts[edge[1]]->end(),
                                   half_space, tmp.begin());
      *result++ = tmp;
      *result++ = *pnts[edge[1]];
    }
...
template <class PointIterator, class ResultIterator>
void clip_polygon(double box_size, PointIterator poly_start,
                  const PointIterator &poly_finish, ResultIterator result) {
  typedef typename std::iterator_traits<PointIterator>::value_type point_type;
  ...
  hsp[0] = 1;
  clip_edge(
      hsp.begin(), poly_start, poly_finish,
  ...
  if (dim == 1) {
    ...
    clip_edge(
        hsp.begin(), tmp[0].begin(), tmp[0].end(),
  ...
  for (unsigned ii = 1; ii < dim - 1; ++ii) {
    ...
    tmp[0].clear();
    clip_edge(
        hsp.begin(), tmp[1].begin(), tmp[1].end(),
        std::insert_iterator<std::list<point_type>>(tmp[0], tmp[0].begin()));
    hsp[ii] = -1;
    hsp[dim] = -box_size;
    tmp[1].clear();
...
template <class BoxMinIterT, class BoxMaxIterT, class SphIterT>
bool sphere_box_collision(BoxMinIterT box_min_begin,
                          BoxMinIterT const &box_min_end,
                          BoxMaxIterT box_max_begin, SphIterT sph_begin) {
  ...
  LOGTLN("Sphere-box collision, min_dist: ", min_dist,
         ", radius: ", (*sph_begin) * (*sph_begin));
  return min_dist < (*sph_begin) * (*sph_begin);
}

template <class EcsSeqT, class BoxSeqT>
bool ecs_box_collision(EcsSeqT const &ecs_min, EcsSeqT const &ecs_max,
                       BoxSeqT const &box_min, BoxSeqT const &box_max) {
  ...
  // collision routine.
  {
    ...
    if (!hpc::sphere_box_collision(box_min.begin(), box_min.end(),
                                   box_max.begin(), sph.begin())) {
  ...
  for (unsigned ii = 0; ii < 8; ++ii) {
    if (ecs(ii, 2) >= ecs_min[2]) {
      LOGTLN("Corner ", ii, " with radius ", ecs(ii, 2), " greater than ",
             ecs_min[2], ".");
      discard = false;
      break;
    }
  ...
  for (int half_ii = 0; half_ii < 2; ++half_ii) {
    ...
    for (int ii = 0; ii < 4; ++ii)
      surf[1].push_back(
          boost::array<real_type, 3>{crds(ii, 0), crds(ii, 1), 0.0});
    if (hpc::approx(ecs_min[0], ecs_max[0], 1e-8)) {
      // Do nothing, as we want an empty list.
      surf[1].clear();
    } else if (hpc::approx(ecs_min[0], 0.0, 1e-8) &&
               hpc::approx(ecs_max[0], 2.0 * M_PI, 1e-8)) {
      // Fill the list with the standard.
    } else {
      boost::array<real_type, 4> ra_planes[2];
      boost::array<real_type, 3> tmp[2];
      num::ecs_to_cartesian<real_type>(min_ra, -0.25 * M_PI, tmp[0][0],
                                       tmp[0][1], tmp[0][2]);
      num::ecs_to_cartesian<real_type>(min_ra, 0.25 * M_PI, tmp[1][0],
                                       tmp[1][1], tmp[1][2]);
      cross_product_3(tmp[1], tmp[0], ra_planes[0]);
      ra_planes[0][3] = 0.0;
      ASSERT(hpc::approx(ra_planes[0][2], 0.0, 1e-8));
      num::ecs_to_cartesian<real_type>(max_ra, -0.25 * M_PI, tmp[0][0],
                                       tmp[0][1], tmp[0][2]);
      num::ecs_to_cartesian<real_type>(max_ra, 0.25 * M_PI, tmp[1][0],
                                       tmp[1][1], tmp[1][2]);
      ...
      clip_edge(ra_planes[0].begin(), surf[1].begin(), surf[1].end(),
                std::insert_iterator<std::list<boost::array<real_type, 3>>>(
                    surf[0], surf[0].begin()));
    ...
    unsigned ii = 0;
    for (typename std::list<boost::array<real_type, 3>>::const_iterator it =
             surf[1].begin();
         it != surf[1].end(); ++it, ++ii) {
      real_type dist;
      ...
      if (ii < surf[1].size() - 1) {
        ...
        boost::array<real_type, 2> ep1 = {surf[1].front()[0],
                                          surf[1].front()[1]};
        dist = distance_to_line_2(ep0, ep1, zero);
      }
      if (dist < min_dist)
    ...
    //         ", box_min[2]=", box_min[2], ", cone_rad=", box_min[2]/tan(
    //         ecs_max[1] ) );
    if (ecs_max[1] > 0.0) {
      if (!hpc::approx(ecs_max[1], 0.5 * M_PI, 1e-8) && box_min[2] > 0.0 &&
          max_dist <= fabs(box_min[2] / tan(ecs_max[1]))) {
      ...
      if (hpc::approx(ecs_max[1], -0.5 * M_PI, 1e-8) ||
          (box_min[2] > 0.0 ||
           min_dist >= fabs(box_min[2] / tan(ecs_max[1])))) {
        LOGTLN("Lower plane inside upper cone.");
        continue;
      }
    ...
    // LOGT( "Checking min DEC: dec=", ecs_min[1], ", max_dist=", max_dist, ",
    // min_dist=", min_dist ); LOGTLN(", box_max[2]=", box_max[2], ",
    // cone_rad=", fabs( box_max[2]*tan( ecs_min[1] ) ) );
    if (ecs_min[1] < 0.0) {
      if (!hpc::approx(ecs_min[1], -0.5 * M_PI, 1e-8) && box_max[2] < 0.0 &&
          max_dist <= fabs(box_max[2] / tan(ecs_min[1]))) {
      ...
      if (hpc::approx(ecs_min[1], 0.5 * M_PI, 1e-8) ||
          (box_max[2] < 0.0 ||
           min_dist >= fabs(box_max[2] / tan(ecs_min[1])))) {
        LOGTLN("Upper plane inside lower cone.");
        continue;
      }
    ...
    // wow that's confusing.
    {
      ...
      if (box_max[2] >= upp_z && max_dist <= upp_rad) {
        if (box_min[2] >= 0.0)
          ...
          if (max_dist <=
              sqrt(ecs_min[2] * ecs_min[2] - box_min[2] * box_min[2]))
            continue;
        } else {
          if (max_dist <= low_rad)
            continue;
      ...
      if (box_min[2] <= low_z && max_dist <= low_rad) {
        if (box_max[2] <= 0.0)
          ...
          if (max_dist <=
              sqrt(ecs_min[2] * ecs_min[2] - box_max[2] * box_max[2]))
            continue;
        } else {
          if (max_dist <= upp_rad)
            continue;
