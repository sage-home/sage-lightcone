...
typename std::iterator_traits<Iterator>::value_type magnitude(Iterator first, const Iterator& last)
{
...
typename PointT::value_type distance_to_line_2(EndPointT const& ep0, EndPointT const& ep1,
                                               PointT const pnt)
{
...
bool inside(PointIterator point_first, const PointIterator& point_last,
...
template <class Point1Iterator, class Point2Iterator, class HalfSpaceIterator, class ResultIterator>
void line_half_space_intersection(Point1Iterator point1_first, const Point1Iterator& point1_last,
                                  Point2Iterator point2_first, const Point2Iterator& point2_last,
                                  HalfSpaceIterator half_space_first, ResultIterator result)
{
    ...
    auto func = expand2(compose2(std::minus<real1_type>(),
    ...
    real1_type denom = inner_product(
        boost::make_transform_iterator(
            boost::make_zip_iterator(boost::make_tuple(point2_first, point1_first)), func),
        boost::make_transform_iterator(
            boost::make_zip_iterator(boost::make_tuple(point2_last, point1_last)), func),
        half_space_first);
    denom = 1.0 / denom;
...
bool box_half_space_overlap(MinIterator min_first, const MinIterator& min_last,
                            MaxIterator max_first, HalfSpaceIterator half_space_first,
...
template <class MinIterator, class MaxIterator, class HalfSpaceIterator>
bool box_half_space_intersection(MinIterator min_first, const MinIterator& min_last,
                                 MaxIterator max_first, HalfSpaceIterator half_space_first)
{
    typedef typename std::iterator_traits<MinIterator>::value_type real1_type;
    typedef typename std::iterator_traits<MaxIterator>::value_type real2_type;
...
bool box_box_collision(BndIterA a_begin, BndIterA const& a_end, BndIterB b_begin)
{
...
template <class MinIterator, class MaxIterator, class MinBndIterator, class MaxBndIterator,
          class ResultMinIterator, class ResultMaxIterator>
void box_box_clip(MinIterator min_first, const MinIterator& min_last, MaxIterator max_first,
                  MinBndIterator bnd_min_first, MaxBndIterator bnd_max_first,
                  ResultMinIterator min_result, ResultMaxIterator max_result)
{
...
box_volume(MinIterator min_first, const MinIterator& min_last, MaxIterator max_first)
{
...
typename PointIterator::value_type::value_type polygon_area(PointIterator point_first,
...
void clip_edge(const HalfSpaceIterator& half_space, PointIterator first, const PointIterator& last,
               ResultIterator result)
{
    ...
    while (first != last)
    {
        ...
        else if (!intersect[edge[0]] && intersect[edge[1]])
        {
            ...
            line_half_space_intersection(pnts[edge[0]]->begin(), pnts[edge[0]]->end(),
                                         pnts[edge[1]]->begin(), pnts[edge[1]]->end(), half_space,
                                         tmp.begin());
            *result++ = tmp;
            *result++ = *pnts[edge[1]];
        }
    ...
    if (first_intersect ^ intersect[edge[0]])
    {
        point_type tmp;
        line_half_space_intersection(pnts[edge[0]]->begin(), pnts[edge[0]]->end(),
                                     first_point->begin(), first_point->end(), half_space,
                                     tmp.begin());
        *result++ = tmp;
    }
}
...
void clip_polygon(double box_size, PointIterator poly_start, const PointIterator& poly_finish,
                  ResultIterator result)
{
    typedef typename std::iterator_traits<PointIterator>::value_type point_type;
...
bool sphere_box_collision(BoxMinIterT box_min_begin, BoxMinIterT const& box_min_end,
...
bool ecs_box_collision(EcsSeqT const& ecs_min, EcsSeqT const& ecs_max, BoxSeqT const& box_min,
                       BoxSeqT const& box_max)
{
    ...
    {
        hpc::num::cartesian_to_ecs<double>(crds(ii, 0), crds(ii, 1), crds(ii, 2), ecs(ii, 0),
                                           ecs(ii, 1), ecs(ii, 2));
    }
    ...
    // collision routine.
    {
        ...
        if (!hpc::sphere_box_collision(box_min.begin(), box_min.end(), box_max.begin(),
                                       sph.begin()))
        {
    ...
    for (int half_ii = 0; half_ii < 2; ++half_ii)
    {
        ...
        else
        {
            ...
            clip_edge(ra_planes[0].begin(), surf[1].begin(), surf[1].end(),
                      std::insert_iterator<std::list<boost::array<real_type, 3>>>(surf[0],
                                                                                  surf[0].begin()));
            LOGTLN("Intermediate surface: ", surf[0]);
            surf[1].clear();
            clip_edge(ra_planes[1].begin(), surf[0].begin(), surf[0].end(),
                      std::insert_iterator<std::list<boost::array<real_type, 3>>>(surf[1],
                                                                                  surf[1].begin()));
