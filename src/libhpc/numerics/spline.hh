...
///
///
template <class T, class PointsSeq = std::vector<T>,
...
public:
  template <class Seq> void set_knot_points(Seq &&pnts) {
    assign(_pnts, std::forward<Seq>(pnts));
  }

  template <class Seq> void set_knot_values(Seq &&vals) {
    assign(_vals, std::forward<Seq>(vals));
  }

  void update() {
    ASSERT(_pnts.size() == _vals.size(),
           "Mismatched points/values arrays in spline.");
  ...
  const knot_points_type &knot_points() const { return _pnts; }

  const knot_values_type &knot_values() const { return _vals; }

  unsigned num_segments() const { return _diff.size(); }

  value_type segment_start(unsigned seg) const { return _pnts[seg]; }

  value_type segment_finish(unsigned seg) const { return _pnts[seg + 1]; }

  value_type segment_width(unsigned seg) const {
    return _pnts[seg + 1] - _pnts[seg];
  }

  knot_points_type &points() { return _pnts; }

  knot_values_type &values() { return _vals; }

  typename knot_points_type::const_iterator points_begin() const {
    return _pnts.cbegin();
  }

  typename knot_points_type::const_iterator points_end() const {
    return _pnts.cend();
  }

  typename knot_values_type::const_iterator values_begin() const {
    return _vals.cbegin();
  }

  unsigned poly(value_type const &crd) const {
    unsigned poly;
  ...
  value_type operator()(const value_type &crd) const {
    unsigned poly = this->poly(crd);
    return _eval_poly(crd, poly);
  }

  value_type operator()(const value_type &crd, unsigned poly) const {
    return _eval_poly(crd, poly);
  }

  typename hpc::view<std::vector<value_type>> const ai() const { return _ai; }

  typename hpc::view<std::vector<value_type>> const bi() const { return _bi; }

protected:
  value_type _eval_poly(const value_type &crd, unsigned poly) const {
    value_type t = (crd - _pnts[poly]) * _diff[poly];
    value_type u = 1.0 - t;
    return u * _vals[poly] + t * _vals[poly + 1] +
           t * u * (_ai[poly] * u + _bi[poly] * t);
  }
  ...
  void _assemble(std::vector<value_type> &diag,
    ...
    for (; ii < off_diag.size(); ++ii) {
      ...
      off_diag[ii] = 0.5 * tmp;
      rhs[ii] =
          3.0 * ((_vals[ii] - _vals[ii - 1]) / (_diff[ii - 1] * _diff[ii - 1]) +
