...
///
///
template <class T, class PointsSeq = std::vector<T>, class ValuesSeq = std::vector<T>>
class spline
{
public:
...
public:
    template <class Seq>
    void set_knot_points(Seq&& pnts)
    {
        assign(_pnts, std::forward<Seq>(pnts));
    }

    template <class Seq>
    void set_knot_values(Seq&& vals)
    {
    ...
    value_type segment_width(unsigned seg) const { return _pnts[seg + 1] - _pnts[seg]; }

    knot_points_type& points() { return _pnts; }

    knot_values_type& values() { return _vals; }

    typename knot_points_type::const_iterator points_begin() const { return _pnts.cbegin(); }

    typename knot_points_type::const_iterator points_end() const { return _pnts.cend(); }

    typename knot_values_type::const_iterator values_begin() const { return _vals.cbegin(); }
...
protected:
    ...
    void _assemble(std::vector<value_type>& diag, std::vector<value_type>& off_diag,
