...
template <class T> class spline_integrator {
public:
...
public:
  ...
  template <class Spline, class Func>
  value_type operator()(const Spline &sp, Func func) const {
    value_type sum = 0.0;
  ...
  template <class Spline, class Func, class Trans>
  value_type operator()(const Spline &sp, Func func, Trans trans) const {
    value_type sum = 0.0;
    for (unsigned ii = 0; ii < sp.num_segments(); ++ii) {
      value_type low = sp.segment_start(ii);
  ...
  template <class Spline, class Func>
  value_type reverse(const Spline &sp, Func func) const {
    value_type sum = 0.0;
    for (unsigned ii = 0; ii < sp.num_segments(); ++ii) {
...
template <class T> class spline_spline_integrator {
public:
  typedef T value_type;
...
public:
  ...
  template <class Spline0, class Spline1, class Func>
  value_type operator()(const Spline0 &sp0, const Spline1 &sp1,
                        Func func) const {
    // Find the range of overlap.
    value_type low =
        std::max(sp0.knot_points().front(), sp1.knot_points().front());
    value_type upp =
        std::min(sp0.knot_points().back(), sp1.knot_points().back());
    ...
    auto it = make_interp_iterator(
        sp0.knot_points().begin(), sp0.knot_points().end(),
        sp1.knot_points().begin(), sp1.knot_points().end(), 1e-7);
    ...
    while (!approx(*it++, upp, 1e-7)) {
      ...
      for (unsigned ii = 0; ii < _quad.size(); ++ii) {
        ...
        value_type y0 = sp0(x, sp0_poly);
        value_type y1 = sp1(x, sp1_poly);
        cur_sum += _quad.weights()[ii] * func(x, y0, y1);
