...
public:
  legendre(unsigned order) : _order(order) {}
...
public:
  ...
  jacobi(value_type alpha, value_type beta)
      : _order(1), _alpha(alpha), _beta(beta) {}

  jacobi(unsigned order, value_type alpha, value_type beta)
      : _order(order), _alpha(alpha), _beta(beta) {}
  ...
  value_type derivative(unsigned order, value_type x, value_type value) const {
    value_type al = this->_alpha, be = this->_beta, p1 = this->_p1,
               tmp = this->_tmp;
    return (order * (al - be - tmp * x) * value +
            2.0 * (order + al) * (order + be) * p1) /
           (tmp * (1.0 - x * x));
  }
  ...
  value_type deriv(value_type x, value_type value) const {
    return derivative(x, value);
  }
...
template <class T> class jacobi_norm : public jacobi<T> {
public:
  typedef T value_type;
...
protected:
  value_type _calc_norm(unsigned order) {
...
template <class Jacobi> class jacobi_derivative : Jacobi {
public:
...
template <class Polynomial1D> class tensor_product {
public:
...
public:
  ...
  tensor_product_derivative(unsigned dim, unsigned order,
  ...
  value_type operator()(unsigned mode, unsigned deriv_dim,
