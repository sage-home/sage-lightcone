...
public:
    ...
    value_type derivative(const value_type& x, const value_type& value) const
    {
        return _order * (x * value - _p1) / (x * x - 1.0);
    }

    value_type deriv(const value_type& x, const value_type& value) const
    {
        return derivative(x, value);
    }

    T previous_order() const { return this->_p1; }
...
public:
    ...
    jacobi(value_type alpha, value_type beta)
        : _order(1)
        , _alpha(alpha)
        , _beta(beta)
    {
    ...
    jacobi(unsigned order, value_type alpha, value_type beta)
        : _order(order)
        , _alpha(alpha)
        , _beta(beta)
    {
    ...
    void set_order(unsigned order) { this->_order = order; }

    unsigned order() const { return this->_order; }

    value_type operator()(unsigned order, const value_type& x)
    {
    ...
    value_type operator()(const value_type& x) { return jacobi::operator()(this->_order, x); }

    value_type derivative(unsigned order, value_type x, value_type value) const
    {
        value_type al = this->_alpha, be = this->_beta, p1 = this->_p1, tmp = this->_tmp;
        return (order * (al - be - tmp * x) * value + 2.0 * (order + al) * (order + be) * p1) /
               (tmp * (1.0 - x * x));
    }

    value_type deriv(unsigned order, value_type x, value_type value) const
    {
        return derivative(order, x, value);
    }

    value_type derivative(value_type x, value_type value) const
    {
        return this->derivative(this->_order, x, value);
    }

    value_type deriv(value_type x, value_type value) const { return derivative(x, value); }

    value_type previous_order() const { return this->_p1; }
...
template <class T>
class jacobi_norm : public jacobi<T>
{
public:
    typedef T value_type;

    jacobi_norm(value_type alpha, value_type beta)
        : jacobi<value_type>(alpha, beta)
    {
        this->_norm = this->_calc_norm(this->_order);
    }

    jacobi_norm(unsigned order, value_type alpha, value_type beta)
...
protected:
    value_type _calc_norm(unsigned order) { return 1.0 / sqrt(2.0 / (2.0 * order + 1.0)); }
...
class jacobi_derivative : Jacobi
{
public:
    typedef typename Jacobi::value_type value_type;

    jacobi_derivative(value_type alpha, value_type beta)
        : Jacobi(alpha + 1.0, beta + 1.0)
    {
    }

    jacobi_derivative(unsigned order, value_type alpha, value_type beta)
    ...
    value_type operator()(const value_type& x) { return operator()(this->_order, x); }
};
...
public:
    ...
    tensor_product(unsigned dim, unsigned order, const Polynomial1D& poly)
        : _dim(dim)
        , _poly(poly)
    {
    ...
    value_type operator()(unsigned mode, view<std::vector<value_type>> const& x)
    {
        ASSERT(this->_dim > 0);
...
template <class Polynomial1D, class PolynomialDeriv1D>
class tensor_product_derivative : public tensor_product<Polynomial1D>
{
public:
    typedef typename Polynomial1D::value_type value_type;

    tensor_product_derivative(unsigned dim, unsigned order, const Polynomial1D& poly,
    ...
    value_type operator()(unsigned mode, unsigned deriv_dim, view<std::vector<value_type>> const& x)
    {
        ASSERT(this->_dim > 0);
        ASSERT(x.size() == this->_dim);
