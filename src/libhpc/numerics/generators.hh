...
template <class Iter>
void gaussian(Iter start, unsigned num_points, typename Iter::value_type amp = 1)
{
    typedef typename Iter::value_type value_type;
    for (unsigned ii = 0; ii < num_points; ++ii)
    {
        value_type x =
            ((value_type)ii / (value_type)(num_points - 1)) * (value_type)2 - (value_type)1;
        *start++ = amp * exp(-(x * x));
    }
}
...
template <class T, class SeqT = std::vector<T>>
SeqT iota(T size, T init = 0)
{
    SeqT seq(size);
