...
template <class T = double> class least_squares {
public:
  typedef T value_type;

public:
  least_squares() : _dim(0), _ord(0), _ns(0) {}

  void resize(unsigned num_smpls, unsigned ord = 1, unsigned dim = 1) {
    _dim = dim;
    _ord = ord;
  ...
  template <class Iter> void add_sample(Iter const &begin, value_type val) {
  ...
  void add_sample(value_type x, value_type val) {
    ASSERT(_dim == 1);
    add_sample(&x, val);
  }
  ...
  void solve() {
    ...
    _sol = _design.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV)
               .solve(_rhs);
  }

  Eigen::Matrix<value_type, Eigen::Dynamic, 1> const &solution() const {
    return _sol;
  }
