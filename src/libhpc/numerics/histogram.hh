    ...
    namespace numerics {
        ...
        template <class T = double> class histogram {
          ...
          public:
            ...
            histogram(std::shared_ptr<axis_type> &axis) : _axis(axis), _bins(axis->size()) {
            }

            std::shared_ptr<axis_type> &axis() {
                return _axis;
            }
            ...
            void add_sample(value_type x) {
                ++_bins[_axis->bin(x)];
            }

            std::vector<unsigned> const &bins() const {
                return _bins;
            }
