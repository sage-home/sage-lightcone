...
void gauss_legendre(unsigned size, T x1, T x2, PointIter points, WeightIter weights,
                    T tolerance = alg::default_newton_tolerance)
{
    ...
    for (unsigned ii = 1; ii <= half; ++ii)
    {
        ...
        hpc::array<T, 2> x_df =
            alg::newtond<polynomial::legendre<T>, T>(legendre, xl, xu, x, tolerance);
        x = x_df[0];
        df = x_df[1];
...
{
public:
    gauss_legendre_generator(T x0 = -1.0, T x1 = 1.0)
        : _x0(x0)
        , _x1(x1)
    {
    }
    ...
    void operator()(unsigned size, PointIter points, WeightIter weights, T tolerance = 1e-8) const
    {
...
///
template <class T, class PointIter, class WeightIter>
void gauss_jacobi_ref(unsigned size, T alpha, T beta, PointIter& points, WeightIter& weights,
                      T tolerance = 1e-8)
{
    // Setup the polynomial functor for the Legendre polynomials.
    unsigned np = size;
...
template <class T, class PointIter, class WeightIter>
void gauss_jacobi(unsigned size, T alpha, T beta, PointIter points, WeightIter weights,
                  T tolerance = 1e-8)
{
...
///
template <class T, class PointIter, class WeightIter>
void gauss_lobatto(unsigned size, T alpha, T beta, PointIter points, WeightIter weights,
                   T tolerance = 1e-8)
{
...
template <class Points1DIter, class Weights1DIter, class PointsOutIter, class WeightsOutIter>
void outer_product(unsigned size_1d, unsigned dim, Points1DIter points_1d, Weights1DIter weights_1d,
                   PointsOutIter points, WeightsOutIter weights)
{
...
void make_quadrature(Generator gen, unsigned order, PointIter points, WeightIter weights,
                     unsigned dim = 1, typename PointIter::value_type tolerance = 1e-8)
{
...
public:
    template <class Seq>
    void set_points(Seq& pnts)
    {
        _pnts = std::move(pnts);
    }

    template <class Seq>
    void set_points(const Seq& pnts)
    {
        _pnts = pnts;
    }

    template <class Seq>
    void set_weights(Seq& wgts)
    {
        _wgts = std::move(wgts);
    }

    template <class Seq>
    void set_weights(const Seq& wgts)
    {
        _wgts = wgts;
    }
...
void make_quadrature(Generator gen, unsigned order, quadrature<T>& quad, unsigned dim = 1)
{
