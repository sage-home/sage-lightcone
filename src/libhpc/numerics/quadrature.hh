...
#ifndef hpc_numerics_quadrature_hh
#define hpc_numerics_quadrature_hh
...
///
template <class T, class PointIter, class WeightIter>
void gauss_legendre(unsigned size, T x1, T x2, PointIter points,
                    WeightIter weights,
...
template <class T> class gauss_legendre_generator {
public:
  gauss_legendre_generator(T x0 = -1.0, T x1 = 1.0) : _x0(x0), _x1(x1) {}

  template <class PointIter, class WeightIter>
  void operator()(unsigned size, PointIter points, WeightIter weights,
...
///
template <class T, class PointIter, class WeightIter>
void gauss_jacobi_ref(unsigned size, T alpha, T beta, PointIter &points,
                      WeightIter &weights, T tolerance = 1e-8) {
  // Setup the polynomial functor for the Legendre polynomials.
  unsigned np = size;
  ...
  for (unsigned ii = 0; ii < np; ++ii) {
    ...
    // Solve using Newton-Raphson.
    hpc::array<T, 2> x_df =
        alg::newtond<polynomial::jacobi<T>, T>(jacobi, -1.0, 1.0, x, tolerance);
    x = x_df[0];
    df = x_df[1];
    ...
    *points++ = -x;
    *weights++ = exp(lgamma<T>(alpha + np) + lgamma<T>(beta + np) -
                     lgamma<T>(np + 1.0) - lgamma<T>(np + alpha + beta + 1.0)) *
                 (2 * np + alpha + beta) * pow(2.0, alpha + beta) /
                 (df * jacobi.previous_order());
  }
...
template <class T, class PointIter, class WeightIter>
void gauss_jacobi(unsigned size, T alpha, T beta, PointIter points,
                  WeightIter weights, T tolerance = 1e-8) {
...
///
template <class T, class PointIter, class WeightIter>
void gauss_lobatto(unsigned size, T alpha, T beta, PointIter points,
                   WeightIter weights, T tolerance = 1e-8) {
  ...
  *points++ = -1.0;
  ...
  if (size > 2) {
    ...
    gauss_jacobi_ref(size, alpha + 1.0, beta + 1.0, points, weights, tolerance);
  }
...
template <class Points1DIter, class Weights1DIter, class PointsOutIter,
          class WeightsOutIter>
void outer_product(unsigned size_1d, unsigned dim, Points1DIter points_1d,
                   Weights1DIter weights_1d, PointsOutIter points,
...
template <class Generator, class PointIter, class WeightIter>
void make_quadrature(Generator gen, unsigned order, PointIter points,
                     WeightIter weights, unsigned dim = 1,
  ...
  outer_product(order + 1, dim, points_1d.begin(), weights_1d.begin(), points,
                weights);
}
...
public:
  template <class Seq> void set_points(Seq &pnts) { _pnts = std::move(pnts); }

  template <class Seq> void set_points(const Seq &pnts) { _pnts = pnts; }

  template <class Seq> void set_weights(Seq &wgts) { _wgts = std::move(wgts); }

  template <class Seq> void set_weights(const Seq &wgts) { _wgts = wgts; }

  size_t size() const { return _wgts.size(); }

  const std::vector<value_type> &points() const { return _pnts; }

  const std::vector<value_type> &weights() const { return _wgts; }
...
void make_quadrature(Generator gen, unsigned order, quadrature<T> &quad,
                     unsigned dim = 1) {
