...
namespace hpc {
    namespace num {
        ...
        void gauss_legendre(unsigned   size,
                            T          x1,
                            T          x2,
                            PointIter  points,
                            WeightIter weights,
            ...
            for(unsigned ii = 1; ii <= half; ++ii) {
                ...
                hpc::array<T, 2> x_df = alg::newtond<polynomial::legendre<T>, T>(legendre, xl, xu, x, tolerance);
                x                     = x_df[0];
                df                    = x_df[1];
        ...
        ///
        template <class T, class PointIter, class WeightIter>
        void gauss_jacobi_ref(unsigned    size,
                              T           alpha,
                              T           beta,
                              PointIter & points,
                              WeightIter &weights,
            ...
            for(unsigned ii = 0; ii < np; ++ii) {
                ...
                // Solve using Newton-Raphson.
                hpc::array<T, 2> x_df = alg::newtond<polynomial::jacobi<T>, T>(jacobi, -1.0, 1.0, x, tolerance);
                x                     = x_df[0];
                df                    = x_df[1];
                ...
                *weights++ = exp(lgamma<T>(alpha + np) + lgamma<T>(beta + np) - lgamma<T>(np + 1.0) -
                             ...
                             (2 * np + alpha + beta) * pow(2.0, alpha + beta) / (df * jacobi.previous_order());
            }
        ...
        template <class T, class PointIter, class WeightIter>
        void gauss_jacobi(unsigned size, T alpha, T beta, PointIter points, WeightIter weights, T tolerance = 1e-8) {
        ...
        ///
        template <class T, class PointIter, class WeightIter>
        void gauss_lobatto(unsigned size, T alpha, T beta, PointIter points, WeightIter weights, T tolerance = 1e-8) {
        ...
        void outer_product(unsigned       size_1d,
                           unsigned       dim,
        ...
        void make_quadrature(Generator                      gen,
                             unsigned                       order,
                             PointIter                      points,
                             WeightIter                     weights,
                             unsigned                       dim       = 1,
        ...
        template <class T> class quadrature {
          ...
          public:
            template <class Seq> void set_points(Seq &pnts) {
                _pnts = std::move(pnts);
            }

            template <class Seq> void set_points(const Seq &pnts) {
                _pnts = pnts;
            }

            template <class Seq> void set_weights(Seq &wgts) {
                _wgts = std::move(wgts);
            }

            template <class Seq> void set_weights(const Seq &wgts) {
                _wgts = wgts;
            }
            ...
            const std::vector<value_type> &points() const {
                return _pnts;
            }
