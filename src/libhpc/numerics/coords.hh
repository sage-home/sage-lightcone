...
///
template <class T>
void cartesian_to_ecs(T x, T y, T z, T& ra, T& dec, boost::optional<T&> r = boost::optional<T&>())
{
    T _r;
...
///
template <class T>
void ecs_to_cartesian(T ra, T dec, T& x, T& y, T& z, boost::optional<T> rad = boost::optional<T>())
...
template <class T>
void gnomonic_projection(T object_ra, T object_dec, T origin_ra, T origin_dec, T& x, T& y)
{
...
template <class T>
void inverse_gnomonic_projection(T x, T y, T origin_ra, T origin_dec, T& ra, T& dec)
{
    T sind0 = sin(origin_dec), cosd0 = cos(origin_dec);
...
///
template <class T>
T redshift_to_comoving_distance(T z, unsigned points = 1000, T hubble = 71.0, T omega_v = 0.73,
                                T omega_m = 0.27)
{
    ASSERT(z >= 0.0);
...
///
template <class T>
T redshift_to_light_travel_distance(T z, unsigned points = 1000, T hubble = 71.0, T omega_v = 0.73,
                                    T omega_m = 0.27)
{
    ASSERT(z >= 0.0);
...
///
template <class T>
T redshift_to_transverse_distance(T z, unsigned points = 1000, T hubble = 71.0, T omega_v = 0.73,
                                  T omega_m = 0.27)
{
    ASSERT(z >= 0.0);
...
///
template <class T>
T redshift_to_luminosity_distance(T z, unsigned points = 1000, T hubble = 71.0, T omega_v = 0.73,
                                  T omega_m = 0.27)
{
...
///
template <class T>
T redshift_to_angular_diameter_distance(T z, unsigned points = 1000, T hubble = 71.0,
