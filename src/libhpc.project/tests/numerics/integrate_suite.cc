...
struct constant
{
    ...
    result_type operator()(const Point& pnt) const
    {
        return value;
    }
...
TEST_CASE("/hpc/numerics/integrate/quadrature_summation/1d")
{
    // Constant integration of 1 over interval of 2.
    {
        ...
        make_quadrature(gauss_legendre_generator<double>(-1.0, 1.0), order, points.begin(),
                        weights.begin());
        double sum = quadrature_summation(points.begin(), points.end(), weights.begin(),
                                          constant<double>(1.0));
        DELTA(sum, 2.0, 1e-6);
    }
    ...
    // Linear over interval of 2.
    {
        ...
        make_quadrature(gauss_legendre_generator<double>(-1.0, 1.0), order, points.begin(),
                        weights.begin());
        double sum = quadrature_summation(points.begin(), points.end(), weights.begin(),
                                          linear<double>(1.0, 0.0));
        DELTA(sum, 0.0, 1e-6);
        sum = quadrature_summation(points.begin(), points.end(), weights.begin(),
                                   linear<double>(1.0, 1.0));
        DELTA(sum, 2.0, 1e-6);
    }
