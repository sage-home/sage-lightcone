...
TEST_CASE("/libhpc/numerics/spline_integrator/flat_lambda")
{
    ...
    spline_integrator<double> integ;
    double sum = integ(sp0, [](double x, double val) { return val * (1.0 / x); });
    DELTA(sum, 0.0, 1e-6);
}
...
TEST_CASE("/libhpc/numerics/spline_integrator/reverse")
{
    ...
    {
        spline_integrator<double> integ;
        sum0 = integ(sp0, [](double x, double val) { return val; });
    }
    double sum1;
    {
        spline_integrator<double> integ;
        sum1 = integ.reverse(sp0, [](double x, double val) { return val; });
    }
...
TEST_CASE("/libhpc/numerics/spline_integrator/transform")
{
    ...
    // clang-format off
    double sum =
        integ(sp0, [](double x, double val) { return 1.0; }, [](double x) { return 1.0 / x; });
    // clang-format on
