...
public:
    void test_both_empty()
    {
        ...
        interp_iterator<vector<float>::const_iterator> it =
            make_interp_iterator(grid0.cbegin(), grid0.cend(), grid1.cbegin(), grid1.cend());
        bool inside = false;
        for (; !it.done(); ++it)
            inside = true;
        TS_ASSERT(!inside);
    ...
    void test_overlapping()
    {
        ...
        interp_iterator<vector<float>::const_iterator> it = make_interp_iterator(
            grid0.cbegin(), grid0.cend(), grid1.cbegin(), grid1.cend(), (float)1e-4);
        unsigned ii = 0;
        for (; !it.done(); ++it, ++ii)
        {
