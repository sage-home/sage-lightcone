...
public:
    void test_add()
    {
        ...
        int* buf = new int[100];
        TS_ASSERT_EQUALS(memory::ctx.find_group("/").data().size - cur_size,
                         100 * sizeof(int) + padding);
        TS_ASSERT_EQUALS(memory::ctx.find_group("/").data().peak - cur_peak,
                         100 * sizeof(int) + padding);
        delete buf;
#endif
    }
...
#pragma omp parallel num_threads(10)
        ...
        TS_ASSERT_EQUALS(memory::ctx.find_group("/").data().size - cur_size,
                         10 * (100 * sizeof(int) + padding));
        TS_ASSERT_EQUALS(memory::ctx.find_group("/").data().peak - cur_peak,
                         10 * (100 * sizeof(int) + padding));
        TS_ASSERT_EQUALS(memory::ctx.find_group("/zero").data().size,
                         2 * (100 * sizeof(int) + padding));
        TS_ASSERT_EQUALS(memory::ctx.find_group("/five").data().size,
                         1 * (100 * sizeof(int) + padding));
#pragma omp parallel num_threads(10)
        {
            int tid = OMP_TID;
            delete buf[tid];
