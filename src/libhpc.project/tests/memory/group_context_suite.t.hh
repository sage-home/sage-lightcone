...
{
public:
    void test_default_ctor() { memory::group_context<int> gc; }
    ...
    void test_add_groups()
    {
        ...
        gc.select("/face/palm");
        unsigned ii = 0;
        for (memory::group_context<int>::iterator it = gc.begin(); it != gc.end(); ++it)
        {
            ++ii;
        }
...
#pragma parallel num_threads(3)
        {
            ...
            else if (OMP_TID == 1)
            {
                ...
                for (memory::group_context<int>::iterator it = gc.begin(); it != gc.end(); ++it)
                {
                    TS_ASSERT(it->path() == "/" || it->path() == "/hello/world1" ||
                              it->path() == "/face/palm1");
                    ++ii;
                }
                TS_ASSERT_EQUALS(ii, 3);
