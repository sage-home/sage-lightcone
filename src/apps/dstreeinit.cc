 ...
 *
 *  Command line modes:
 *  init - actually creates the KDTREE indexed HDF5 file (requires the original
 * galaxyTree HDF5 so it can pick out the columns needed for SED and put them in
 * a "sed/data" group in such a way that globaltreeid can be used as an index to
 * access) flatten - legacy tree2sage - convert TAOImport output hdf5 file with
 * tree structure to KDTree indexed hdf5 dump - legacy count - legacy
 *
 *
 */
...
public:
  myapplication(int argc, char *argv[]) : hpc::mpi::application(argc, argv) {
    ...
    // Setup some options.
    options().add_options()(
        "mode,m", hpc::po::value<std::string>(&_mode)->default_value("init"),
        "mode of operation")("sage,s", hpc::po::value<hpc::fs::path>(&_sage_fn),
                             "SAGE HDF5 file")(
        "tree,t", hpc::po::value<hpc::fs::path>(&_tree_fn), "SAGE trees file")(
        "ppc,p", hpc::po::value<unsigned>(&_ppc)->default_value(1000),
        "particles per cell")("output,o",
                              hpc::po::value<hpc::fs::path>(&_out_fn),
                              "output HDF5 file");
  ...
  void initANY(hpc::fs::path const &sage_fn, hpc::fs::path const &tree_fn,
               hpc::fs::path const &out_fn) {
    LOGBLOCKI("Initialising new file.");

    tao::xml_dict sidecar_xml =
        tao::data_dict::getSidecar(sage_fn.string(), "/sageinput");
    _fields = tao::data_dict::getFieldsANY(sidecar_xml, "/sageinput/Field");
    size_t nfields = _fields.size();
    hpc::h5::derive der =
        tao::data_dict::getCompositeANY(_fields, _mem_type, _file_type);
    ...
    std::set<std::string> snap_names;
    {
      ...
      for (auto const &name : lnks) {
        if (name.length() == 11 && name.compare(0, 8, "snapshot") == 0 &&
            isdigit(name[8]) && isdigit(name[9]) && isdigit(name[10])) {
    ...
    hpc::h5::file out_file(out_fn.native(), H5F_ACC_TRUNC,
                           hpc::mpi::comm::world);
    ...
    out_file.write_serial("snapshot_displs",
                          hpc::counts_to_displs(n_gals_per_snap));

    // Prepare output datatypes.
    {
      hsize_t offs = 0;
      hpc::h5::derive der(3 * sizeof(double) + sizeof(unsigned long long) +
                          sizeof(unsigned));
      der.add(hpc::h5::datatype::native_double, offs,
              hpc::h5::datatype::ieee_f64be, "x");
      offs += sizeof(double);
      der.add(hpc::h5::datatype::native_double, offs,
              hpc::h5::datatype::ieee_f64be, "y");
      offs += sizeof(double);
      der.add(hpc::h5::datatype::native_double, offs,
              hpc::h5::datatype::ieee_f64be, "z");
      offs += sizeof(double);
      der.add(hpc::h5::datatype::native_ullong, offs,
              hpc::h5::datatype::std_u64be, "global_index");
      offs += sizeof(unsigned long long);
      der.add(hpc::h5::datatype::native_uint, offs,
              hpc::h5::datatype::std_u32be, "subsize");
      der.commit(_lc_mem_type, _lc_file_type);
    }

    // Create coordinate dataset.
    hpc::h5::dataset lc_data_ds(out_file, "lightcone/data", _lc_file_type,
                                n_gals);
  ...
  void process_snapshotANY(hpc::h5::file &file, std::string const &name,
    ...
    // read_coords( file.dataset( name ), file.dataset( name + "_map" ), crds,
    // tree_idxs, subsize ); if( crds[0].size() )
    // {
    std::vector<unsigned long long> idxs(crds[0].size());
    boost::iota(idxs, 0);
    data_permuter perm(crds, tree_idxs, subsize, idxs);
    auto part =
        hpc::make_binary_partitioner(crds.begin(), crds.end(), perm, _ppc);
    hpc::kdtree<> tree;
    {
    ...
    // Dump coordinates.
    {
      ...
      for (unsigned ii = 0; ii < 3; ++ii) {
        ...
        props.set_preserve();
        data.write(crds[ii].data(), dt, crds[0].size(), displ,
      ...
      }
      {
        ...
        hpc::h5::property_list props(H5P_DATASET_XFER);
        props.set_preserve();
        data.write(tree_idxs.data(), dt, tree_idxs.size(), displ,
                   hpc::mpi::comm::self, props);
      }
      {
        hpc::h5::datatype dt;
        ...
        hpc::h5::property_list props(H5P_DATASET_XFER);
        props.set_preserve();
        data.write(subsize.data(), dt, subsize.size(), displ,
                   hpc::mpi::comm::self, props);
      }
  ...
  // Map lowercase_with_underscores field names to match SAGE HDF5 CamelCase
  // format SAGE fields: use exact CamelCase from SAGE output Computed fields:
  // keep as lowercase_with_underscores
  std::string map_to_camelcase_field_name(const std::string &lowercase_name) {
    static std::map<std::string, std::string> field_map = {
        ...
        {"metals_ics",
         "MetalsIntraClusterStars"}, // sageh5toh5 abbreviates to 'metals_ics'
        {"metals_cgm_gas", "MetalsCGMgas"},
        {"metals_intracluster_stars",
         "MetalsIntraClusterStars"}, // Full name if present
        ...
        {"tcool_over_tff",
         "tcool_over_tff"},       // Lowercase with underscore in SAGE
        ...
        {"cooling", "Cooling"},
        {"heating", "Heating"},
        {"quasar_mode_bh_accretion_mass",
         "QuasarModeBHaccretionMass"}, // Mixed case in SAGE
        ...
        // Computed fields during conversion - keep lowercase_with_underscores
        // unchanged
  ...
  void write_attributesANY(hpc::h5::dataset &in_ds, hpc::h5::file &out_file,
    ...
    hpc::permuteANY(buf.begin(), buf.end(), idxs.begin(),
                    n_mems); // TODO: RS: I don't understand this but looks like
                             // it needs to change
    for (unsigned ii = 0; ii < n_mems; ++ii) {
      std::string name = _mem_type.member_name(ii);
      std::string name_lowercase = name;
      ...
      for (unsigned jj = 0; jj < todo; ++jj)
        memcpy((char *)tmp.data() + dt.size() * jj,
               (char *)buf.data() + _mem_type.size() * jj + dt_offs, dt.size());
  ...
  void write_kdtree(
    ...
    // Write bounds.
    {
      ...
      der.add(hpc::h5::datatype::native_double, 0,
              hpc::h5::datatype::ieee_f64be, "minimum");
  ...
  void write_sed_dataANY(hpc::h5::file &out_file,
    ...
    der.add(hpc::h5::datatype::native_int, HOFFSET(sed_data_t, descendant),
            hpc::h5::datatype::std_i32be, "descendant");
    der.add(hpc::h5::datatype::native_int, HOFFSET(sed_data_t, snapshot),
            hpc::h5::datatype::std_i32be, "snapshot");
    der.add(hpc::h5::datatype::native_int, HOFFSET(sed_data_t, local_index),
            hpc::h5::datatype::std_i32be, "local_index");
    der.add(hpc::h5::datatype::native_int, HOFFSET(sed_data_t, merge_type),
            hpc::h5::datatype::std_i32be, "merge_type");
    der.add(hpc::h5::datatype::native_double, HOFFSET(sed_data_t, dt),
            hpc::h5::datatype::ieee_f64be, "dt");
    der.add(hpc::h5::datatype::native_double, HOFFSET(sed_data_t, disk_sfr),
            hpc::h5::datatype::ieee_f64be, "disk_sfr");
    der.add(hpc::h5::datatype::native_double, HOFFSET(sed_data_t, bulge_sfr),
            hpc::h5::datatype::ieee_f64be, "bulge_sfr");
    der.add(hpc::h5::datatype::native_double, HOFFSET(sed_data_t, disk_sfr_z),
            hpc::h5::datatype::ieee_f64be, "disk_sfr_z");
    der.add(hpc::h5::datatype::native_double, HOFFSET(sed_data_t, bulge_sfr_z),
            hpc::h5::datatype::ieee_f64be, "bulge_sfr_z");
    ...
    uint16_t descendant_index =
        tao::data_dict::findField(_fields, "descendant");
    uint16_t snapnum_index = tao::data_dict::findField(_fields, "snapnum");
    uint16_t localindex_index =
        tao::data_dict::findField(_fields, "localindex");
  ...
  void read_coordsANY(hpc::h5::dataset const &dset,
    ...
    uint16_t globalindex_index =
        tao::data_dict::findField(_fields, "global_index");
    std::vector<tao::ANY> buf(chunk_size * n_mems);
    while (n_crds) {
  ...
  count_galaxiesANY(hpc::h5::file &file, int nfields, int snapnum_index) {
    LOGBLOCKI("Counting galaxies.");
    std::vector<unsigned long long> cnts(
        file.dataset("snapshot_redshifts").extent());
    boost::fill(cnts, 0);
    ...
    while (my_n_gals) {
      ...
      LOGILN("Counting chunk from ", my_displ, " to ", my_displ + buf.size(),
             " (", my_n_gals, " remaining)");
  ...
  void tree2sageANY(hpc::fs::path const &tree_fn, hpc::fs::path const &out_fn) {
    LOGBLOCKI(
        "Convert tree h5 file to sage hd5 file suitable for input to init");
    tao::xml_dict sidecar_xml =
        tao::data_dict::getSidecar(tree_fn.native(), "/sageinput");
    std::vector<tao::data_dict_field> fields =
        tao::data_dict::getFieldsANY(sidecar_xml, "/sageinput/Field");
    ...
    std::vector<unsigned long long> gal_cnts =
        count_galaxiesANY(file, nfields, snapnum_index);
    std::vector<unsigned long long> gal_displs =
        hpc::mpi::comm::world.scan(gal_cnts);
    ...
    for (unsigned ii = 0; ii < n_snaps; ++ii) {
      ...
      dsets[ii].create(out_file,
                       std::string("snapshot") + hpc::index_string(ii, 3),
                       _file_type, gsize);
      // ord_dsets[ii].create( out_file, std::string( "snapshot" ) +
      // hpc::index_string( ii, 3 ) + "_map",
    ...
    while (my_n_gals) {
      ...
      int todo = std::min(my_n_gals, chunk_size);
      ubuf.resize(todo * nfields);
      LOGILN("Writing chunk from ", my_displ, " to ", my_displ + todo, " (",
             my_n_gals, " remaining)");
      gals_ds.read(ubuf.data(), _mem_type, todo, my_displ);
      ...
      for (unsigned ii = 0; ii < todo; ++ii) {
        ...
        for (size_t iii = 0; iii < nfields; ++iii) {
          uout_buf(snapnum, out_buf_sizes[snapnum] * nfields + iii) =
              ubuf[ii * nfields + iii];
        }
      ...
      for (unsigned ii = 0; ii < n_snaps; ++ii) {
        if (out_buf_sizes[ii]) {
          dsets[ii].write(uout_buf[ii].data(), _mem_type, out_buf_sizes[ii],
                          gal_displs[ii]);
