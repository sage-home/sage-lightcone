...
struct data_permuter {
  data_permuter() : crds(nullptr), tree_idxs(nullptr), idxs(nullptr) {}
  ...
  data_permuter(std::array<std::vector<double>, 3> &crds,
      ...
      : crds(&crds), tree_idxs(&tree_idxs), idxs(&idxs) {}
...
void sage2kdtree_application::phase1_direct_sage_to_snapshot() {
  if (_verb && _comm->rank() == 0) {
    std::cout << "=== Phase 1 (Direct): Aggregating SAGE snapshots ==="
              << std::endl;
  }
  ...
  if (rank == 0) {
    ...
    hpc::h5::dataset z_dset(out_file, "snapshot_redshifts",
                            hpc::h5::datatype::native_double, z_space,
  ...
  for (size_t snap = 0; snap < _redshifts.size(); ++snap) {
    ...
    for (int i = my_start; i < my_end; ++i) {
      try {
        auto f = std::make_shared<hpc::h5::file>(sage_files[i].string(),
                                                 H5F_ACC_RDONLY);
    ...
    MPI_Allreduce(&local_gals, &total_gals, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM,
                  _comm->mpi_comm());
    MPI_Exscan(&local_gals, &my_offset, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM,
               _comm->mpi_comm());
    if (rank == 0)
      my_offset = 0;
    ...
    if (_verb >= 2 && rank == 0) {
      std::cout << "    Processing " << group_out << " (" << total_gals
                << " gals)" << std::endl;
    }
    ...
    MPI_Allreduce(&info_rank, &global_info_rank, 1, MPI_INT, MPI_MAX,
                  _comm->mpi_comm());
    ...
    for (int k = 0; k < n_fields; ++k) {
      ...
      fields[k].resize(len);
      MPI_Bcast(const_cast<char *>(fields[k].data()), len, MPI_CHAR,
                global_info_rank, _comm->mpi_comm());
    }
    ...
    for (const auto &fname : fields) {
      ...
      hpc::h5::dataset dset(out_group, fname, type, dspace,
                            hpc::h5::property_list());
...
void sage2kdtree_application::operator()() {
  ...
  try {
    ...
    // Replaces Phase 1 & 3
    if (_verb && _comm->rank() == 0) {
      std::cout << "=== Phase 1 (Direct): SAGE HDF5 → Snapshot Organized ==="
                << std::endl;
    }
