...
public:
    SageImportApp(int argc, char *argv[]) : hpc::mpi::application(argc, argv) {
        options().add_options()
            ("settings", hpc::po::value<std::string>(&_settings_file), "XML Settings File")
            ("resume", hpc::po::value<bool>(&_resume)->default_value(false), "Resume Processing");
...
private:
    ...
    void parse_settings() {
        ...
        if (!result) {
            throw std::runtime_error("Failed to parse settings file: " + std::string(result.description()));
        }

        pugi::xml_node root = doc.first_child(); // Settings

        // 1. Parse Sage Fields (Index 0)
        pugi::xml_node sage_fields_node = root.child("sageFields"); // Or by index if tag names vary
        if (!sage_fields_node) sage_fields_node = *root.begin(); // Fallback to first child
        ...
        if (!running_settings) {
             // Fallback by index if needed, but tag search is safer if tags are consistent
             auto it = root.begin();
             std::advance(it, 2);
             running_settings = *it;
        }
        ...
        _config.output_file = running_settings.child("OutputFile").text().as_string();
    ...
    void process() {
        ...
        if (my_rank == 0) {
            ...
            if (!in_file.is_open()) {
                std::cerr << "Rank 0: Failed to open input file: " << _config.input_file << std::endl;
                throw std::runtime_error("Rank 0: Failed to open input file");
            }
            hpc::h5::file out_file(_config.output_file, H5F_ACC_TRUNC);
            if (!out_file.is_open()) {
                std::cerr << "Rank 0: Failed to open output file: " << _config.output_file << std::endl;
    ...
    void create_sidecar_xml() {
        std::string filename = _config.output_file.substr(0, _config.output_file.length() - 3) + ".xml";
        ...
        for (const auto& field : _config.fields) {
            ...
            xml_file << "      group=\"" << field.group << "\">" << field.name << "</Field>\n";
            count++;
        }
    ...
    void process_trees(const std::string& in_fname, const std::string& out_fname, int rank, int n_ranks) {
        hpc::h5::file in_file(in_fname, H5F_ACC_RDONLY, hpc::mpi::comm::world);
        if (!in_file.is_open()) {
            std::cerr << "Rank " << rank << ": Failed to open input file (MPI): " << in_fname << std::endl;
            throw std::runtime_error("Failed to open input file (MPI)");
        }
        hpc::h5::file out_file(out_fname, H5F_ACC_RDWR, hpc::mpi::comm::world);
        if (!out_file.is_open()) {
            std::cerr << "Rank " << rank << ": Failed to open output file (MPI): " << out_fname << std::endl;
        ...
        size_t end_tree = (rank == n_ranks - 1) ? n_trees : (rank + 1) * trees_per_rank;
        ...
        if(!found_descendant) {
            throw std::runtime_error("Descendant field '" + _config.tree_mapping[1] + "' not found in input file");
        }
        ...
        for(size_t t = start_tree; t < end_tree; ++t) {
            ...
            for(int i=0; i<count; ++i) {
                ...
                // Read Descendant
                int descendant_global = *reinterpret_cast<int*>(in_buffer.data() + i * in_struct_size + descendant_offset);
                
                if (descendant_global != -1) {
                    // Descendant in the input file is a LOCAL index (relative to the start of the tree)
            ...
            // Python implementation uses a stack and pushes children in increasing order,
            // which means they are popped in decreasing order.
            // To match this, we should visit progenitors in reverse order (descending index).
            
            // ALSO: Python pushes all roots to the stack in increasing order, so they are popped
            // in decreasing order. So we must iterate roots in reverse for DFS.
            ...
            for(int i=0; i<count; ++i) {
                ...
                extra_ll[0] = start + i; // globaltreeid (Actually Global Galaxy Index based on Python code)
