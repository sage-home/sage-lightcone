/*!
 * @file sageh5toxml.cc
 * @brief Utility to read SAGE HDF5 files and extract Header/Simulation and Snap_ group information
    ...
    void readFirstSnapGroup() {
        ...
        try {
            ...
            if (!first_snap_group.empty()) {
                std::cout << "Found first Snap_ group: " << first_snap_group << std::endl;
                ...
                try {
                    ...
                    std::cout << "Error opening Snap_ group " << first_snap_group << ": " << e.what() << std::endl;
                }
...
private:
    ...
    void readGroupAttributes(hpc::h5::group& group, const std::string& group_path) {
    ...
    void readSingleAttributeByIndex(hid_t group_id, int attr_index) {
        try {
            ...
            hid_t attr_id = H5Aopen_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, attr_index, H5P_DEFAULT, H5P_DEFAULT);
            if (attr_id < 0) {
            ...
            std::cerr << "Error reading attribute " << attr_index << ": " << e.what() << std::endl;
        }
    }
    ...
    void readDatasetAttributes(hpc::h5::group& group, const std::string& group_path) {
        ...
        try {
            ...
            for (hsize_t i = 0; i < num_objects; i++) {
                ...
                std::vector<char> name_buffer(name_size + 1);
                if (H5Lget_name_by_idx(group_id, ".", H5_INDEX_NAME, H5_ITER_INC, i, name_buffer.data(), name_size + 1, H5P_DEFAULT) < 0) {
                    continue;
                }
    ...
    void readSingleDatasetAttributes(hid_t group_id, const std::string& dataset_name) {
        try {
            ...
            std::cerr << "Error reading attributes for dataset " << dataset_name << ": " << e.what() << std::endl;
        }
    }
    
    std::string getDataTypeInfo(hid_t dtype_id) {
        if (dtype_id < 0) return "Unknown";
        ...
        switch (type_class) {
            ...
            case H5T_COMPOUND:
                result = "Compound Type (" + std::to_string(H5Tget_nmembers(dtype_id)) + " members)";
                break;
            case H5T_ARRAY:
                result = "Array Type";
                break;
    ...
    void readSingleDatasetAttribute(hid_t dataset_id, int attr_index) {
        try {
            ...
            std::cerr << "Error reading dataset attribute " << attr_index << ": " << e.what() << std::endl;
        }
    }
};

void printUsage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [options] <hdf5_file>" << std::endl;
    std::cout << std::endl;
    std::cout << "Reads SAGE HDF5 files and extracts information from:" << std::endl;
