...
public:
  application(int argc, char *argv[])
      : hpc::mpi::application(argc, argv), _comm(&hpc::mpi::comm::world) {
    // Setup some options.
    options().add_options()(
  ...
  void convert() {
    ...
    unsigned long long tot_trees = 0, tot_gals = 0;
    {
      ...
      for (size_t ii = idx_rng[0]; ii < idx_rng[1]; ++ii) {
        ...
        for (unsigned jj = 0; jj < files.size(); ++jj) {
          ...
          LOGDLN("File ", jj, " has ", n_gals, " galaxies and ", n_trees,
                 " trees.");
#ifndef NDEBUG
          if (n_c_trees == -1)
            n_c_trees = n_trees;
    ...
    _tree_displs_dset.create(_out_file, "tree_displs",
                             hpc::h5::datatype::native_ullong, tot_trees + 1);
    _tree_cnts_dset.create(_out_file, "tree_counts",
                           hpc::h5::datatype::native_uint, tot_trees);
    ...
    _tree_cnts_buf.create(_tree_cnts_dset, hpc::h5::datatype::native_ullong,
                          hpc::h5::buffer_default_size, _goffs[0]);
    _gals_buf.create(_gals_dset, _mem_type, hpc::h5::buffer_default_size,
                     _goffs[1]);
  ...
  void process_index(size_t idx) {
    ...
    int n_trees;
    {
      for (unsigned ii = 0; ii < files.size(); ++ii) {
        ...
        LOGDLN("File ", ii, " has ", n_gals, " galaxies and ", n_trees,
               " trees.");
        ...
        files[ii].read((char *)file_tree_sizes[ii].data(),
                       n_trees * sizeof(int));
    ...
    std::vector<unsigned long long> tree_displs =
        hpc::counts_to_displs(tree_sizes);
    ...
    for (unsigned ii = 0; ii < n_trees; ++ii) {
      ...
      for (unsigned jj = 0; jj < files.size(); ++jj) {
        ...
        process_tree_redshift(files[jj], file_tree_sizes(jj, ii), h5_gals,
                              displ, desc_map, merge_map);
      }
  ...
  void process_tree_redshift(
      std::ifstream &file, unsigned n_file_gals,
      std::vector<sage::galaxy> &h5_gals, unsigned &displ,
      std::unordered_map<long long, int> &desc_map,
      std::unordered_multimap<hpc::varray<unsigned, 2>, unsigned> &merge_map) {
    ...
    file.read((char *)sage_gals.data(),
              sage_gals.size() * sizeof(GALAXY_OUTPUT));
    ASSERT(file.good(), "Error reading SAGE file.");
    for (unsigned ii = 0; ii < sage_gals.size(); ++ii)
      _convert(sage_gals[ii], h5_gals, displ + ii, ii, _goffs[1] + displ + ii,
               desc_map, merge_map);
  ...
  void check() {
    ...
    for (unsigned ii = tree_rng[0]; ii < tree_rng[1]; ++ii) {
      ...
      unsigned long long displ2 =
          _tree_displs_dset.read<unsigned long long>(ii + 1);
  ...
  void check_tree(std::vector<sage::galaxy> const &gals,
                  unsigned long long displ, unsigned tree_idx) {
    ...
    for (unsigned jj = 0; jj < gals.size(); ++jj) {
      ...
      ASSERT(gals[jj].global_index >= displ &&
                 gals[jj].global_index < displ + gals.size(),
             "Invalid global index: ", gals[jj].global_index);
      ASSERT(gals[jj].global_descendant == -1 ||
                 (gals[jj].global_index >= displ &&
                  gals[jj].global_index < displ + gals.size()),
      ...
      // 	  gals[gals[jj].descendant].snapshot ==
      // gals[jj].merge_into_snapshot), 	 "Incorrect merger: merge into
      // ID=", gals[jj].merge_into_id,
      ...
      if (!(gals[jj].merge_into_id == -1 ||
            (gals[jj].descendant != -1 && gals[gals[jj].descendant].snapshot ==
                                              gals[jj].merge_into_snapshot))) {
        std::cerr << "Warning: incorrect merger detected.\n";
        std::cerr << "  Claimed snapshot = " << gals[jj].merge_into_snapshot
                  << "\n";
        std::cerr << "  Actual snapshot  = "
                  << gals[gals[jj].descendant].snapshot << "\n";
      }
    }
    ASSERT(gids.size() == gals.size(),
           "Duplicate global indices: ", gids.size(), ", ", gals.size());
    ASSERT(*gids.begin() == displ, "Global indices begin at wrong index.");
    ASSERT(*gids.rbegin() == displ + gals.size() - 1,
           "Global indices end at wrong index.");
    for (unsigned jj = 0; jj < gals.size(); ++jj) {
      if (gals[jj].global_descendant != -1) {
        ASSERT(hpc::has(gids, gals[jj].global_descendant),
               "Invalid global descendant: ", gals[jj].global_descendant,
               " from global index: ", gals[jj].global_index);
      }
    }
  }
...
protected:
  void _convert(
      GALAXY_OUTPUT const &sage_gal, std::vector<sage::galaxy> &h5_gals,
      unsigned gal_idx, unsigned file_gal_idx, unsigned long long gidx,
      std::unordered_map<long long, int> &desc_map,
      std::unordered_multimap<hpc::varray<unsigned, 2>, unsigned> &merge_map) {
    LOGBLOCKT("Converting galaxy: ", gal_idx);

    ASSERT(sage_gal.Type < 2,
           "Found an invalid SAGE galaxy type: ", sage_gal.Type);
    ...
    if (sage_gal.mergeIntoID != -1) {
      hpc::varray<unsigned, 2> id{(unsigned)sage_gal.mergeIntoID,
                                  (unsigned)sage_gal.mergeIntoSnapNum};
    ...
    // Check for a merge.
    hpc::varray<unsigned, 2> merge_id{(unsigned)file_gal_idx,
                                      (unsigned)sage_gal.SnapNum};
    if (hpc::has(merge_map, merge_id)) {
      ...
      while (rng.first != rng.second) {
        ...
        ASSERT(
            h5_gals[par].global_descendant == -1 ||
                (h5_gals[par].global_descendant >= _goffs[1] &&
                 h5_gals[par].global_descendant < _goffs[1] + h5_gals.size()),
            "Invalid global descendant.");
    ...
    ASSERT(h5_gals[gal_idx].merge_into_id == -1 ||
               h5_gals[gal_idx].merge_into_id < h5_gals.size(),
           "Invalid merged-into ID: ", h5_gals[gal_idx].merge_into_id);
    ...
    // printf("sage2h5[gal_idx=%u]: pos=(%f,%f,%f), vel=(%f,%f,%f),
    // spin=(%f,%f,%f)\n",
    //        gal_idx,
    //        sage_gal.Pos[0],
    //        sage_gal.Pos[1],
    ...
    for (unsigned kk = 0; kk < 3; ++kk) {
      ...
      ASSERT(h5_gals[gal_idx].pos[kk] == h5_gals[gal_idx].pos[kk],
             "Bad spin: ", "file index=", _fileidx, ", file z=", _filez,
             ", tree index=", _treeidx, ", galaxy index=", gal_idx);
      ASSERT(h5_gals[gal_idx].vel[kk] == h5_gals[gal_idx].vel[kk],
             "Bad spin: ", "file index=", _fileidx, ", file z=", _filez,
             ", tree index=", _treeidx, ", galaxy index=", gal_idx);
      // TODO: Bolshoi has NaN spins for some reason...
      // ASSERT( h5_gals[gal_idx].spin[kk] == h5_gals[gal_idx].spin[kk], "Bad
      // spin: ", 	 "file index=", _fileidx, ", file z=", _filez, ", tree
      // index=", _treeidx,
    ...
    h5_gals[gal_idx].quasar_mode_bh_accretion_mass =
        sage_gal.QuasarModeBHaccretionMass;
  ...
  void _load_param(hpc::fs::path const &fn) {
    ...
    while (file.good()) {
      ...
      if (boost::regex_match(line.c_str(), match, last_prog)) {
        ...
        EXCEPT(_idx_rng[0] != std::numeric_limits<size_t>::max(),
               "Found LastFile before FirstFile.");
      ...
      if (boost::regex_match(line.c_str(), match, box_size_prog)) {
        ...
        EXCEPT(_box_size == std::numeric_limits<double>::max(),
               "Found BoxSize multiple times.");
        _box_size = boost::lexical_cast<double>(
            match[1].first, match[1].second - match[1].first);
      }
      if (boost::regex_match(line.c_str(), match, hubble_prog)) {
        ...
        _hubble = boost::lexical_cast<double>(match[1].first,
