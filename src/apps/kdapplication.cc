...
KdApplication::KdApplication(int argc, char *argv[])
  ...
  hpc::application::options().add_options()(
      "dataset,d", hpc::po::value<std::string>(&_global_cli_dict._dataset),
      "dataset file")(
      "decmin",
      hpc::po::value<double>(&_global_cli_dict._decmin)->default_value(0.0),
      "minimum declination")(
      "decmax",
      hpc::po::value<double>(&_global_cli_dict._decmax)->default_value(10.0),
      "maximum declination")(
      "ramin",
      hpc::po::value<double>(&_global_cli_dict._ramin)->default_value(0.0),
      "minimum right ascension")(
      "ramax",
      hpc::po::value<double>(&_global_cli_dict._ramax)->default_value(10.0),
      "maximum right ascension")(
      "zmin",
      hpc::po::value<double>(&_global_cli_dict._zmin)->default_value(0.0),
      "minimum redshift")(
      "zmax",
      hpc::po::value<double>(&_global_cli_dict._zmax)->default_value(1.0),
      "maximum redshift")(
      "unique",
      hpc::po::value<bool>(&_global_cli_dict._unique)->default_value(false),
      "unique lightcone if true, random lightcone if false")(
      "seed",
      hpc::po::value<int>(&_global_cli_dict._rng_seed)->default_value(0),
      "random number generator seed")(
      "filterfield,f",
      hpc::po::value<std::string>(&_global_cli_dict._filter_field)
          ->default_value("stellar_mass"),
      "field to filter on")(
      "filtermin",
      hpc::po::value<std::string>(&_global_cli_dict._filter_min)
          ->default_value("0"),
      "minimum value for filter field")(
      "filtermax",
      hpc::po::value<std::string>(&_global_cli_dict._filter_max)
          ->default_value(""),
      "maximum value for filter field")(
      "outfile,o",
      hpc::po::value<std::string>(&_global_cli_dict._outfile)
          ->default_value("output.hdf5"),
      "output file name")(
      "outfields",
      hpc::po::value<std::vector<std::string>>(&_global_cli_dict._output_fields)
          ->multitoken(),
      "fields to output")("outdir",
                          hpc::po::value<std::string>(&_global_cli_dict._outdir)
  ...
  if (mpi::comm::world.rank() == 0) {
    ...
    if (validate(_global_cli_dict)) {
      if (!exists(_global_cli_dict._outdir)) {
        std::cout << "Creating output directory: " << _global_cli_dict._outdir
                  << std::endl;
...
bool KdApplication::validate(const cli_dict &_global_cli_dict) {
  ...
  if (_global_cli_dict._zmin < 0.0) {
    std::cerr << "Minimum redshift must be greater than or equal to zero."
              << std::endl;
    return false;
  }
  if (_global_cli_dict._zmax <= _global_cli_dict._zmin) {
    std::cerr << "Minimum redshift must be less than Maximum redshift."
              << std::endl;
    return false;
  }
  ...
  if (_global_cli_dict._filter_min.empty() &&
      _global_cli_dict._filter_max.empty()) {
    std::cerr << "Error: --filtermin and --filtermax cannot both be empty."
              << std::endl;
    return false;
  }
  if (_global_cli_dict._filter_min == _global_cli_dict._filter_max) {
    std::cerr << "Error: --filtermin and --filtermax cannot be the same."
              << std::endl;
    return false;
  }
  return true;
...
void KdApplication::operator()() {
  ...
  LOG_PUSH(new mpi::logger(
      _global_cli_dict._outdir + "/log/debug.log." + subjobindex, log::debug));
  ...
  if (mpi::comm::world.rank() == 0) {
    if (mpi::comm::world.size() > 1) {
      ...
      tao::modules::hdf5<backends::kdtree_backend>::process_cli_options(
          _global_cli_dict);
    }
    // If we are the root rank, we need to gather results from all ranks.
    // Close the log file.
...
void KdApplication::load_and_connect_modules() {
  ...
  LOGILN("Registering modules from command line. Hardcoded to be lightcone and "
         "hdf5.");
  _fact.create_module("light-cone", "lightcone", pugi::xml_node());
  _fact.create_module("hdf5", "hdf5", pugi::xml_node());
...
///
void KdApplication::preprocess_xml() {
  _currentxml_version = "1.0";
  return;
}
