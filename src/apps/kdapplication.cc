...
namespace tao {
    ...
    KdApplication::KdApplication(int argc, char *argv[]) : mpi::application(argc, argv) {
        // Setup some options.
        hpc::application::options().add_options()
            ("dataset,d", hpc::po::value<std::string>(&_global_cli_dict._dataset), "dataset file")
            ("decmin",hpc::po::value<double>(&_global_cli_dict._decmin)->default_value(0.0), "minimum declination")
            ("decmax",hpc::po::value<double>(&_global_cli_dict._decmax)->default_value(10.0), "maximum declination")
            ("ramin",hpc::po::value<double>(&_global_cli_dict._ramin)->default_value(0.0), "minimum right ascension")
            ("ramax",hpc::po::value<double>(&_global_cli_dict._ramax)->default_value(10.0), "maximum right ascension")
            ("zmin",hpc::po::value<double>(&_global_cli_dict._zmin)->default_value(0.0), "minimum redshift")
            ...
            ("outfields", hpc::po::value<std::vector<std::string>>(&_global_cli_dict._output_fields)->multitoken(), "fields to output")
            ("outdir", hpc::po::value<std::string>(&_global_cli_dict._outdir)->default_value("output"), "directory for output files")
        ...
        if (mpi::comm::world.rank() == 0) {
            if (_global_cli_dict._unique)
                ...
                std::cout << "random lightcone "<<" using seed="<<_global_cli_dict._rng_seed<<std::endl;
        }
    ...
    void KdApplication::operator()() {
        ...
        LOG_PUSH(new mpi::logger(_global_cli_dict._outdir+"/log/debug.log." + subjobindex, log::debug));
    ...
    void KdApplication::load_and_connect_modules() {
        ...
        LOGILN("Registering modules from command line. Hardcoded to be lightcone and hdf5.");
