...
struct data_permuter {
  data_permuter()
      : crds(nullptr), tree_idxs(nullptr), subsize(nullptr), idxs(nullptr) {}
  ...
  data_permuter(std::array<std::vector<double>, 3> &crds,
      ...
      : crds(&crds), tree_idxs(&tree_idxs), subsize(&subsize), idxs(&idxs) {}
...
std::string read_dataset_string_attribute(hpc::h5::location &loc,
...
void write_dataset_string_attribute(hpc::h5::location &loc,
                                    const std::string &dataset_path,
                                    const std::string &attr_name,
  ...
  hid_t attr_id = H5Acreate2(dset_id, attr_name.c_str(), str_type, space_id,
...
private:
  ...
  unsigned
  _depthfirst_recurse(unsigned idx, hpc::view<std::vector<TreeNode>> &gals,
  ...
  void _write_attributes(hpc::h5::file &file, std::string const &snap_name,
                         ...
                         unsigned long long displ);
  void _write_kdtree(
      hpc::h5::file &file, std::string const &snap_name,
  ...
  //==========================================================================
  void
  _validate_inputs(); // Validate all inputs before processing (Priority 1.1)
...
sage2kdtree_application::sage2kdtree_application(int argc, char *argv[])
    : hpc::mpi::application(argc, argv),
      _comm(const_cast<hpc::mpi::comm *>(&hpc::mpi::comm::world)), _box_size(0),
      _hubble(0), _omega_l(0), _omega_m(0), _ppc(1000), _verb(1) {

  // Setup command-line options
  options().add_options()("sage,s",
                          hpc::po::value<hpc::fs::path>(&_sage_dir)->required(),
...
void sage2kdtree_application::operator()() {
  ...
  try {
    ...
    if (_verb && _comm->rank() == 0) {
      std::cout << "=== Phase 1: SAGE HDF5 → Depth-first ordered ==="
                << std::endl;
    }
    ...
    metrics.push_back({"Phase 2",
                       std::chrono::duration<double>(p2_end - p2_start).count(),
                       get_peak_rss_kb()});
    ...
    metrics.push_back({"Phase 3",
                       std::chrono::duration<double>(p3_end - p3_start).count(),
                       get_peak_rss_kb()});
    ...
    if (_verb && _comm->rank() == 0) {
      std::cout << "\n=== Phase 4: Building KD-tree spatial index ==="
                << std::endl;
    }
    ...
    metrics.push_back({"Phase 4",
                       std::chrono::duration<double>(p4_end - p4_start).count(),
                       get_peak_rss_kb()});
    ...
    if (_verb && _comm->rank() == 0) {
      ...
      std::cout << std::left << std::setw(10) << "Phase" << std::right
                << std::setw(15) << "Time (s)" << std::right << std::setw(15)
                << "Peak RSS (KB)" << std::endl;
      ...
      for (const auto &m : metrics) {
        std::cout << std::left << std::setw(10) << m.name << std::right
                  ...
                  << m.duration << std::right << std::setw(15) << m.peak_rss
                  << std::endl;
      ...
      std::cout << std::left << std::setw(10) << "Total" << std::right
                << std::setw(15) << std::fixed << std::setprecision(2)
                << total_time << std::right << std::setw(15)
                << get_peak_rss_kb() << std::endl;
    }
...
void sage2kdtree_application::_validate_inputs() {
  ...
  if (!hpc::fs::exists(_sage_dir)) {
    ...
    errors.push_back("SAGE output path is not a directory: " +
                     _sage_dir.string());
  }
  ...
  if (!found_hdf5_files) {
    errors.push_back(
        "No HDF5 files found in SAGE directory: " + _sage_dir.string() +
  ...
  const size_t min_file_size =
      100000; // 100KB - data files are typically much larger
        ...
        if (file_size < min_file_size) {
          if (_verb >= 2) {
            std::cout << "    Skipping small file (likely master): "
                      << dir_itr->path().filename() << " (" << file_size
                      << " bytes)" << std::endl;
          }
          continue;
        }
  ...
  if (!first_hdf5.empty()) {
    if (_verb >= 2) {
      std::cout << "    Validating file: " << first_hdf5.filename()
                << std::endl;
    }
    ...
    try {
      ...
      if (!has_header_simulation) {
        ...
        std::vector<std::string> required_attrs = {"BoxSize", "Hubble_h",
        ...
        if (!missing_attrs.empty()) {
          ...
          warnings.push_back(
              "Missing cosmology attributes in Header/Simulation: " +
              missing_list +
      ...
      if (snap_count == 0) {
        errors.push_back("No Snap_N groups found in " +
                         first_hdf5.filename().string() +
        ...
        if (!missing_fields.empty()) {
          ...
          errors.push_back(
              "Missing required fields in Snap_0: " + missing_list +
  ...
  if (param_file.good()) {
    ...
    std::vector<std::string> recommended_params = {
        "Omega_Lambda", "OmegaLambda", "Omega_m", "Omega"};
    ...
    bool has_omega_lambda =
        (found_params.find("Omega_Lambda") != found_params.end() ||
  ...
  if (alist_file.good()) {
    ...
    if (line_count == 0) {
      errors.push_back(
          "Expansion factor list is empty: " + _alist_fn.filename().string() +
          "\n      Suggestion: File should contain one scale factor per "
          "snapshot");
    } else {
      if (_verb >= 2)
        std::cout << "    Found " << line_count << " scale factors"
                  << std::endl;
    }
  }
...
void sage2kdtree_application::phase1_sageh5_to_depthfirst() {
  auto log_mem = [&](const std::string &stage) {
    if (_verb >= 1 && _comm->rank() == 0) {
      ...
      std::cout << "  [MEM] " << stage << " - Current: " << (curr / 1024.0)
                << " MB, Peak: " << (peak / 1024.0) << " MB" << std::endl;
    }
  };
  ...
  if (hpc::fs::is_directory(_sage_dir)) {
    ...
    for (hpc::fs::directory_iterator dir_itr(_sage_dir); dir_itr != end_iter;
      ...
      if (hpc::fs::is_regular_file(dir_itr->status())) {
        ...
        std::string stem = dir_itr->path().stem().string();
        if (ext == ".hdf5" &&
            (stem.find("model_hdf5_") == 0 || stem.find("model_") == 0)) {
  ...
  std::sort(input_files.begin(),
            input_files.end()); // Ensure deterministic order
  ...
  if (_verb && _comm->rank() == 0) {
    std::cout << "  → Scanning files to count trees and galaxies..."
              << std::endl;
  }
  ...
  for (size_t file_idx = 0; file_idx < my_files.size(); ++file_idx) {
    ...
    if (_verb >= 2 && _comm->rank() == 0) {
      std::cout << "    Scanning file " << (file_idx + 1) << "/"
                << my_files.size() << ": " << fn.filename() << std::endl;
    }
    ...
    try {
      ...
      throw PipelineException(1, "Failed to open SAGE HDF5 file '" +
                                     fn.string() +
                                     "': " + std::string(e.what()));
    }
    ...
    if (f.has_link("Snap_63/SAGETreeIndex")) {
      ...
      if (!tree_indices.empty()) {
        int max_tree_idx =
            *std::max_element(tree_indices.begin(), tree_indices.end());
  ...
  if (_verb && _comm->rank() == 0) {
    std::cout << "  → Found " << total_trees << " trees with " << total_gals
              << " galaxies" << std::endl;
  }
  ...
  if (!input_files.empty()) {
    ...
    for (int snap = 0; snap < 64; ++snap) {
      ...
      for (hsize_t i = 0; i < num_objs; ++i) {
        ...
        std::string desc = read_dataset_string_attribute(
            snap_group, sage_field_name, "Description");
        std::string units =
            read_dataset_string_attribute(snap_group, sage_field_name, "Units");

        // Use SAGE field name directly (preserve CamelCase)
        // Use SAGE field name as fallback for description
        if (desc.empty())
          desc = sage_field_name;
    ...
    if (_verb >= 2 && _comm->rank() == 0) {
      std::cout << "  → Loaded metadata for " << field_metadata.size()
                << " fields" << std::endl;
    }
    ...
    if (!tao::validate_sage_fields(available_fields, missing_fields)) {
      if (_comm->rank() == 0) {
        std::cerr << "\n❌ ERROR: SAGE HDF5 input is missing mandatory fields:"
                  << std::endl;
        for (const auto &field : missing_fields) {
          std::cerr << "  - " << field << std::endl;
        }
        std::cerr << "\nPlease check that your SAGE HDF5 files contain all "
                     "required fields."
                  << std::endl;
        std::cerr
            << "See FIELD_NAMING.md for the complete list of mandatory fields."
            << std::endl;
      }
  ...
  // Note: SnapNum from SAGE will be accessible via lowercase "snapnum" key in
  // the map
  field_list.push_back({"local_index", hpc::h5::datatype::native_int, 0,
                        "Local index within snapshot", ""});
  field_list.push_back({"global_index", hpc::h5::datatype::native_llong, 0,
                        "Global index across all galaxies", ""});
  field_list.push_back({"descendant", hpc::h5::datatype::native_int, 0,
                        "Local index of descendant", ""});
  field_list.push_back({"global_descendant", hpc::h5::datatype::native_llong, 0,
                        "Global index of descendant", ""});
  // NOTE: "subsize" removed - Phase 1 doesn't write it. Phase 2 creates
  // "subtree_count" instead.

  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "  → Using columnar storage with " << field_list.size()
              << " fields" << std::endl;
  }
  ...
  // Use shared_ptr to avoid issues with dataset handle invalidation during map
  // operations
  std::map<std::string, std::shared_ptr<hpc::h5::dataset>> field_datasets;

  for (const auto &field : field_list) {
    // Store in map with lowercase key for lookup, but HDF5 dataset uses actual
    // name
    std::string lookup_key = field.name;
    std::transform(lookup_key.begin(), lookup_key.end(), lookup_key.begin(),
                   ::tolower);
    ...
    if (!field.description.empty()) {
      write_dataset_string_attribute(out_file, "fields/" + field.name,
                                     "Description", field.description);
    }
    if (!field.units.empty()) {
      write_dataset_string_attribute(out_file, "fields/" + field.name, "Units",
                                     field.units);
    }
  }

  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "  → Created " << field_datasets.size()
              << " dataset entries in map" << std::endl;
  }
  ...
  // REMOVED: all_int_fields, all_llong_fields, all_float_fields (now using
  // batching)

  // Tree metadata datasets (unchanged)
  hpc::h5::dataset tree_displs_dset(out_file, "tree_displs",
                                    hpc::h5::datatype::native_ullong,
                                    hpc::h5::dataspace(total_trees + 1));
  hpc::h5::dataset tree_cnts_dset(out_file, "tree_counts",
                                  hpc::h5::datatype::native_uint,
  ...
  // 4. Process Files
  if (_verb && _comm->rank() == 0) {
    std::cout << "  → Processing " << my_files.size()
              << " files and applying depth-first ordering..." << std::endl;
  }

  unsigned long long current_gal_global_idx = global_gal_offset;
  unsigned long long current_tree_displ = global_gal_offset;
  unsigned long long current_gal_write_offset =
      global_gal_offset; // For writing fields

  // Memory constraint: Process trees in batches
  const size_t MAX_GALAXIES =
      50000; // Adjust as needed (e.g. 20M galaxies per batch)

  for (size_t fidx = 0; fidx < my_files.size(); ++fidx) {
    hpc::fs::path fn = my_files[fidx];

    if (_verb >= 2 && _comm->rank() == 0) {
      std::cout << "    Processing file " << (fidx + 1) << "/"
                << my_files.size() << ": " << fn.filename() << " ("
                << file_tree_counts[fidx] << " trees, " << file_gal_counts[fidx]
                << " galaxies)" << std::endl;
    }
    ...
    // We need to re-scan SAGETreeIndex to know how many galaxies are in each
    // tree This adds a pass over the file but is necessary for
    ...
    if (_verb >= 2 && _comm->rank() == 0) {
      std::cout << "      Split into " << batches.size() << " batches (max "
                << (MAX_GALAXIES / 1000000.0) << "M galaxies/batch)"
                << std::endl;
    }
    ...
    for (size_t b_idx = 0; b_idx < batches.size(); ++b_idx) {
      ...
      if (_verb >= 2 && _comm->rank() == 0) {
        log_mem("Start Batch " + std::to_string(b_idx + 1) + "/" +
      ...
      for (int snap = 0; snap < 64; ++snap) {
        ...
        auto read_col = [&](const char *name, auto &vec) {
          ...
          if (g.has_link(name)) {
            ...
            std::fill(vec.begin(), vec.end(),
                      0); // Handle missing columns with 0
          }
        };
        ...
        for (const auto &entry : field_metadata) {
          ...
          } else if (H5Tequal(dtype.id(),
                              hpc::h5::datatype::native_llong.id()) > 0 ||
                     H5Tequal(dtype.id(),
                              hpc::h5::datatype::native_ullong.id()) > 0) {
        ...
        for (size_t i = 0; i < n_gals_snap; ++i) {
          ...
          if (tree_idx >= (int)batch.start_tree &&
              tree_idx < (int)batch.end_tree) {
            ...
            size_t gal_idx_in_tree =
                trees[local_tree_idx].size() - 1; // Just added

            trees_all_fields[local_tree_idx].resize(gal_idx_in_tree + 1);
            GalaxyFields &gf =
                trees_all_fields[local_tree_idx][gal_idx_in_tree];
      ...
      for (const auto &field : field_list) {
        ...
        if (H5Tequal(field.type.id(), hpc::h5::datatype::native_int.id()) > 0 ||
            H5Tequal(field.type.id(), hpc::h5::datatype::native_uint.id()) >
                0) {
          batch_int_fields[key].reserve(batch.total_gals);
        } else if (H5Tequal(field.type.id(),
                            hpc::h5::datatype::native_llong.id()) > 0 ||
                   H5Tequal(field.type.id(),
                            hpc::h5::datatype::native_ullong.id()) > 0) {
      ...
      // Track actual number of galaxies processed (may differ from
      // batch.total_gals if trees are empty)
      ...
      for (size_t t = 0; t < batch_n_trees; ++t) {
        ...
        for (size_t i = 0; i < tree_gals.size(); ++i) {
          ...
          if (g.merge_into_id != -1) {
            hpc::varray<unsigned, 2> id{(unsigned)g.merge_into_id,
                                        (unsigned)g.merge_into_snapshot};
        ...
        for (size_t i = 0; i < tree_gals.size(); ++i)
          tree_gals[i].original_index = i;

        // Reorder galaxies
        _depthfirst_ordering(hpc::view<std::vector<TreeNode>>(tree_gals));

        // Accumulate ALL field data for this tree into memory (in reordered
        // sequence)
        ...
        for (size_t new_idx = 0; new_idx < n_gals; ++new_idx) {
          ...
          batch_int_fields["local_index"].push_back(
              (int)new_idx); // Match original behavior (reordered index 0..N-1)
          batch_llong_fields["global_index"].push_back(g.global_index);
          batch_llong_fields["global_descendant"].push_back(
              g.global_descendant);
          ...
          static const std::set<std::string> computed_fields = {
              "snapnum", "descendant", "local_index", "global_index",
              "global_descendant"};
          ...
          for (const auto &entry : gf.int_fields) {
            std::string key = entry.first;
            std::transform(key.begin(), key.end(), key.begin(), ::tolower);
            if (computed_fields.count(key) ==
                0) { // Skip if it's a computed field
      ...
      // Write batch to HDF5
      if (_verb >= 2 && _comm->rank() == 0) {
        std::cout << "      Writing batch " << (b_idx + 1) << " ("
                  << actual_batch_gals << " galaxies)..." << std::endl;
      }
      ...
      if (actual_batch_gals != batch.total_gals && _comm->rank() == 0) {
        std::cout << "      ⚠ WARNING: actual_batch_gals (" << actual_batch_gals
                  << ") != batch.total_gals (" << batch.total_gals << ")"
                  << std::endl;
      }
      ...
      for (const auto &entry : batch_int_fields) {
        ...
        if (it != field_datasets.end() && it->second) {
          ...
          file_space.select_hyperslab(H5S_SELECT_SET,
                                      (hsize_t)actual_batch_gals,
                                      (hsize_t)current_gal_write_offset);
          it->second->write(data.data(), hpc::h5::datatype::native_int,
      ...
      for (const auto &entry : batch_llong_fields) {
        ...
        if (it != field_datasets.end() && it->second) {
          ...
          file_space.select_hyperslab(H5S_SELECT_SET,
                                      (hsize_t)actual_batch_gals,
                                      (hsize_t)current_gal_write_offset);
          it->second->write(data.data(), hpc::h5::datatype::native_llong,
                            mem_space, file_space);
        }
      }
      ...
      for (const auto &entry : batch_float_fields) {
        ...
        if (it != field_datasets.end() && it->second) {
          ...
          file_space.select_hyperslab(H5S_SELECT_SET,
                                      (hsize_t)actual_batch_gals,
                                      (hsize_t)current_gal_write_offset);
          it->second->write(data.data(), hpc::h5::datatype::native_float,
                            mem_space, file_space);
        }
      }
    ...
    if (_verb >= 2 && _comm->rank() == 0) {
      log_mem("After processing file " + std::to_string(fidx + 1) + "/" +
              std::to_string(my_files.size()));
    }
  }
  ...
  if (_comm->rank() == _comm->size() - 1) {
    ...
    tree_displs_dset.write(&total_gals, hpc::h5::datatype::native_ullong,
                           mem_space, file_space);
  }
...
void sage2kdtree_application::_load_param(hpc::fs::path const &fn) {
  ...
  if (!file.good()) {
    throw PipelineException(
        1, "Failed to open parameter file: " + fn.string() +
  ...
  if (_box_size <= 0.0 || _hubble <= 0.0) {
    throw PipelineException(
        1, "Invalid or missing cosmology parameters in " +
               fn.filename().string() +
  ...
  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "    Loaded: BoxSize=" << _box_size
              << ", Hubble_h=" << _hubble / 100.0 << ", Omega_m=" << _omega_m
...
void sage2kdtree_application::_load_redshifts(hpc::fs::path const &fn) {
  ...
  if (!file.good()) {
    throw PipelineException(
        1, "Failed to open expansion factor list: " + fn.string() +
               "\n      Check that the file exists and is readable");
  }
  ...
  while (std::getline(file, line)) {
    ...
    if (a <= 0.0 || a > 1.0) {
      throw PipelineException(
          1, "Invalid scale factor at line " + std::to_string(line_num) +
                 " in " + fn.filename().string() + ": a=" + std::to_string(a) +
  ...
  if (_redshifts.empty()) {
    throw PipelineException(
        1, "No valid scale factors found in " + fn.string() +
  ...
  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "    Loaded " << _redshifts.size() << " redshifts "
              << "(z_min="
              << *std::min_element(_redshifts.begin(), _redshifts.end())
              << ", z_max="
              << *std::max_element(_redshifts.begin(), _redshifts.end()) << ")"
              << std::endl;
  }
}

void sage2kdtree_application::_depthfirst_ordering(
    hpc::view<std::vector<TreeNode>> gals) {
  // Cache the smallest and largest global index
  std::array<unsigned long long, 2> gidxs{
      (unsigned long long)gals.front().global_index,
...
unsigned sage2kdtree_application::_depthfirst_recurse(
    unsigned idx, hpc::view<std::vector<TreeNode>> &gals,
    std::vector<unsigned> &map, unsigned &dfi,
    std::multimap<unsigned, unsigned> const &parents) {
  map[idx] = dfi++;
  gals[idx].subsize = 1;
  ...
  while (rng.first != rng.second) {
    gals[idx].subsize +=
        _depthfirst_recurse(rng.first->second, gals, map, dfi, parents);
    ++rng.first;
  }
...
void sage2kdtree_application::phase2_add_traversal_metadata() {
  ...
  if (_verb >= 2 && my_rank == 0) {
    std::cout << "  → Found " << phase1_fields.size()
              << " fields in Phase 1 output" << std::endl;
  }
  ...
  // Create output columnar datasets (6 original + 5 new traversal fields = 11
  // total)
  ...
  H5Pset_fill_time(no_fill_props.id(),
                   H5D_FILL_TIME_NEVER); // Critical for Lustre!

  if (_verb >= 2 && my_rank == 0) {
    std::cout << "  → Creating contiguous datasets (no fill, no chunking)"
              << std::endl;
  }
  ...
  for (const auto &field : phase1_fields) {
    out_datasets[field.name] = std::make_shared<hpc::h5::dataset>(
        out_file, "fields/" + field.name, field.type,
        hpc::h5::dataspace(total_gals), no_fill_props);

    // Copy attributes from Phase 1 input
    std::string desc = read_dataset_string_attribute(
        in_file, "fields/" + field.name, "Description");
    std::string units =
        read_dataset_string_attribute(in_file, "fields/" + field.name, "Units");
  ...
  out_datasets["globaltreeid"] = std::make_shared<hpc::h5::dataset>(
      out_file, "fields/globaltreeid", hpc::h5::datatype::native_llong,
      hpc::h5::dataspace(total_gals), no_fill_props);
  out_datasets["breadthfirst_traversalorder"] =
      std::make_shared<hpc::h5::dataset>(
          out_file, "fields/breadthfirst_traversalorder",
          hpc::h5::datatype::native_llong, hpc::h5::dataspace(total_gals),
          no_fill_props);
  out_datasets["depthfirst_traversalorder"] =
      std::make_shared<hpc::h5::dataset>(
          out_file, "fields/depthfirst_traversalorder",
          hpc::h5::datatype::native_llong, hpc::h5::dataspace(total_gals),
          no_fill_props);
  out_datasets["subtree_count"] = std::make_shared<hpc::h5::dataset>(
      out_file, "fields/subtree_count", hpc::h5::datatype::native_llong,
      hpc::h5::dataspace(total_gals), no_fill_props);
  out_datasets["localgalaxyid"] = std::make_shared<hpc::h5::dataset>(
      out_file, "fields/localgalaxyid", hpc::h5::datatype::native_int,
      hpc::h5::dataspace(total_gals), no_fill_props);
  ...
  for (const auto &kv : traversal_metadata) {
    write_dataset_string_attribute(out_file, "fields/" + kv.first,
                                   "Description", kv.second.first);
    if (!kv.second.second.empty()) {
      write_dataset_string_attribute(out_file, "fields/" + kv.first, "Units",
                                     kv.second.second);
    }
  }
  ...
  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "  → Copying " << phase1_fields.size()
              << " fields from Phase 1 output..." << std::endl;
  }
  ...
  for (const auto &field : phase1_fields) {
    ...
    if (_verb >= 2 && _comm->rank() == 0) {
      ...
      std::cout << "    Copying field: " << fname << " [" << type_name << ", "
                << type_size << " bytes, "
                << (n_loc * type_size / 1024.0 / 1024.0) << " MB] ..."
                << std::flush;
    }
    ...
    if (type_class == H5T_INTEGER && type_size == 4) {
      ...
      in_datasets.at(fname).read(field_data.data(), dtype, mem_space,
                                 file_space);
      if (_verb >= 2 && _comm->rank() == 0) {
        std::cout << " write..." << std::flush;
      }
      out_datasets[fname]->write(field_data.data(), dtype, mem_space,
                                 file_space);
  ...
  size_t end_tree =
      (my_rank == n_ranks - 1) ? n_trees : (my_rank + 1) * trees_per_rank;
  ...
  for (size_t b_idx = 0; b_idx < batches.size(); ++b_idx) {
    ...
    if (_verb >= 2 && _comm->rank() == 0) {
      if (batches.size() > 1) {
        std::cout << "    Processing batch " << (b_idx + 1) << "/"
                  << batches.size() << " (" << batch_n_trees << " trees, "
    ...
    long long batch_total_gals =
        tree_offsets[batch.end_tree] - batch_start_offset;
    ...
    batch_file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)batch_total_gals,
                                      (hsize_t)batch_start_offset);
    get_dataset("descendant")
        .read(descendant_batch.data(), hpc::h5::datatype::native_int,
              batch_mem_space, batch_file_space);
    ...
    for (size_t t = batch.start_tree; t < batch.end_tree; ++t) {
      ...
      // Build tree structure using descendant links (local to this tree within
      // batch)
    ...
    out_datasets["globaltreeid"]->write(globaltreeid_batch.data(),
                                        hpc::h5::datatype::native_llong,
                                        batch_mem_space, batch_file_space);
    out_datasets["breadthfirst_traversalorder"]->write(
        bfs_order_batch.data(), hpc::h5::datatype::native_llong,
        batch_mem_space, batch_file_space);
    out_datasets["depthfirst_traversalorder"]->write(
        dfs_order_batch.data(), hpc::h5::datatype::native_llong,
        batch_mem_space, batch_file_space);
    out_datasets["subtree_count"]->write(subtree_count_batch.data(),
                                         hpc::h5::datatype::native_llong,
                                         batch_mem_space, batch_file_space);
    out_datasets["localgalaxyid"]->write(localgalaxyid_batch.data(),
                                         hpc::h5::datatype::native_int,
                                         batch_mem_space, batch_file_space);
...
void sage2kdtree_application::phase3_tree_to_snapshot() {
  if (_verb >= 1 && _comm->rank() == 0) {
    std::cout
        << "Phase 3: Converting tree order to snapshot order (columnar)..."
        << std::endl;
  }
  ...
  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "  → Found " << num_fields << " fields in Phase 2 output"
              << std::endl;
  }
  ...
  std::vector<unsigned long long> snap_counts(64, 0);
  {
    ...
    file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)n_loc, (hsize_t)offs);
    get_dataset("snapnum").read(snapnums.data(), hpc::h5::datatype::native_int,
                                mem_space, file_space);
    ...
    _comm->all_reduce(hpc::view<std::vector<unsigned long long>>(snap_counts),
                      MPI_SUM);
  }

  unsigned long long total_from_counts = 0;
  for (auto cnt : snap_counts)
    total_from_counts += cnt;
  ...
  for (int snap = 0; snap < 64; ++snap) {
    ...
    for (const auto &fname : all_field_names) {
      ...
      snap_datasets[snap][fname] =
          hpc::h5::dataset(snap_group, fname, field_types[fname], dspace);

      // Copy attributes from Phase 2 input
      std::string desc = read_dataset_string_attribute(
          tree_file, "fields/" + fname, "Description");
      std::string units =
          read_dataset_string_attribute(tree_file, "fields/" + fname, "Units");

      std::string snap_path = ss.str() + "/" + fname;
      if (!desc.empty())
        write_dataset_string_attribute(out_file, snap_path, "Description",
                                       desc);
      if (!units.empty())
        write_dataset_string_attribute(out_file, snap_path, "Units", units);
    }
  }
  ...
  for (const auto &fname : all_field_names) {
    ...
    snapnum_file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)n_loc,
                                        (hsize_t)offs);
    get_dataset("snapnum").read(snapnums.data(), hpc::h5::datatype::native_int,
                                mem_space, snapnum_file_space);
    ...
    for (unsigned long long i = 0; i < n_loc; ++i) {
      ...
      if (snap >= 0 && snap < 64) {
        ...
        snap_buffers[snap].insert(snap_buffers[snap].end(), elem_ptr,
                                  elem_ptr + elem_size);
      }
    }
    ...
    for (int snap = 0; snap < 64; ++snap) {
      ...
      if (local_count > 0) {
        ...
        hpc::h5::dataspace write_file_space =
            snap_datasets[snap][fname].dataspace();
        write_file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)local_count,
                                          (hsize_t)my_offset);
        snap_datasets[snap][fname].write(snap_buffers[snap].data(), dtype,
                                         write_mem_space, write_file_space);
      } else {
        ...
        snap_datasets[snap][fname].write(nullptr, dtype, write_mem_space,
                                         write_file_space);
      }
    }
  }
  ...
  if (_comm->rank() == 0) {
    try {
      hpc::h5::copy(tree_file, "cosmology", out_file);
    } catch (...) {
    }
    try {
      hpc::h5::copy(tree_file, "snapshot_redshifts", out_file);
    } catch (...) {
    }
  }
...
void sage2kdtree_application::phase4_build_kdtree_index() {
  ...
  std::vector<unsigned long long> snap_displs(n_snapshots +
                                              1); // n_snapshots + 1 final total
  ...
  if (_verb >= 2 && _comm->rank() == 0) {
    std::cout << "  Total galaxies across all snapshots: " << total_gals
              << std::endl;
  }

  // Create lightcone/data compound dataset
  hpc::h5::datatype lc_type(
      H5Tcreate(H5T_COMPOUND, sizeof(double) * 3 + sizeof(unsigned long long) +
                                  sizeof(unsigned)));
  ...
  lc_type.insert(uint_type, "subsize",
                 sizeof(double) * 3 + sizeof(unsigned long long));
  ...
  if (_comm->rank() == 0) {
    write_dataset_string_attribute(
        out_file, "lightcone/data", "Description",
        "Compound dataset with galaxy coordinates and metadata");
    write_dataset_string_attribute(out_file, "lightcone/data", "x_units",
                                   "Mpc/h");
    write_dataset_string_attribute(out_file, "lightcone/data", "y_units",
                                   "Mpc/h");
    write_dataset_string_attribute(out_file, "lightcone/data", "z_units",
                                   "Mpc/h");
  }
  ...
  if (_comm->rank() == 0 && total_gals > 0) {
    if (_verb >= 2) {
      std::cout << "  Discovering fields and creating data/* datasets..."
                << std::endl;
    }
    for (int snap = 0; snap < n_snapshots; ++snap) {
      std::ostringstream ss;
      ...
      if (snap_counts[snap] > 0 && snap_file.has_link(ss.str())) {
        ...
        for (hsize_t ii = 0; ii < num_objs; ++ii) {
          char name_buf[256];
          H5Gget_objname_by_idx(snap_group.id(), ii, name_buf,
                                sizeof(name_buf));
          std::string field_name(name_buf);
          ...
          std::string desc = read_dataset_string_attribute(
              snap_file, field_path, "Description");
          std::string units =
              read_dataset_string_attribute(snap_file, field_path, "Units");

          if (!desc.empty())
            write_dataset_string_attribute(out_file, "data/" + out_name,
                                           "Description", desc);
          if (!units.empty())
            write_dataset_string_attribute(out_file, "data/" + out_name,
                                           "Units", units);
  ...
  if (_comm->rank() == 0) {
    ...
    hpc::h5::dataset counts_ds(out_file, "snapshot_counts", ulonglong_type,
                               counts_space, hpc::h5::property_list());
    counts_ds.write(snap_counts);

    hpc::h5::dataspace displs_space(n_snapshots +
                                    1); // n_snapshots + 1 final total
    hpc::h5::dataset displs_ds(out_file, "snapshot_displs", ulonglong_type,
                               displs_space, hpc::h5::property_list());
...
void sage2kdtree_application::_process_snapshot(hpc::h5::file &file,
  ...
  hpc::binary_partitioner<std::array<std::vector<double>, 3>::iterator,
                          data_permuter>
      bp(crds.begin(), crds.end(), dp, _ppc);
  hpc::kdtree<> kdt;
  kdt.construct(bp);
  ...
  // Write global_index
  {
    ...
    hpc::h5::property_list props(H5P_DATASET_XFER);
    props.set_preserve();
    data.write(tree_idxs.data(), dt, n_gals, displ, hpc::mpi::comm::self,
               props);
  }
...
void sage2kdtree_application::_read_coords(
  ...
  std::string globaltreeid_path =
      find_field("globaltreeid", "globaltreeid"); // already lowercase
  std::string subtree_count_path =
      find_field("subtree_count", "subtree_count"); // already lowercase
  ...
  posx_ds.read(posx_float.data(), hpc::h5::datatype::native_float, mem_space,
               sp);
  file.dataset(posy_path).read(posy_float.data(),
                               hpc::h5::datatype::native_float, mem_space, sp);
  file.dataset(posz_path).read(posz_float.data(),
                               hpc::h5::datatype::native_float, mem_space, sp);
  ...
  file.dataset(globaltreeid_path)
      .read(globaltreeid_data.data(), hpc::h5::datatype::native_llong,
            mem_space, sp);
  file.dataset(subtree_count_path)
      .read(subtree_count_data.data(), hpc::h5::datatype::native_llong,
            mem_space, sp);
...
void sage2kdtree_application::_write_attributes(
    ...
    std::vector<unsigned long long> const &idxs, unsigned long long displ) {
  ...
  for (hsize_t ii = 0; ii < num_objs; ++ii) {
    ...
    for (unsigned long long jj = 0; jj < n_gals; ++jj) {
      ...
      memcpy(permuted_data.data() + jj * elem_size,
             field_data.data() + src_idx * elem_size, elem_size);
    }
    ...
    hpc::h5::property_list props(H5P_DATASET_XFER);
    props.set_preserve();
    out_ds.write(permuted_data.data(), field_type, out_mem_space,
                 out_file_space, hpc::mpi::comm::self, props);
  }
}
...
void sage2kdtree_application::_write_kdtree(
  ...
  // Write bounds (matching dstreeinit.cc lines 327-334)
  {
    ...
    der.add(hpc::h5::datatype::native_double, 0, hpc::h5::datatype::ieee_f64be,
            "minimum");
    der.add(hpc::h5::datatype::native_double, sizeof(double),
            hpc::h5::datatype::ieee_f64be, "maximum");
...
void sage2kdtree_application::_write_empty_kdtree(hpc::h5::file &file,
                                                  int snap_num) {
  ...
  // Write empty splits (size 0)
  {
    ...
    der.add(hpc::h5::datatype::native_double, 0, hpc::h5::datatype::ieee_f64be,
            "position");
    der.add(hpc::h5::datatype::native_uint, sizeof(double),
            hpc::h5::datatype::std_i32be, "dimension");
...
void sage2kdtree_application::_write_sed_data(hpc::h5::file &out_file,
                                              hpc::fs::path const &tree_fn) {
