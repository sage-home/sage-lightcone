...
struct data_permuter
{
    ...
    data_permuter(std::array<std::vector<double>, 3>& crds,
                  std::vector<unsigned long long>& tree_idxs, std::vector<unsigned>& subsize,
...
private:
    ...
    unsigned _depthfirst_recurse(unsigned idx, hpc::view<std::vector<TreeNode>>& gals,
...
void sage2kdtree_application::operator()()
{
    ...
    try
    {
        ...
        metrics.push_back({"Phase 1", std::chrono::duration<double>(p1_end - p1_start).count(),
                           get_peak_rss_kb()});
        ...
        metrics.push_back({"Phase 2", std::chrono::duration<double>(p2_end - p2_start).count(),
                           get_peak_rss_kb()});
        ...
        metrics.push_back({"Phase 3", std::chrono::duration<double>(p3_end - p3_start).count(),
                           get_peak_rss_kb()});
...
void sage2kdtree_application::phase1_sageh5_to_depthfirst()
{
    ...
    if (_verb && _comm->rank() == 0)
    {
        std::cout << "  → Found " << total_trees << " trees with " << total_gals << " galaxies"
                  << std::endl;
    }
    ...
    if (!input_files.empty())
    {
        ...
        for (int snap = 0; snap < 64; ++snap)
        {
            ...
            for (hsize_t i = 0; i < num_objs; ++i)
            {
                ...
                std::string desc =
                    read_dataset_string_attribute(snap_group, sage_field_name, "Description");
                std::string units =
        ...
        if (_verb >= 2 && _comm->rank() == 0)
        {
            std::cout << "  → Loaded metadata for " << field_metadata.size() << " fields"
                      << std::endl;
        }
        ...
        if (!tao::validate_sage_fields(available_fields, missing_fields))
        {
            if (_comm->rank() == 0)
            {
                ...
                std::cerr << "\nPlease check that your SAGE HDF5 files contain all "
                             "required fields."
                          << std::endl;
                std::cerr << "See FIELD_NAMING.md for the complete list of mandatory fields."
                          << std::endl;
            }
    ...
    field_list.push_back(
        {"local_index", hpc::h5::datatype::native_int, 0, "Local index within snapshot", ""});
    field_list.push_back({"global_index", hpc::h5::datatype::native_llong, 0,
                          "Global index across all galaxies", ""});
    field_list.push_back(
        {"descendant", hpc::h5::datatype::native_int, 0, "Local index of descendant", ""});
    ...
    if (_verb >= 2 && _comm->rank() == 0)
    {
        std::cout << "  → Using columnar storage with " << field_list.size() << " fields"
                  << std::endl;
    }
    ...
    for (const auto& field : field_list)
    {
        ...
        {
            std::cerr << "  ⚠  Duplicate lowercase key '" << lookup_key << "' for field '"
                      << field.name << "' - skipping" << std::endl;
        }

        // Write attributes to dataset
        if (!field.description.empty())
        {
            write_dataset_string_attribute(out_file, "fields/" + field.name, "Description",
                                           field.description);
        }
    ...
    if (_verb >= 2 && _comm->rank() == 0)
    {
        std::cout << "  → Created " << field_datasets.size() << " dataset entries in map"
                  << std::endl;
    }
    ...
    for (size_t fidx = 0; fidx < my_files.size(); ++fidx)
    {
        ...
        if (_verb >= 2 && _comm->rank() == 0)
        {
            std::cout << "    Processing file " << (fidx + 1) << "/" << my_files.size() << ": "
                      << fn.filename() << " (" << file_tree_counts[fidx] << " trees, "
                      << file_gal_counts[fidx] << " galaxies)" << std::endl;
        }
        ...
        for (size_t b_idx = 0; b_idx < batches.size(); ++b_idx)
        {
            ...
            for (size_t t = 0; t < batch_n_trees; ++t)
            {
                ...
                for (size_t new_idx = 0; new_idx < n_gals; ++new_idx)
                {
                    ...
                    for (const auto& entry : gf.int_fields)
                    {
                        std::string key = entry.first;
                        std::transform(key.begin(), key.end(), key.begin(), ::tolower);
                        if (computed_fields.count(key) == 0)
            ...
            if (_verb >= 2 && _comm->rank() == 0)
            {
                std::cout << "      Writing batch " << (b_idx + 1) << " (" << actual_batch_gals
                          << " galaxies)..." << std::endl;
            }
            ...
            for (const auto& entry : batch_int_fields)
            {
                ...
                if (it != field_datasets.end() && it->second)
                {
                    ...
                    file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)actual_batch_gals,
                                                (hsize_t)current_gal_write_offset);
                    it->second->write(data.data(), hpc::h5::datatype::native_int, mem_space,
                                      file_space);
                }
            }
            ...
            for (const auto& entry : batch_llong_fields)
            {
                ...
                if (it != field_datasets.end() && it->second)
                {
                    ...
                    file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)actual_batch_gals,
                                                (hsize_t)current_gal_write_offset);
                    it->second->write(data.data(), hpc::h5::datatype::native_llong, mem_space,
                                      file_space);
                }
            }
            ...
            for (const auto& entry : batch_float_fields)
            {
                ...
                if (it != field_datasets.end() && it->second)
                {
                    ...
                    file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)actual_batch_gals,
                                                (hsize_t)current_gal_write_offset);
                    it->second->write(data.data(), hpc::h5::datatype::native_float, mem_space,
                                      file_space);
                }
            }
    ...
    if (_comm->rank() == _comm->size() - 1)
    {
        ...
        tree_displs_dset.write(&total_gals, hpc::h5::datatype::native_ullong, mem_space,
                               file_space);
    }
...
unsigned
sage2kdtree_application::_depthfirst_recurse(unsigned idx, hpc::view<std::vector<TreeNode>>& gals,
                                             std::vector<unsigned>& map, unsigned& dfi,
                                             std::multimap<unsigned, unsigned> const& parents)
{
    map[idx] = dfi++;
...
void sage2kdtree_application::phase2_add_traversal_metadata()
{
    ...
    if (_verb >= 2 && my_rank == 0)
    {
        std::cout << "  → Found " << phase1_fields.size() << " fields in Phase 1 output"
                  << std::endl;
    }
    ...
    for (const auto& field : phase1_fields)
    {
        out_datasets[field.name] =
            std::make_shared<hpc::h5::dataset>(out_file, "fields/" + field.name, field.type,
                                               hpc::h5::dataspace(total_gals), no_fill_props);

        // Copy attributes from Phase 1 input
        std::string desc =
            read_dataset_string_attribute(in_file, "fields/" + field.name, "Description");
        std::string units = read_dataset_string_attribute(in_file, "fields/" + field.name, "Units");

        if (!desc.empty())
            write_dataset_string_attribute(out_file, "fields/" + field.name, "Description", desc);
        if (!units.empty())
            write_dataset_string_attribute(out_file, "fields/" + field.name, "Units", units);
    }
    ...
    out_datasets["breadthfirst_traversalorder"] = std::make_shared<hpc::h5::dataset>(
        out_file, "fields/breadthfirst_traversalorder", hpc::h5::datatype::native_llong,
        hpc::h5::dataspace(total_gals), no_fill_props);
    out_datasets["depthfirst_traversalorder"] = std::make_shared<hpc::h5::dataset>(
        out_file, "fields/depthfirst_traversalorder", hpc::h5::datatype::native_llong,
        hpc::h5::dataspace(total_gals), no_fill_props);
    ...
    {
        write_dataset_string_attribute(out_file, "fields/" + kv.first, "Description",
                                       kv.second.first);
    ...
    if (_verb >= 2 && _comm->rank() == 0)
    {
        std::cout << "  → Copying " << phase1_fields.size() << " fields from Phase 1 output..."
                  << std::endl;
    }
    ...
    for (const auto& field : phase1_fields)
    {
        ...
        if (_verb >= 2 && _comm->rank() == 0)
        {
            ...
            std::cout << "    Copying field: " << fname << " [" << type_name << ", " << type_size
                      << " bytes, " << (n_loc * type_size / 1024.0 / 1024.0) << " MB] ..."
                      << std::flush;
        }
    ...
    for (size_t b_idx = 0; b_idx < batches.size(); ++b_idx)
    {
        ...
        if (_verb >= 2 && _comm->rank() == 0)
        {
            if (batches.size() > 1)
            {
                std::cout << "    Processing batch " << (b_idx + 1) << "/" << batches.size() << " ("
                          << batch_n_trees << " trees, " << (batch.total_gals / 1000.0)
                          << "K galaxies)" << std::endl;
            }
            log_mem("Start Batch " + std::to_string(b_idx + 1) + "/" +
                    std::to_string(batches.size()));
        ...
        get_dataset("descendant")
            .read(descendant_batch.data(), hpc::h5::datatype::native_int, batch_mem_space,
                  batch_file_space);
        ...
        for (size_t t = batch.start_tree; t < batch.end_tree; ++t)
        {
            ...
            while (!queue.empty())
            {
                ...
                std::sort(u->progenitors.begin(), u->progenitors.end(),
                          [](Node* a, Node* b) { return a->original_index < b->original_index; });
        ...
        out_datasets["globaltreeid"]->write(globaltreeid_batch.data(),
                                            hpc::h5::datatype::native_llong, batch_mem_space,
                                            batch_file_space);
        out_datasets["breadthfirst_traversalorder"]->write(bfs_order_batch.data(),
                                                           hpc::h5::datatype::native_llong,
                                                           batch_mem_space, batch_file_space);
        out_datasets["depthfirst_traversalorder"]->write(dfs_order_batch.data(),
                                                         hpc::h5::datatype::native_llong,
                                                         batch_mem_space, batch_file_space);
        ...
        out_datasets["localgalaxyid"]->write(localgalaxyid_batch.data(),
                                             hpc::h5::datatype::native_int, batch_mem_space,
                                             batch_file_space);
...
void sage2kdtree_application::phase3_tree_to_snapshot()
{
    ...
    std::vector<unsigned long long> snap_counts(64, 0);
    {
        ...
        file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)n_loc, (hsize_t)offs);
        get_dataset("snapnum").read(snapnums.data(), hpc::h5::datatype::native_int, mem_space,
                                    file_space);
    ...
    for (int snap = 0; snap < 64; ++snap)
    {
        ...
        for (const auto& fname : all_field_names)
        {
            ...
            std::string desc =
                read_dataset_string_attribute(tree_file, "fields/" + fname, "Description");
            std::string units =
                read_dataset_string_attribute(tree_file, "fields/" + fname, "Units");

            std::string snap_path = ss.str() + "/" + fname;
            if (!desc.empty())
                write_dataset_string_attribute(out_file, snap_path, "Description", desc);
            if (!units.empty())
    ...
    for (const auto& fname : all_field_names)
    {
        ...
        for (unsigned long long i = 0; i < n_loc; ++i)
        {
            ...
            if (snap >= 0 && snap < 64)
            {
                ...
                snap_buffers[snap].insert(snap_buffers[snap].end(), elem_ptr, elem_ptr + elem_size);
            }
        }
        ...
        for (int snap = 0; snap < 64; ++snap)
        {
            ...
            if (local_count > 0)
            {
                ...
                write_file_space.select_hyperslab(H5S_SELECT_SET, (hsize_t)local_count,
                                                  (hsize_t)my_offset);
                snap_datasets[snap][fname].write(snap_buffers[snap].data(), dtype, write_mem_space,
                                                 write_file_space);
            }
            else
            {
...
void sage2kdtree_application::phase4_build_kdtree_index()
{
    ...
    hpc::h5::datatype lc_type(H5Tcreate(
        H5T_COMPOUND, sizeof(double) * 3 + sizeof(unsigned long long) + sizeof(unsigned)));
    ...
    if (_comm->rank() == 0 && total_gals > 0)
    {
        ...
        for (int snap = 0; snap < n_snapshots; ++snap)
        {
            ...
            if (snap_counts[snap] > 0 && snap_file.has_link(ss.str()))
            {
                ...
                static const std::set<std::string> computed_fields = {"breadthfirst_traversalorder",
                                           ...
                                           std::vector<unsigned>& subsize)
{
    ...
    file.dataset(subtree_count_path)
        .read(subtree_count_data.data(), hpc::h5::datatype::native_llong, mem_space, sp);
