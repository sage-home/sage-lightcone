...
public:
  application(int argc, char *argv[])
      : hpc::mpi::application(argc, argv),
        ...
        _box_size(0), _hubble(0), _omega_l(0), _omega_m(0) {
    // Setup some options.
    options().add_options()(
        "mode,m", hpc::po::value<std::string>(&_mode)->default_value("convert"),
        "mode of operation")(
        ...
        "SAGE expansion list file")(
        "output,o", hpc::po::value<hpc::fs::path>(&_out_fn), "output file")(
        "verbose,v", hpc::po::value<int>(&_verb)->default_value(0),
        "verbosity");
  ...
  void convert() {
    ...
    if (hpc::fs::is_directory(_sage_dir)) {
      ...
      for (hpc::fs::directory_iterator dir_itr(_sage_dir); dir_itr != end_iter;
        ...
        if (hpc::fs::is_regular_file(dir_itr->status())) {
          ...
          std::cout << "Checking file: " << dir_itr->path() << " stem: " << stem
                    << std::endl;
          if (ext == ".hdf5" &&
    ...
    for (const auto &fn : my_files) {
      ...
      if (f.has_link("Snap_63/SAGETreeIndex")) {
        ...
        std::cout << "Snap_63 has " << tree_indices.size() << " galaxies with "
                  << unique_trees.size() << " unique tree indices" << std::endl;
        if (!tree_indices.empty()) {
          ...
          for (int iii = 0; iii < tree_indices.size(); ++iii) {
            if (tree_indices[iii] == max_tree_idx) {
              std::cout << "found in Snap_63: " << iii
                        << " tree_idx: " << tree_indices[iii]
      ...
      // Fallback to TreeInfo if Snap_63 didn't give us trees (e.g. empty
      // snapshot)
      ...
      for (int snap = 0; snap < 64; ++snap) { // Assuming 64 snapshots
        ...
        if (g.has_link("SAGETreeIndex")) {
          ...
          if (snap == 63 && tree_indices.size() > 0) {
            std::cout << "Snap 63: Found " << tree_indices.size()
                      << " galaxies. First tree_idx: " << tree_indices[0]
                      << std::endl;
          }
          ...
          for (int tree_idx : tree_indices) {
            if (tree_idx >= 0 && (size_t)tree_idx < n_trees) {
              ...
              std::cout << "Invalid tree_idx: " << tree_idx
                        << " n_trees: " << n_trees << std::endl;
            }
          }
        } else {
    ...
    // Global offsets
    // scan() returns exclusive prefix sum by default, so it is already the
    // offset.
    ...
    sage::make_hdf5_sidecar(
        "Millennium", _out_fn.native(),
        std::set<double>(_redshifts.begin(), _redshifts.end()), _hubble,
        _box_size);

    hpc::h5::file out_file(_out_fn.native(), H5F_ACC_TRUNC, *_comm);
    hpc::h5::dataset gals_dset(out_file, "galaxies", _file_type,
    ...
    gals_buf.create(gals_dset, _mem_type, hpc::h5::buffer_default_size,
                    global_gal_offset);
    ...
    for (size_t fidx = 0; fidx < my_files.size(); ++fidx) {
      ...
      for (int snap = 0; snap < 64; ++snap) {
        ...
        LOGILN("Snap ", snap, " has ", n_gals_snap,
               " galaxies. First tree index: ", tree_indices[0]);
        ...
        std::vector<float> posx, posy, posz, velx, vely, velz, spinx, spiny,
            spinz;
        std::vector<float> mvir, central_mvir, rvir, vvir, vmax, vel_disp;
        std::vector<float> cold_gas, stellar_mass, bulge_mass, hot_gas,
            ejected_mass, blackhole_mass, ics;
        std::vector<float> metals_cold_gas, metals_stellar_mass,
            metals_bulge_mass, metals_hot_gas, metals_ejected_mass, metals_ics;
        std::vector<float> sfr_disk, sfr_bulge, sfr_disk_z, sfr_bulge_z;
        std::vector<float> disk_radius, cooling, heating, quasar_acc,
            last_major, last_minor, outflow;
        std::vector<float> infall_mvir, infall_vmax, infall_vvir;
        std::vector<long long> gal_idx, central_gal_idx, sim_halo_idx;
        std::vector<int> sage_halo_idx, type, merge_type, merge_into_id,
            merge_into_snap;
        std::vector<float> dt;
        std::vector<int> len;
        ...
        read_col("Vely", vely);
        read_col("Velz", velz);
        read_col("Spinx", spinx);
        read_col("Spiny", spiny);
        read_col("Spinz", spinz);
        read_col("Mvir", mvir);
        read_col("CentralMvir", central_mvir);
        read_col("Rvir", rvir);
        read_col("Vvir", vvir);
        read_col("Vmax", vmax);
        read_col("VelDisp", vel_disp);
        read_col("ColdGas", cold_gas);
        read_col("StellarMass", stellar_mass);
        read_col("BulgeMass", bulge_mass);
        read_col("HotGas", hot_gas);
        read_col("EjectedMass", ejected_mass);
        read_col("BlackHoleMass", blackhole_mass);
        read_col("IntraClusterStars", ics);
        read_col("MetalsColdGas", metals_cold_gas);
        read_col("MetalsStellarMass", metals_stellar_mass);
        read_col("MetalsBulgeMass", metals_bulge_mass);
        read_col("MetalsHotGas", metals_hot_gas);
        read_col("MetalsEjectedMass", metals_ejected_mass);
        read_col("MetalsIntraClusterStars", metals_ics);
        read_col("SfrDisk", sfr_disk);
        read_col("SfrBulge", sfr_bulge);
        read_col("SfrDiskZ", sfr_disk_z);
        read_col("SfrBulgeZ", sfr_bulge_z);
        read_col("DiskRadius", disk_radius);
        read_col("Cooling", cooling);
        read_col("Heating", heating);
        read_col("QuasarModeBHaccretionMass", quasar_acc);
        read_col("TimeOfLastMajorMerger", last_major);
        read_col("TimeOfLastMinorMerger", last_minor);
        read_col("OutflowRate", outflow);
        read_col("infallMvir", infall_mvir);
        read_col("infallVmax", infall_vmax);
        read_col("infallVvir", infall_vvir);
        read_col("GalaxyIndex", gal_idx);
        read_col("CentralGalaxyIndex", central_gal_idx);
        read_col("SimulationHaloIndex", sim_halo_idx);
        read_col("SAGEHaloIndex", sage_halo_idx);
        read_col("Type", type);
        read_col("mergeType", merge_type);
        ...
        for (size_t i = 0; i < n_gals_snap; ++i) {
          ...
          g.mvir = mvir[i];
          g.central_mvir = central_mvir[i];
          g.rvir = rvir[i];
          g.vvir = vvir[i];
          g.vmax = vmax[i];
          g.vel_disp = vel_disp[i];

          g.cold_gas = cold_gas[i];
          g.stellar_mass = stellar_mass[i];
          g.bulge_mass = bulge_mass[i];
          g.hot_gas = hot_gas[i];
          g.ejected_mass = ejected_mass[i];
          g.blackhole_mass = blackhole_mass[i];
          g.ics = ics[i];

          g.metals_cold_gas = metals_cold_gas[i];
          g.metals_stellar_mass = metals_stellar_mass[i];
          g.metals_bulge_mass = metals_bulge_mass[i];
          g.metals_hot_gas = metals_hot_gas[i];
          g.metals_ejected_mass = metals_ejected_mass[i];
          g.metals_ics = metals_ics[i];

          g.sfr_disk = sfr_disk[i];
          g.sfr_bulge = sfr_bulge[i];
          g.sfr_disk_z = sfr_disk_z[i];
          g.sfr_bulge_z = sfr_bulge_z[i];

          g.disk_scale_radius = disk_radius[i];
          g.cooling = cooling[i];
          g.heating = heating[i];
          g.quasar_mode_bh_accretion_mass = quasar_acc[i];
          g.time_of_last_major_merger = last_major[i];
          g.time_of_last_minor_merger = last_minor[i];
          g.outflow_rate = outflow[i];

          g.infall_mvir = infall_mvir[i];
          g.infall_vmax = infall_vmax[i];
          g.infall_vvir = infall_vvir[i];
      ...
      for (size_t t = 0; t < n_trees; ++t) {
        ...
        std::cout << "Writing Tree " << t << " size: " << tree_gals.size()
                  << std::endl;
        //}
        if (tree_gals.empty()) {
        ...
        for (size_t i = 0; i < tree_gals.size(); ++i) {
          ...
          // Check for merges
          hpc::varray<unsigned, 2> merge_id{
              (unsigned)g.local_index,
              (unsigned)
                  g.snapshot}; // Wait, local_index here is not file_gal_idx.
          ...
          // "mergeIntoID <HDF5 dataset "mergeIntoID": shape (35899,), type
          // "<i4">" If it's <i4>, it's likely an index, not a long long
          // GalaxyIndex. If it's an index, is it the index within the snapshot?
          // Or within the tree? SAGE HDF5 documentation says: "mergeIntoID: The
          // unique ID of the galaxy that this galaxy merges into." But
          // GalaxyIndex is i8 (long long). mergeIntoID is i4. This suggests
          // mergeIntoID is NOT GalaxyIndex. It might be the index in the
          // snapshot array? If so, I need to know the index in the snapshot
          // array to resolve merges. But I've lost that index when I pushed to
          // `trees`. I need to store the original snapshot index in
          // `sage::galaxy` temporarily? Or, maybe mergeIntoID is the
          // `SAGEHaloIndex`? No. Let's assume it's the index in the snapshot
          // file. So I need to track `file_gal_idx` (index in snapshot). I can
          // store it in `local_index` temporarily before reordering? No,
          // `local_index` is used for output. I'll add a temporary field or use
          // `local_index` and reset it later. Let's use `local_index` to store
          // `file_gal_idx` during reading.
        }
        ...
        // First pass: populate maps using the stored file_gal_idx (in
        // local_index) But wait, I need to process in order. The `trees` vector
        // is sorted by snapshot (0..63).

        for (size_t i = 0; i < tree_gals.size(); ++i) {
          auto &g = tree_gals[i];
          // g.local_index currently holds the index in the snapshot file (from
          // reading loop)
    ...
    if (_comm->rank() == _comm->size() - 1) {
      ...
      // After the loop, `current_tree_displ` is the start of the *next* tree
      // (or end of last). So I should write one more value. But
      // `tree_displs_buf` is sized for `my_tot_trees`. If I write one more, it
      // might go out of bounds of the buffer's expected count? Actually,
      // `tree_displs` dataset has size `total_trees + 1`. Each rank writes
      // `my_tot_trees` entries. The last rank needs to write the final entry.
  ...
  void depthfirst_ordering(hpc::view<std::vector<sage::galaxy>> gals) {
    ...
    for (unsigned ii = 0; ii < gals.size(); ++ii) {
      // ASSERT(gidxs[0] + ii <= gidxs[1]); // This assertion might fail if gaps
      // exist, but here we are contiguous
  ...
  unsigned
  _depthfirst_recurse(unsigned idx, hpc::view<std::vector<sage::galaxy>> &gals,
