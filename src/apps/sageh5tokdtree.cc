...
public:
  sageh5tokdtree_app(int argc, char *argv[])
      : hpc::mpi::application(argc, argv) {
    LOG_PUSH(new hpc::mpi::logger("sageh5tokdtree.log.", hpc::log::info));

    options().add_options()(
        "sage,s", hpc::po::value<hpc::fs::path>(&_sage_fn)->required(),
        "Input SAGE HDF5 file")(
        "output,o", hpc::po::value<hpc::fs::path>(&_out_fn)->required(),
        "Output KDTree HDF5 file")(
  ...
  void operator()() {
    ...
    hpc::h5::file in_file(_sage_fn.string(), H5F_ACC_RDONLY,
                          hpc::mpi::comm::world);
    hpc::h5::file out_file(_out_fn.string(), H5F_ACC_TRUNC,
                           hpc::mpi::comm::world);
...
private:
  void build_tree_structure(hpc::h5::file &in, int n_snaps) {
    ...
    for (int i = 0; i < n_snaps; ++i) {
      ...
      bool has_mergers = snap_group.has_link("mergeIntoSnapNum") &&
                         snap_group.has_link("mergeIntoID");
      if (has_mergers) {
    ...
    std::sort(all_gals.begin(), all_gals.end(),
              [](const GalaxyInfo &a, const GalaxyInfo &b) {
    ...
    while (start_idx < all_gals.size()) {
      ...
      while (end_idx < all_gals.size() &&
             all_gals[end_idx].tree_id == all_gals[start_idx].tree_id) {
        end_idx++;
      }
  ...
  void process_single_tree(std::vector<GalaxyInfo> &gals, size_t start,
                           size_t end) {
    ...
    for (size_t i = start; i < end; ++i) {
      if (_subsize_lookup[gals[i].snap_num].size() <= gals[i].file_index) {
        _subsize_lookup[gals[i].snap_num].resize(gals[i].file_index +
                                                 1000); // Resize with buffer
      }
      // Ensure size
      if (_subsize_lookup[gals[i].snap_num].size() <= gals[i].file_index) {
  ...
  void write_kdtree(
      hpc::h5::group &lc_snap, hpc::kdtree<> const &kdt,
    ...
    // Write counts and offsets.
    {
      ...
      hpc::h5::dataset dset(lc_snap, "cell_counts",
                            hpc::h5::datatype::native_uint, counts.size());
      dset.write(counts);
    }
    {
      ...
      hpc::h5::dataset dset(lc_snap, "cell_offs",
                            hpc::h5::datatype::native_ullong, offs.size());
      dset.write(offs);
    }
  }

  void append_dataset_compound(hpc::h5::group &data_group,
                               const std::string &name,
                               hpc::h5::datatype &mem_type,
                               hpc::h5::datatype &file_type, const void *data,
                               size_t count, unsigned long long offset) {
    ...
    if (data_group.has_link(name)) {
      ...
      hid_t dset_id = H5Dcreate2(data_group.id(), name.c_str(), file_type.id(),
  ...
  void append_lightcone_data(hpc::h5::group &lc_group,
    ...
    der.add(hpc::h5::datatype::native_double, HOFFSET(particle_t, x),
            hpc::h5::datatype::ieee_f64be, "x");
    der.add(hpc::h5::datatype::native_double, HOFFSET(particle_t, y),
            hpc::h5::datatype::ieee_f64be, "y");
    der.add(hpc::h5::datatype::native_double, HOFFSET(particle_t, z),
            hpc::h5::datatype::ieee_f64be, "z");
    der.add(hpc::h5::datatype::native_llong, HOFFSET(particle_t, global_index),
            hpc::h5::datatype::std_u64be, "global_index");
    der.add(hpc::h5::datatype::native_llong, HOFFSET(particle_t, galaxy_index),
            hpc::h5::datatype::std_u64be, "central_galaxy_index");
    der.add(hpc::h5::datatype::native_uint, HOFFSET(particle_t, subsize),
            hpc::h5::datatype::std_u32be, "subsize");

    hpc::h5::datatype mdt, fdt;
    der.commit(mdt, fdt);

    append_dataset_compound(lc_group, "data", mdt, fdt, data.data(), count,
                            offset);
  }

  void append_dataset(hpc::h5::group &data_group, const std::string &name,
                      hid_t type_id, size_t element_size, const void *data,
                      size_t count, unsigned long long offset) {
    ...
    if (data_group.has_link(name)) {
      ...
      hid_t dset_id = H5Dcreate2(data_group.id(), name.c_str(), type_id,
                                 space_id, H5P_DEFAULT, plist_id, H5P_DEFAULT);
      dset = hpc::h5::dataset(dset_id, false);
  ...
  void permute_and_write_generic(hpc::h5::group &in_group,
                                 const std::string &name,
    ...
    append_dataset(out_data_group, name, type_id, size, permuted.data(),
                   npoints, offset);
  ...
  void copy_cosmology(hpc::h5::file &in, hpc::h5::file &out) {
    ...
    if (in.has_link("Header/Simulation")) {
      ...
      LOGILN("Header/Simulation group not found. Cosmology info might be "
             "missing.");
    }
  }

  void copy_attr(hpc::h5::group &in, hpc::h5::group &out,
                 const std::string &name, const std::string &out_name = "") {
    hid_t in_id = in.id();
    if (H5Aexists(in_id, name.c_str()) > 0) {
      ...
      if (attr_id >= 0) {
        ...
        hssize_t npoints = H5Sget_simple_extent_npoints(space_id);
        if (npoints > 0)
          buf.resize(size * npoints);

        H5Aread(attr_id, type_id, buf.data());

        hid_t out_id = out.id();
        std::string oname = out_name.empty() ? name : out_name;
        hid_t out_attr_id = H5Acreate2(out_id, oname.c_str(), type_id, space_id,
  ...
  void process_snapshot(hpc::h5::file &in, int snap_idx, hpc::h5::file &out) {
    ...
    append_lightcone_data(lc_group, crds, permuted_global_idxs, galaxy_idxs,
                          subsize, _snapshot_displs.back());
    ...
    // We need to pass the permutation vector that maps OutputIndex ->
    // InputIndex. 'idxs' currently holds the KDTree permutation of the ALREADY
    // SORTED data. We need a vector that maps OutputIndex ->
    // OriginalInputIndex. Let's reconstruct it. Step 1: Sort by SAGETreeIndex
    // -> 'sorted_idxs' (maps Sorted -> Original) Step 2: KDTree partition ->
    // 'idxs' (maps KDTree -> Sorted) Combined: 'final_idxs' (maps KDTree ->
    // Original) = sorted_idxs[idxs[i]]
    ...
    std::stable_sort(sorted_idxs.begin(), sorted_idxs.end(),
                     [&](unsigned long long a, unsigned long long b) {
                       return sage_tree_idxs[a] < sage_tree_idxs[b];
                     });
    ...
    callback_data cb_data = {this, &snap_group, &data_group, &final_idxs,
                             offset};

    H5Literate(
        snap_group.id(), H5_INDEX_NAME, H5_ITER_NATIVE, NULL,
        [](hid_t g_id, const char *name, const H5L_info_t *info,
           void *op_data) -> herr_t {
          auto *data = static_cast<callback_data *>(op_data);
          data->app->permute_and_write_generic(*data->snap_group, name,
                                               *data->data_group, *data->idxs,
                                               data->offset);
          return 0;
        },
        &cb_data);
    ...
    append_dataset(data_group, "subsize", H5T_NATIVE_UINT, sizeof(unsigned),
                   subsize.data(), n_gals, offset);

    // Write global_index to data group (already permuted)
    append_dataset(data_group, "global_index", H5T_NATIVE_LLONG,
                   sizeof(long long), permuted_global_idxs.data(), n_gals,
                   offset);
  ...
  template <typename T>
  void read_dataset(hpc::h5::group &g, const std::string &name,
                    std::vector<T> &vec) {
    if (g.has_link(name)) {
